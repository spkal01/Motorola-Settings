...
public class UidDetailProvider {
    ...
    private UidDetail buildUidDetail(int i) {
        ...
        if (i3 == -10) {
            uidDetail.label = resources.getString(R$string.video_data_label);
            try {
                uidDetail.icon = packageManager.getApplicationIcon("com.android.dialer");
            } catch (PackageManager.NameNotFoundException unused) {
                Log.w("DataUsage", "Phone app icon is not found, use default icon!");
                uidDetail.icon = packageManager.getDefaultActivityIcon();
            }
            return uidDetail;
        } else if (i3 == 1000) {
            ...
            if (!isKeyForUser(i) || (userInfo = userManager.getUserInfo(getUserIdForKey(i))) == null) {
                ...
                try {
                    ...
                    if (length == 1) {
                        if (InstalledAppUtils.get(this.mContext).isHiddenPackage(packagesForUid[0])) {
                            return null;
                        }
