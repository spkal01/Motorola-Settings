...
import com.android.settingslib.utils.AppCloneUtils;
...
public class TileUtils {
    ...
    public static List<DashboardCategory> getCategories(Context context, Map<Pair<String, String>, Tile> map) {
        ...
        for (UserHandle next : AppCloneUtils.getUserProfileExceptClone((UserManager) context.getSystemService("user"))) {
    ...
    private static void loadProviderTiles(Context context, UserHandle userHandle, Map<Pair<String, String>, Tile> map, String str, List<Tile> list, Intent intent) {
        for (ResolveInfo resolveInfo : context.getPackageManager().queryIntentContentProvidersAsUser(intent, 0, userHandle.getIdentifier())) {
            if (resolveInfo.system) {
                ...
                List<Bundle> dynamicTileDataFromProvider = getDynamicTileDataFromProvider(context, providerInfo.authority);
                if (dynamicTileDataFromProvider != null && !dynamicTileDataFromProvider.isEmpty()) {
                    for (Bundle loadTile : dynamicTileDataFromProvider) {
    ...
    static Bundle getDynamicTileDataFromProvider(Context context, String str, String str2) {
        return getBundleFromUri(context, buildUri(str, "getProviderTileData", str2), new ArrayMap(), (Bundle) null);
    }

    private static List<Bundle> getDynamicTileDataFromProvider(Context context, String str) {
        Bundle bundleFromUri = getBundleFromUri(context, buildUri(str, "getProviderTileData"), new ArrayMap(), (Bundle) null);
        if (bundleFromUri != null) {
            return bundleFromUri.getParcelableArrayList("provider_tile_data");
        }
        return null;
    }
    ...
    public static Integer getIntegerFromUri(Context context, Uri uri, Map<String, IContentProvider> map, String str) {
        Bundle bundleFromUri = getBundleFromUri(context, uri, map, (Bundle) null);
        if (bundleFromUri == null || !bundleFromUri.containsKey(str)) {
            return null;
        }
        return Integer.valueOf(bundleFromUri.getInt(str));
    }
