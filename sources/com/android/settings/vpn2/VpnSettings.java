...
public class VpnSettings extends RestrictedSettingsFragment implements Handler.Callback, Preference.OnPreferenceClickListener {
    ...
    public void onActivityCreated(Bundle bundle) {
        ...
        addPreferencesFromResource(C1994R$xml.vpn_settings2);
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater menuInflater) {
        ...
        if (!Utils.isProviderModelEnabled(getContext()) || getContext().getPackageManager().hasSystemFeature("android.software.ipsec_tunnels")) {
            menuInflater.inflate(C1988R$menu.vpn, menu);
        } else {
    ...
    public boolean onOptionsItemSelected(MenuItem menuItem) {
        if (menuItem.getItemId() != C1985R$id.vpn_create) {
    ...
    public void onResume() {
        ...
        if (hasUserRestriction) {
            if (!isUiRestrictedByOnlyAdmin()) {
                getEmptyTextView().setText(C1992R$string.vpn_settings_not_available);
            }
            getPreferenceScreen().removeAll();
            return;
        }
        setEmptyView(getEmptyTextView());
        getEmptyTextView().setText(C1992R$string.vpn_no_vpns_added);
    ...
    public int getHelpResource() {
        return C1992R$string.help_url_vpn;
    }
