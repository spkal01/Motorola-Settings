...
public class AppManagementFragment extends SettingsPreferenceFragment implements Preference.OnPreferenceChangeListener, Preference.OnPreferenceClickListener, ConfirmLockdownFragment.ConfirmLockdownListener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        addPreferencesFromResource(C1994R$xml.vpn_app_management);
    ...
    public void onResume() {
        super.onResume();
        if (loadInfo()) {
            this.mPreferenceVersion.setTitle((CharSequence) getPrefContext().getString(C1992R$string.vpn_version, new Object[]{this.mPackageInfo.versionName}));
            updateUI();
            return;
        }
        finish();
    ...
    private void updateRestrictedViews() {
        if (isAdded()) {
            ...
            if (this.mVpnManager.isAlwaysOnVpnPackageSupportedForUser(this.mUserId, this.mPackageName)) {
                this.mPreferenceAlwaysOn.setSummary(C1992R$string.vpn_always_on_summary);
                return;
            }
            this.mPreferenceAlwaysOn.setEnabled(false);
            this.mPreferenceLockdown.setEnabled(false);
            this.mPreferenceAlwaysOn.setSummary(C1992R$string.vpn_always_on_summary_not_supported);
        }
    }
    ...
    public static class CannotConnectFragment extends InstrumentedDialogFragment {
        ...
        public Dialog onCreateDialog(Bundle bundle) {
            ...
            return new AlertDialog.Builder(getActivity()).setTitle((CharSequence) getActivity().getString(C1992R$string.vpn_cant_connect_title, new Object[]{string})).setMessage((CharSequence) getActivity().getString(C1992R$string.vpn_cant_connect_message)).setPositiveButton(C1992R$string.okay, (DialogInterface.OnClickListener) null).create();
        }
    }
}
