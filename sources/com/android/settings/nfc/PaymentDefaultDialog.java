        ...
        if (componentName == null || str == null) {
            ...
            if (defaultPaymentApp == null || !defaultPaymentApp.equals(componentName)) {
                ...
                if (paymentAppInfo2 == null) {
                    alertParams.mTitle = getString(C1992R$string.nfc_payment_set_default_label);
                    alertParams.mMessage = String.format(getString(C1992R$string.nfc_payment_set_default), new Object[]{sanitizePaymentAppCaption(paymentAppInfo.label.toString())});
                    alertParams.mPositiveButtonText = getString(C1992R$string.nfc_payment_btn_text_set_deault);
                } else {
                    alertParams.mTitle = getString(C1992R$string.nfc_payment_update_default_label);
                    alertParams.mMessage = String.format(getString(C1992R$string.nfc_payment_set_default_instead_of), new Object[]{sanitizePaymentAppCaption(paymentAppInfo.label.toString()), sanitizePaymentAppCaption(paymentAppInfo2.label.toString())});
                    alertParams.mPositiveButtonText = getString(C1992R$string.nfc_payment_btn_text_update);
                }
                alertParams.mNegativeButtonText = getString(C1992R$string.cancel);
