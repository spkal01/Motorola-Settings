...
public class AutomaticStorageManagerDescriptionPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin {
    ...
    public void displayPreference(PreferenceScreen preferenceScreen) {
        ...
        if (j == 0 || j2 == 0 || !Utils.isStorageManagerEnabled(context)) {
            footerPreference.setSummary(C1992R$string.automatic_storage_manager_text);
            return;
        }
        footerPreference.setSummary((CharSequence) context.getString(C1992R$string.automatic_storage_manager_freed_bytes, new Object[]{Formatter.formatFileSize(context, j), DateUtils.formatDateTime(context, j2, 16)}));
    }
}
