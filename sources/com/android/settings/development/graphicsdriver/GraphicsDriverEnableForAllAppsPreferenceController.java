...
public class GraphicsDriverEnableForAllAppsPreferenceController extends BasePreferenceController implements Preference.OnPreferenceChangeListener, GraphicsDriverContentObserver.OnGraphicsDriverContentChangedListener, LifecycleObserver, OnStart, OnStop {
    ...
    public GraphicsDriverEnableForAllAppsPreferenceController(Context context, String str) {
        ...
        this.mPreferenceDefault = resources.getString(C1992R$string.graphics_driver_app_preference_default);
        this.mPreferenceProductionDriver = resources.getString(C1992R$string.graphics_driver_app_preference_production_driver);
        this.mPreferencePrereleaseDriver = resources.getString(C1992R$string.graphics_driver_app_preference_prerelease_driver);
    ...
    public static CharSequence[] constructEntryList(Context context, boolean z) {
        ...
        arrayList.add(resources.getString(C1992R$string.graphics_driver_app_preference_default));
        PackageManager packageManager = context.getPackageManager();
        if (!TextUtils.isEmpty(str) && hasDriverPackage(packageManager, str)) {
            arrayList.add(resources.getString(C1992R$string.graphics_driver_app_preference_prerelease_driver));
        }
        if (!TextUtils.isEmpty(str2) && hasDriverPackage(packageManager, str2)) {
            arrayList.add(resources.getString(C1992R$string.graphics_driver_app_preference_production_driver));
        }
        if (z) {
            arrayList.add(resources.getString(C1992R$string.graphics_driver_app_preference_system));
        }
