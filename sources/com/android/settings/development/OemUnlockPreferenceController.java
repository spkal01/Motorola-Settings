...
public class OemUnlockPreferenceController extends DeveloperOptionsPreferenceController implements Preference.OnPreferenceChangeListener, PreferenceControllerMixin, OnActivityResultListener {
    ...
    public void updateState(Preference preference) {
        ...
        if (this.mPreference.isEnabled()) {
            this.mPreference.checkRestrictionAndSetDisabled("no_moto_oem_unlock");
        }
    }
    ...
    private void updateOemUnlockSettingDescription() {
        int i = C1992R$string.oem_unlock_enable_summary;
        if (isBootloaderUnlocked()) {
            i = C1992R$string.oem_unlock_enable_disabled_summary_bootloader_unlocked;
        } else if (isSimLockedDevice()) {
            i = C1992R$string.oem_unlock_enable_disabled_summary_sim_locked_device;
        } else if (!isOemUnlockAllowedByUserAndCarrier()) {
            i = C1992R$string.oem_unlock_enable_disabled_summary_connectivity_or_locked;
        }
    ...
    public boolean showKeyguardConfirmation(Resources resources, int i) {
        return new ChooseLockSettingsHelper.Builder(this.mActivity, this.mFragment).setRequestCode(i).setTitle(resources.getString(C1992R$string.oem_unlock_enable)).show();
    }
    ...
    public boolean isOemUnlockAllowedByUserAndCarrier() {
        ...
        return this.mOemLockManager.isOemUnlockAllowedByCarrier() && !this.mUserManager.hasBaseUserRestriction("no_factory_reset", of) && !this.mUserManager.hasBaseUserRestriction("no_moto_oem_unlock", of);
    }
