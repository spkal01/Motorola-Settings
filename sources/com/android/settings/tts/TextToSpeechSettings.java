...
public class TextToSpeechSettings extends SettingsPreferenceFragment implements Preference.OnPreferenceChangeListener, GearPreference.OnGearClickListener {
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER = new BaseSearchIndexProvider(C1994R$xml.tts_settings);
    ...
    public void onCreate(Bundle bundle) {
        ...
        addPreferencesFromResource(C1994R$xml.tts_settings);
        ...
        boolean z = false;
        this.mActionButtons = ((ActionButtonsPreference) findPreference("action_buttons")).setButton1Text(C1992R$string.tts_play).setButton1OnClickListener(new TextToSpeechSettings$$ExternalSyntheticLambda2(this)).setButton1Enabled(false).setButton2Text(C1992R$string.tts_reset).setButton2OnClickListener(new TextToSpeechSettings$$ExternalSyntheticLambda1(this)).setButton1Enabled(true);
        ...
        Pair<TextToSpeech, Boolean> ttsAndWhetherNew = ((TextToSpeechViewModel) ViewModelProviders.m5of((Fragment) this).get(TextToSpeechViewModel.class)).getTtsAndWhetherNew(this.mInitListener);
    ...
    public void onResume() {
        ...
        if (textToSpeech != null) {
            ...
            if (TextUtils.isEmpty(this.mCurrentEngine) || (!TextUtils.isEmpty(defaultEngine) && !defaultEngine.equals(this.mTts.getCurrentEngine()))) {
                TextToSpeechViewModel textToSpeechViewModel = (TextToSpeechViewModel) ViewModelProviders.m5of((Fragment) this).get(TextToSpeechViewModel.class);
                try {
    ...
    private void updateDefaultLocalePref(Intent intent) {
        ...
        Locale locale = null;
        String currentEngine = this.mTts.getCurrentEngine();
        if (!TextUtils.isEmpty(currentEngine) && !this.mEnginesHelper.isLocaleSetToDefaultForEngine(currentEngine)) {
            locale = this.mEnginesHelper.getLocalePrefForEngine(currentEngine);
        }
        ...
        charSequenceArr[0] = getActivity().getString(C1992R$string.tts_lang_use_system);
        charSequenceArr2[0] = "";
    ...
    private void setLocalePreference(int i) {
        if (i < 0) {
            ...
            this.mLocalePreference.setSummary(C1992R$string.tts_lang_not_selected);
            return;
        }
    ...
    private String getDefaultSampleString() {
        ...
        if (!(textToSpeech == null || textToSpeech.getLanguage() == null)) {
            try {
                ...
                String[] stringArray = getActivity().getResources().getStringArray(C1978R$array.tts_demo_strings);
                String[] stringArray2 = getActivity().getResources().getStringArray(C1978R$array.tts_demo_string_langs);
        ...
        return getString(C1992R$string.tts_default_sample_string);
    }
    ...
    private void updateLanguageTo(Locale locale) {
        ...
        String currentEngine = this.mTts.getCurrentEngine();
        if (!TextUtils.isEmpty(currentEngine)) {
            this.mEnginesHelper.updateLocalePrefForEngine(currentEngine, locale);
        }
        TextToSpeech textToSpeech = this.mTts;
        if (locale == null) {
    ...
    private void displayNetworkAlert() {
        ...
        builder.setTitle(17039380).setMessage((CharSequence) getActivity().getString(C1992R$string.tts_engine_network_required)).setCancelable(false).setPositiveButton(17039370, (DialogInterface.OnClickListener) null);
        builder.create().show();
    }
