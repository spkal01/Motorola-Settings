...
public final class WifiTetherDisablePreferenceController extends TetherBasePreferenceController {
    ...
    public CharSequence getSummary() {
        ...
        if (tetheringStateOfOtherInterfaces == 2) {
            return this.mContext.getString(C1992R$string.disable_wifi_hotspot_when_usb_on);
        }
        if (tetheringStateOfOtherInterfaces == 4) {
            return this.mContext.getString(C1992R$string.disable_wifi_hotspot_when_bluetooth_on);
        }
        if (tetheringStateOfOtherInterfaces == 6) {
            return this.mContext.getString(C1992R$string.disable_wifi_hotspot_when_usb_and_bluetooth_on);
        }
        if (tetheringStateOfOtherInterfaces == 32) {
            return this.mContext.getString(C1992R$string.disable_wifi_hotspot_when_ethernet_on);
        }
        if (tetheringStateOfOtherInterfaces == 34) {
            return this.mContext.getString(C1992R$string.disable_wifi_hotspot_when_usb_and_ethernet_on);
        }
        if (tetheringStateOfOtherInterfaces == 36) {
            return this.mContext.getString(C1992R$string.disable_wifi_hotspot_when_bluetooth_and_ethernet_on);
        }
        if (tetheringStateOfOtherInterfaces != 38) {
            return this.mContext.getString(C1992R$string.summary_placeholder);
        }
        return this.mContext.getString(C1992R$string.disable_wifi_hotspot_when_usb_and_bluetooth_and_ethernet_on);
    }
