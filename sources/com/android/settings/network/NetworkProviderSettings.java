...
import com.motorola.settings.utils.SubsidyLockUtil;
import com.motorola.settings.wifi.EapSimRequiredDialog;
import com.motorola.settings.wifi.MotoDevicePolicyUtils;
import com.motorola.settings.wifi.UnsecuredNetworkDialog;
...
public class NetworkProviderSettings extends RestrictedSettingsFragment implements WifiPickerTracker.WifiPickerTrackerCallback, WifiDialog2.WifiDialog2Listener, DialogInterface.OnDismissListener, AirplaneModeEnabler.OnAirplaneModeChangedListener, InternetUpdater.InternetChangeListener {
    ...
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER = new BaseSearchIndexProvider(C1994R$xml.network_provider_settings) {
    ...
    final Runnable mHideProgressBarRunnable = new NetworkProviderSettings$$ExternalSyntheticLambda9(this);
    protected InternetResetHelper mInternetResetHelper;
    InternetUpdater mInternetUpdater;
    private boolean mIsJioSubsidyLocked = false;
    ...
    final Runnable mRemoveLoadingRunnable = new NetworkProviderSettings$$ExternalSyntheticLambda8(this);
    ...
    final Runnable mUpdateWifiEntryPreferencesRunnable = new NetworkProviderSettings$$ExternalSyntheticLambda5(this);
    ...
    public void onViewCreated(View view, Bundle bundle) {
        ...
        if (activity != null) {
            if (this.mIsJioSubsidyLocked) {
                activity.setShowWhenLocked(true);
            }
            setPinnedHeaderView(C1987R$layout.progress_header);
            setProgressBarVisible(false);
            ...
            if (wifiManager != null) {
                ...
                getView().postDelayed(this.mRemoveLoadingRunnable, this.mWifiManager.isWifiEnabled() ? 1000 : 100);
            }
        }
    }
    ...
    public void onCreate(Bundle bundle) {
        ...
        this.mIsJioSubsidyLocked = SubsidyLockUtil.isJioSubsidyLocked(getContext());
        if (Build.IS_DEBUGGABLE) {
            Log.d("NetworkProviderSettings", "mIsJioSubsidyLocked = " + this.mIsJioSubsidyLocked);
        }
        addPreferences();
    ...
    private void addPreferences() {
        addPreferencesFromResource(C1994R$xml.network_provider_settings);
        ...
        if (this.mIsJioSubsidyLocked) {
            removePreference(NetworkMobileProviderController.PREF_KEY_PROVIDER_MOBILE_NETWORK);
            removePreference("configure_wifi_settings");
            removePreference(PREF_KEY_DATA_USAGE);
        }
    }
    ...
    public void onActivityCreated(Bundle bundle) {
        ...
        this.mSaveListener = new WifiManager.ActionListener() {
            ...
            public void onFailure(int i) {
                ...
                if (activity != null) {
                    Toast.makeText(activity, C1992R$string.wifi_failed_save_message, 0).show();
                }
            }
        };
        this.mForgetListener = new WifiManager.ActionListener() {
            ...
            public void onFailure(int i) {
                FragmentActivity activity = NetworkProviderSettings.this.getActivity();
                if (activity != null) {
                    Toast.makeText(activity, C1992R$string.wifi_failed_forget_message, 0).show();
                }
            }
        };
    ...
    private void restrictUi() {
        if (!isUiRestrictedByOnlyAdmin()) {
            getEmptyTextView().setText(C1992R$string.wifi_empty_list_user_restricted);
        }
    ...
    public void onCreateContextMenu(ContextMenu contextMenu, View view, ContextMenu.ContextMenuInfo contextMenuInfo) {
        ...
        if (preference instanceof LongPressWifiEntryPreference) {
            ...
            if (this.mSelectedWifiEntry.canConnect()) {
                contextMenu.add(0, 2, 0, C1992R$string.wifi_connect);
            }
            if (this.mSelectedWifiEntry.canDisconnect()) {
                contextMenu.add(0, 7, 0, C1992R$string.share);
                contextMenu.add(0, 3, 1, C1992R$string.wifi_disconnect_button_text);
            }
            if (canForgetNetwork()) {
                contextMenu.add(0, 4, 0, C1992R$string.forget);
            }
            if (!WifiUtils.isNetworkLockedDown(getActivity(), this.mSelectedWifiEntry.getWifiConfiguration()) && this.mSelectedWifiEntry.isSaved() && this.mSelectedWifiEntry.getConnectedState() != 2) {
                contextMenu.add(0, 5, 0, C1992R$string.wifi_modify);
            }
        }
    }
    ...
    public boolean onContextItemSelected(MenuItem menuItem) {
        ...
        if (itemId == 2) {
            if (UnsecuredNetworkDialog.shouldShowDialog((Context) getActivity(), this.mSelectedWifiEntry)) {
                UnsecuredNetworkDialog.show(getActivity(), this.mSelectedWifiEntry.getSsid(), new NetworkProviderSettings$$ExternalSyntheticLambda0(this), (DialogInterface.OnClickListener) null);
            } else {
            ...
            WifiDppUtils.showLockScreen(getContext(), new NetworkProviderSettings$$ExternalSyntheticLambda6(this));
            return true;
        }
    }

    /* access modifiers changed from: private */
    public /* synthetic */ void lambda$onContextItemSelected$3(DialogInterface dialogInterface, int i) {
        connect(this.mSelectedWifiEntry, true, false);
    }
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        if (preference instanceof LongPressWifiEntryPreference) {
            WifiEntry wifiEntry = ((LongPressWifiEntryPreference) preference).getWifiEntry();
            if (MotoDevicePolicyUtils.applyAdminRestriction(wifiEntry, (WifiConfiguration) null, getActivity())) {
                if (!Build.IS_DEBUGGABLE) {
                    return false;
                }
                Log.d("NetworkProviderSettings", "-> Admin restriction was applied can't connect to network...");
                return false;
            } else if (wifiEntry.shouldEditBeforeConnect()) {
                launchConfigNewNetworkFragment(wifiEntry);
                return true;
            } else if (UnsecuredNetworkDialog.shouldShowDialog((Context) getActivity(), wifiEntry)) {
                UnsecuredNetworkDialog.show(getActivity(), wifiEntry.getSsid(), new NetworkProviderSettings$$ExternalSyntheticLambda1(this, wifiEntry), (DialogInterface.OnClickListener) null);
            } else {
    ...
    /* access modifiers changed from: private */
    public /* synthetic */ void lambda$onPreferenceTreeClick$5(WifiEntry wifiEntry, DialogInterface dialogInterface, int i) {
        connect(wifiEntry, true, true);
    }
    ...
    public void onInternetTypeChanged(int i) {
        ThreadUtils.postOnMainThread(new NetworkProviderSettings$$ExternalSyntheticLambda7(this));
    }

    /* renamed from: onWifiStateChanged */
    public void lambda$onInternetTypeChanged$6() {
    ...
    public void onWifiEntriesChanged() {
        if (!isFinishingOrDestroyed()) {
            ...
            if (this.mOpenSsid != null) {
                Optional findFirst = this.mWifiPickerTracker.getWifiEntries().stream().filter(new NetworkProviderSettings$$ExternalSyntheticLambda10(this)).filter(NetworkProviderSettings$$ExternalSyntheticLambda12.INSTANCE).filter(NetworkProviderSettings$$ExternalSyntheticLambda11.INSTANCE).findFirst();
    ...
    /* access modifiers changed from: private */
    public /* synthetic */ boolean lambda$onWifiEntriesChanged$7(WifiEntry wifiEntry) {
    ...
    /* access modifiers changed from: private */
    public static /* synthetic */ boolean lambda$onWifiEntriesChanged$9(WifiEntry wifiEntry) {
        ...
        if (getActivity() != null && getView() != null && !this.mIsRestricted && this.mWifiPickerTracker.getWifiState() == 3) {
            ...
            if (connectedWifiEntry != null) {
                ...
                if (longPressWifiEntryPreference == null || longPressWifiEntryPreference.getWifiEntry() != connectedWifiEntry) {
                    ...
                    createConnectedWifiEntryPreference.setOnPreferenceClickListener(new NetworkProviderSettings$$ExternalSyntheticLambda2(this, connectedWifiEntry, createConnectedWifiEntryPreference));
                    createConnectedWifiEntryPreference.setOnGearClickListener(new NetworkProviderSettings$$ExternalSyntheticLambda3(this, createConnectedWifiEntryPreference));
            ...
            for (WifiEntry next : this.mWifiPickerTracker.getWifiEntries()) {
                ...
                if (next.getHelpUriString() != null) {
                    createLongPressWifiEntryPreference.setOnButtonClickListener(new NetworkProviderSettings$$ExternalSyntheticLambda4(this, next));
                }
            ...
            if (!z) {
                ...
                preference.setSummary(C1992R$string.wifi_empty_list_wifi_on);
    ...
    /* access modifiers changed from: private */
    public /* synthetic */ boolean lambda$updateWifiEntryPreferences$10(WifiEntry wifiEntry, ConnectedWifiEntryPreference connectedWifiEntryPreference, Preference preference) {
    ...
    /* access modifiers changed from: private */
    public /* synthetic */ void lambda$updateWifiEntryPreferences$11(ConnectedWifiEntryPreference connectedWifiEntryPreference, ConnectedWifiEntryPreference connectedWifiEntryPreference2) {
        launchNetworkDetailsFragment(connectedWifiEntryPreference);
    }

    /* access modifiers changed from: private */
    public /* synthetic */ void lambda$updateWifiEntryPreferences$12(WifiEntry wifiEntry, WifiEntryPreference wifiEntryPreference) {
    ...
    private void launchNetworkDetailsFragment(LongPressWifiEntryPreference longPressWifiEntryPreference) {
        ...
        if (FeatureFlagUtils.isEnabled(context, "settings_wifi_details_datausage_header")) {
            ...
            charSequence = context.getText(C1992R$string.pref_title_network_details);
        }
    ...
    private void launchAddNetworkFragment() {
        new SubSettingLauncher(getContext()).setTitleRes(C1992R$string.wifi_add_network).setDestination(AddNetworkFragment.class.getName()).setSourceMetricsCategory(getMetricsCategory()).setResultListener(this, 2).launch();
    }
    ...
    public void setAdditionalSettingsSummaries() {
        ...
        if (!FeatureFlagUtils.isEnabled(getContext(), "settings_provider_model")) {
            ...
            if (isWifiWakeupEnabled()) {
                i = C1992R$string.wifi_configure_settings_preference_summary_wakeup_on;
            } else {
                i = C1992R$string.wifi_configure_settings_preference_summary_wakeup_off;
            }
    ...
    private String getSavedNetworkSettingsSummaryText(int i, int i2) {
        if (i2 == 0) {
            return getResources().getQuantityString(C1990R$plurals.wifi_saved_access_points_summary, i, new Object[]{Integer.valueOf(i)});
        } else if (i == 0) {
            return getResources().getQuantityString(C1990R$plurals.wifi_saved_passpoint_access_points_summary, i2, new Object[]{Integer.valueOf(i2)});
        } else {
            int i3 = i + i2;
            return getResources().getQuantityString(C1990R$plurals.wifi_saved_all_access_points_summary, i3, new Object[]{Integer.valueOf(i3)});
        }
    }
    ...
    private void handleAddNetworkSubmitEvent(Intent intent) {
        ...
        if (!MotoDevicePolicyUtils.applyAdminRestriction((WifiEntry) null, wifiConfiguration, getActivity())) {
            this.mWifiManager.save(wifiConfiguration, this.mSaveListener);
        } else if (Build.IS_DEBUGGABLE) {
            Log.d("NetworkProviderSettings", "-> Admin restrictions was applied ...");
        }
    }
    ...
    public int getHelpResource() {
        ...
        return C1992R$string.help_url_wifi;
    }
    ...
    public void onSubmit(WifiDialog2 wifiDialog2) {
        ...
        if (MotoDevicePolicyUtils.applyAdminRestriction(wifiEntry, (WifiConfiguration) null, getContext())) {
            if (Build.IS_DEBUGGABLE) {
                Log.d("NetworkProviderSettings", "-> Admin restrictions was applied can't connect to network...");
            }
            MotoDevicePolicyUtils.startAdminDialog(getContext(), "moto_enterprise_wifi_security_active");
        } else if (mode == 2) {
            if (config == null) {
                Toast.makeText(getContext(), C1992R$string.wifi_failed_save_message, 0).show();
            } else {
    ...
    public void connect(WifiEntry wifiEntry, boolean z, boolean z2) {
        ...
        if (MotoDevicePolicyUtils.applyAdminRestriction(wifiEntry, (WifiConfiguration) null, getContext())) {
            ...
            MotoDevicePolicyUtils.startAdminDialog(getContext(), "moto_enterprise_wifi_security_active");
        } else if (EapSimRequiredDialog.shouldShowSimRequiredDialog(getContext(), wifiEntry)) {
            EapSimRequiredDialog.show(getContext(), wifiEntry.getSsid());
        } else {
    ...
    private class WifiConnectActionListener implements WifiManager.ActionListener {
        ...
        public void onFailure(int i) {
            if (!NetworkProviderSettings.this.isFinishingOrDestroyed()) {
                Toast.makeText(NetworkProviderSettings.this.getContext(), C1992R$string.wifi_failed_connect_message, 0).show();
            }
        }
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater menuInflater) {
        if (!this.mAirplaneModeEnabler.isAirplaneModeOn()) {
            MenuItem add = menu.add(0, 6, 0, C1992R$string.fix_connectivity);
            add.setIcon(C1983R$drawable.ic_repair_24dp);
    ...
    public void showResetInternetDialog() {
        ...
        builder.setTitle(C1992R$string.reset_your_internet_title).setMessage(C1992R$string.reset_internet_text).setPositiveButton(C1992R$string.tts_reset, (DialogInterface.OnClickListener) new DialogInterface.OnClickListener() {
