...
public class MobilePlanPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, LifecycleObserver, OnCreate, OnSaveInstanceState {
    ...
    public boolean isAvailable() {
        boolean z = this.mContext.getResources().getBoolean(C1980R$bool.config_show_mobile_plan);
    ...
    private void onManageMobilePlanClick() {
        ...
        if (this.mTm.hasIccCard() && activeNetworkInfo != null) {
                ...
                if (!TextUtils.isEmpty(mobileProvisioningUrl)) {
                    ...
                    if (TextUtils.isEmpty(simOperatorName)) {
                        ...
                        if (TextUtils.isEmpty(networkOperatorName)) {
                            this.mMobilePlanDialogMessage = resources.getString(C1992R$string.mobile_unknown_sim_operator);
                        } else {
                            this.mMobilePlanDialogMessage = resources.getString(C1992R$string.mobile_no_provisioning_url, new Object[]{networkOperatorName});
                        }
                    } else {
                        this.mMobilePlanDialogMessage = resources.getString(C1992R$string.mobile_no_provisioning_url, new Object[]{simOperatorName});
                    }
                }
            } else {
            ...
            this.mMobilePlanDialogMessage = resources.getString(C1992R$string.mobile_insert_sim_card);
        } else {
            this.mMobilePlanDialogMessage = resources.getString(C1992R$string.mobile_connect_to_internet);
        }
        if (!TextUtils.isEmpty(this.mMobilePlanDialogMessage)) {
