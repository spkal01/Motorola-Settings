...
public class NetworkProviderCallsSmsController extends AbstractPreferenceController implements SubscriptionsChangeListener.SubscriptionsChangeListenerClient, LifecycleObserver {
    ...
    public CharSequence getSummary() {
        ...
        if (activeSubscriptions.isEmpty()) {
            return setSummaryResId(C1992R$string.calls_sms_no_sim);
        }
    ...
    public CharSequence getPreferredStatus(int i, int i2) {
        ...
        if (!SubscriptionManager.isValidSubscriptionId(i2) || !isInService(i2)) {
            if (i > 1) {
                i3 = C1992R$string.calls_sms_unavailable;
            } else {
                i3 = C1992R$string.calls_sms_temp_unavailable;
            }
            return setSummaryResId(i3);
        } else if (z2 && z) {
            return setSummaryResId(C1992R$string.calls_sms_preferred);
        } else {
            if (z2) {
                return setSummaryResId(C1992R$string.calls_sms_calls_preferred);
            }
            return z ? setSummaryResId(C1992R$string.calls_sms_sms_preferred) : "";
        }
    }
