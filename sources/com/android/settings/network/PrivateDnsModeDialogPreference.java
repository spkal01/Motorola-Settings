...
public class PrivateDnsModeDialogPreference extends CustomDialogPreferenceCompat implements DialogInterface.OnClickListener, RadioGroup.OnCheckedChangeListener, TextWatcher {
    ...
    static {
        ...
        hashMap.put(1, Integer.valueOf(C1985R$id.private_dns_mode_off));
        hashMap.put(2, Integer.valueOf(C1985R$id.private_dns_mode_opportunistic));
        hashMap.put(3, Integer.valueOf(C1985R$id.private_dns_mode_provider));
    }
    ...
    public static /* synthetic */ void lambda$new$0(View view) {
        ...
        Intent helpIntent = HelpUtils.getHelpIntent(context, context.getString(C1992R$string.help_uri_private_dns), context.getClass().getName());
        if (helpIntent != null) {
            try {
    ...
    private void initialize() {
        setWidgetLayoutResource(C1987R$layout.restricted_icon);
    }
    ...
    public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
        ...
        View findViewById = preferenceViewHolder.findViewById(C1985R$id.restricted_icon);
    ...
    public void onBindDialogView(View view) {
        ...
        EditText editText = (EditText) view.findViewById(C1985R$id.private_dns_mode_provider_hostname);
        this.mEditText = editText;
        editText.addTextChangedListener(this);
        this.mEditText.setText(getHostnameFromSettings(contentResolver));
        RadioGroup radioGroup = (RadioGroup) view.findViewById(C1985R$id.private_dns_radio_group);
        ...
        int i = C1985R$id.private_dns_mode_opportunistic;
        radioGroup2.check(map.getOrDefault(valueOf, Integer.valueOf(i)).intValue());
        ((RadioButton) view.findViewById(C1985R$id.private_dns_mode_off)).setText(C1992R$string.private_dns_mode_off);
        ((RadioButton) view.findViewById(i)).setText(C1992R$string.private_dns_mode_opportunistic);
        ((RadioButton) view.findViewById(C1985R$id.private_dns_mode_provider)).setText(C1992R$string.private_dns_mode_provider);
        TextView textView = (TextView) view.findViewById(C1985R$id.private_dns_help_info);
        textView.setMovementMethod(LinkMovementMethod.getInstance());
        AnnotationSpan.LinkInfo linkInfo = new AnnotationSpan.LinkInfo(context, "url", HelpUtils.getHelpIntent(context, context.getString(C1992R$string.help_uri_private_dns), context.getClass().getName()));
        if (linkInfo.isActionable()) {
            textView.setText(AnnotationSpan.linkify(context.getText(C1992R$string.private_dns_help_message), linkInfo));
        }
    }
    ...
    public void onCheckedChanged(RadioGroup radioGroup, int i) {
        if (i == C1985R$id.private_dns_mode_off) {
            this.mMode = 1;
        } else if (i == C1985R$id.private_dns_mode_opportunistic) {
            this.mMode = 2;
        } else if (i == C1985R$id.private_dns_mode_provider) {
            this.mMode = 3;
        }
        updateDialogInfo();
    }
