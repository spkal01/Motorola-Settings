...
public class NrDisabledInDsdsFooterPreferenceController extends BasePreferenceController {
    ...
    private CharSequence getFooterText() {
        ...
        AnnotationSpan.LinkInfo linkInfo = new AnnotationSpan.LinkInfo(this.mContext, "url", HelpUtils.getHelpIntent(context, context.getString(C1992R$string.help_uri_5g_dsds), this.mContext.getClass().getName()));
        if (!linkInfo.isActionable()) {
            return AnnotationSpan.textWithoutLink(this.mContext.getText(C1992R$string.no_5g_in_dsds_text));
        }
        return AnnotationSpan.linkify(this.mContext.getText(C1992R$string.no_5g_in_dsds_text), linkInfo);
    }
    ...
    public int getAvailabilityStatus() {
        ...
        int[] activeSubIds = MotoMnsCache.getIns(this.mContext).getActiveSubIds();
        if (activeSubIds == null) {
            i = 0;
        } else {
            i = activeSubIds.length;
        }
