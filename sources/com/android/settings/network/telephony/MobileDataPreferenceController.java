...
public class MobileDataPreferenceController extends TelephonyTogglePreferenceController implements LifecycleObserver, OnStart, OnStop {
    ...
    public void updateState(Preference preference) {
        ...
        if (isOpportunistic()) {
            ...
            preference.setSummary(C1992R$string.mobile_data_settings_summary_auto_switch);
        } else {
            preference.setEnabled(true);
            preference.setSummary(C1992R$string.mobile_data_settings_summary);
        }
        if (this.mSubId == -1) {
            preference.setSelectable(false);
            preference.setSummary(C1992R$string.mobile_data_settings_summary_unavailable);
            return;
        }
    ...
    private boolean isOpportunistic() {
        SubscriptionInfo activeSubInfo = MotoMnsCache.getIns(this.mContext).getActiveSubInfo(this.mSubId);
        return activeSubInfo != null && activeSubInfo.isOpportunistic();
    }
