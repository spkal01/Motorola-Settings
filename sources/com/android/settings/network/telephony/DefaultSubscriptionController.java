...
public abstract class DefaultSubscriptionController extends TelephonyBasePreferenceController implements LifecycleObserver, Preference.OnPreferenceChangeListener, SubscriptionsChangeListener.SubscriptionsChangeListenerClient {
    ...
    public int getAvailabilityStatus(int i) {
        if (MotoTelephonyFeature.isFtr6379Enabled(this.mContext, i)) {
            return 2;
        }
        if (SubscriptionUtil.getActiveSubscriptions(this.mManager).size() > 1 || Utils.isProviderModelEnabled(this.mContext)) {
            return 0;
        }
        return 2;
    }
    ...
    public CharSequence getSummary() {
        ...
        return isAskEverytimeSupported() ? this.mContext.getString(C1992R$string.calls_and_sms_ask_every_time) : "";
    }
        ...
        if (this.mPreference != null) {
            ...
            if (!Utils.isProviderModelEnabled(this.mContext) || activeSubscriptions.size() != 1) {
                ...
                if (isAskEverytimeSupported()) {
                    arrayList.add(this.mContext.getString(C1992R$string.calls_and_sms_ask_every_time));
