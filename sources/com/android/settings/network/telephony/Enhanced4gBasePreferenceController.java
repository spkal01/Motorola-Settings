...
public class Enhanced4gBasePreferenceController extends TelephonyTogglePreferenceController implements LifecycleObserver, OnStart, OnStop {
    ...
    public Enhanced4gBasePreferenceController init(int i) {
        ...
        MotoMnsLog.logd(TAG, "subId:" + this.mSubId + ", show4GForLTE:" + z + ", m4gCurrentMode:" + this.m4gCurrentMode + ", mShow5gLimitedDialog:" + this.mShow5gLimitedDialog + ", mIsNrEnabledFromCarrierConfig:" + this.mIsNrEnabledFromCarrierConfig);
        return this;
    }
    ...
    public int getAvailabilityStatus(int i) {
        init(i);
        if (!isModeMatched()) {
            MotoMnsLog.logd(TAG, "isModeMatched: false");
            return 2;
        }
        ...
        PersistableBundle carrierConfigForSubId = getCarrierConfigForSubId(i);
        if (carrierConfigForSubId == null || carrierConfigForSubId.getBoolean("hide_enhanced_4g_lte_bool")) {
            MotoMnsLog.logd(TAG, "KEY_HIDE_ENHANCED_4G_LTE_BOOL: true");
            return 2;
        } else if (!queryImsState.isReadyToVoLte()) {
            MotoMnsLog.logd(TAG, "isReadyToVoLte: false");
            return 2;
        } else if (!MotoMobileNetworkUtils.isVolteSupportedByModem(this.mContext, i)) {
            MotoMnsLog.logd(TAG, "isVolteSupportedByModem: false");
            return 2;
        } else {
            boolean isUserControlAllowed = isUserControlAllowed(carrierConfigForSubId);
            boolean isAllowUserControl = queryImsState.isAllowUserControl();
            MotoMnsLog.logd(TAG, "isUserControlAllowed:" + isUserControlAllowed + ", isAllowUserControl:" + isAllowUserControl);
            if (!isUserControlAllowed || !isAllowUserControl) {
                return 1;
            }
            return 0;
        }
    ...
    public void updateState(Preference preference) {
        ...
        if (preference != null) {
            ((SwitchPreference) preference).setEnabled(isUserControlAllowed(getCarrierConfigForSubId(this.mSubId)) && queryImsState(this.mSubId).isAllowUserControl());
            updateTitleAndSummary(preference);
        }
    }
    ...
    private void show5gLimitedDialog(final ImsMmTelManager imsMmTelManager) {
        ...
        builder.setTitle(C1992R$string.volte_5G_limited_title).setMessage(C1992R$string.volte_5G_limited_text).setNeutralButton(this.mContext.getResources().getString(C1992R$string.cancel), (DialogInterface.OnClickListener) null).setPositiveButton(this.mContext.getResources().getString(C1992R$string.condition_turn_off), new DialogInterface.OnClickListener() {
