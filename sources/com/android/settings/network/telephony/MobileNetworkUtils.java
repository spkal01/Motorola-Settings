...
public class MobileNetworkUtils {
    ...
    public static boolean isContactDiscoveryVisible(Context context, int i) {
        if (((CarrierConfigManager) context.getSystemService(CarrierConfigManager.class)) == null) {
            Log.w("MobileNetworkUtils", "isContactDiscoveryVisible: Could not resolve carrier config");
            return false;
        }
        PersistableBundle carrierConfigForSubId = MotoMnsCache.getIns(context).getCarrierConfigForSubId(i);
        boolean z = carrierConfigForSubId.getBoolean("use_rcs_presence_bool", false) || carrierConfigForSubId.getBoolean("ims.rcs_bulk_capability_exchange_bool", false);
        if (z) {
            int i2 = carrierConfigForSubId.getInt("moto_default_disable_initial_addressbook_scan_value_int", 0);
            ContentResolver contentResolver = context.getContentResolver();
            if (Settings.System.getInt(contentResolver, "RCS.disableInitialAddressBookScan" + i, i2) == 1) {
                Log.i("MobileNetworkUtils", "RCS.disableInitialAddressBookScan is On!");
                return false;
            }
        }
        return z;
    ...
    public static boolean isCdmaOptions(Context context, int i) {
        ...
        if (carrierConfigForSubId != null && !carrierConfigForSubId.getBoolean("hide_carrier_network_settings_bool") && carrierConfigForSubId.getBoolean("world_phone_bool")) {
            return true;
        }
    ...
    public static boolean isWorldMode(Context context, int i) {
        ...
        return carrierConfigForSubId.getBoolean("world_mode_enabled_bool");
    }
    ...
    public static boolean shouldDisplayNetworkSelectOptions(Context context, int i) {
        ...
        TelephonyManager createForSubscriptionId = ((TelephonyManager) context.getSystemService(TelephonyManager.class)).createForSubscriptionId(i);
        PersistableBundle carrierConfigForSubId = MotoMnsCache.getIns(context).getCarrierConfigForSubId(i);
        if (i == -1 || carrierConfigForSubId == null || !carrierConfigForSubId.getBoolean("operator_selection_expand_bool") || carrierConfigForSubId.getBoolean("hide_carrier_network_settings_bool") || ((carrierConfigForSubId.getBoolean("csp_enabled_bool") && !createForSubscriptionId.isManualNetworkSelectionAllowed()) || (((networkTypeFromRaf = getNetworkTypeFromRaf((int) createForSubscriptionId.getAllowedNetworkTypesForReason(0))) == 8 && isWorldMode(context, i)) || shouldSpeciallyUpdateGsmCdma(context, i)))) {
            return false;
        }
        if (isGsmBasicOptions(context, i)) {
            return true;
    ...
    public static int getAvailability(Context context, int i, TelephonyAvailabilityCallback telephonyAvailabilityCallback) {
        ...
        for (int availabilityStatus : activeSubIds) {
        ...
        return telephonyAvailabilityCallback.getAvailabilityStatus(activeSubIds[0]);
    }
    ...
    public static Drawable getSignalStrengthIcon(Context context, int i, int i2, int i3, boolean z) {
        ...
        int dimensionPixelSize = context.getResources().getDimensionPixelSize(C1982R$dimen.signal_strength_icon_size);
    ...
    public static CharSequence getCurrentCarrierNameForDisplay(Context context, int i) {
        SubscriptionInfo activeSubInfo;
        if (((SubscriptionManager) context.getSystemService(SubscriptionManager.class)) == null || (activeSubInfo = MotoMnsCache.getIns(context).getActiveSubInfo(i)) == null) {
            return getOperatorNameFromTelephonyManager(context);
        }
        return activeSubInfo.getCarrierName();
    }

    public static CharSequence getCurrentCarrierNameForDisplay(Context context) {
        SubscriptionInfo activeSubInfo;
        if (((SubscriptionManager) context.getSystemService(SubscriptionManager.class)) == null || (activeSubInfo = MotoMnsCache.getIns(context).getActiveSubInfo(SubscriptionManager.getDefaultSubscriptionId())) == null) {
            return getOperatorNameFromTelephonyManager(context);
        }
        return activeSubInfo.getCarrierName();
    }
    ...
    public static boolean isWifiCallingEnabled(Context context, int i, WifiCallingQueryImsState wifiCallingQueryImsState, PhoneAccountHandle phoneAccountHandle) {
        ...
        if (phoneAccountHandle != null) {
            Intent buildPhoneAccountConfigureIntent = buildPhoneAccountConfigureIntent(context, phoneAccountHandle);
            z = buildPhoneAccountConfigureIntent != null;
            MotoMnsLog.logd("MobileNetworkUtils", "isWifiCallingEnabled intent:" + buildPhoneAccountConfigureIntent);
        } else {
            if (wifiCallingQueryImsState == null) {
                wifiCallingQueryImsState = new WifiCallingQueryImsState(context, i);
            }
            z = wifiCallingQueryImsState.isReadyToWifiCalling();
        }
        MotoMnsLog.logd("MobileNetworkUtils", "isWifiCallingEnabled: " + z);
        return z;
    }
    ...
    private static CharSequence getPreferredCallStatus(Context context, SubscriptionInfo subscriptionInfo) {
        return subscriptionInfo.getSubscriptionId() == SubscriptionManager.getDefaultVoiceSubscriptionId() ? setSummaryResId(context, C1992R$string.calls_sms_preferred) : "";
    }

    private static CharSequence getPreferredSmsStatus(Context context, SubscriptionInfo subscriptionInfo) {
        return subscriptionInfo.getSubscriptionId() == SubscriptionManager.getDefaultSmsSubscriptionId() ? setSummaryResId(context, C1992R$string.calls_sms_preferred) : "";
    }
