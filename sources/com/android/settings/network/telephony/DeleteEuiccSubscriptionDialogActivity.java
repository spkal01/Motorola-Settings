...
public class DeleteEuiccSubscriptionDialogActivity extends SubscriptionActionDialogActivity implements SidecarFragment.Listener, ConfirmDialogFragment.OnConfirmListener {
    ...
    public void onConfirm(int i, boolean z) {
        if (!z) {
            ...
            showProgressDialog(getString(C1992R$string.erasing_sim));
    ...
    private void handleDeleteEuiccSubscriptionSidecarStateChange() {
        ...
        if (state == 2) {
            ...
            this.mDeleteEuiccSubscriptionSidecar.reset();
            showErrorDialog(getString(C1992R$string.erase_sim_fail_title), getString(C1992R$string.erase_sim_fail_text));
        }
    }

    private void showDeleteSimConfirmDialog() {
        ConfirmDialogFragment.show(this, ConfirmDialogFragment.OnConfirmListener.class, 1, getString(C1992R$string.erase_sim_dialog_title), getString(C1992R$string.erase_sim_dialog_text, new Object[]{SubscriptionUtil.getUniqueSubscriptionDisplayName(this.mSubscriptionToBeDeleted, (Context) this)}), getString(C1992R$string.erase_sim_confirm_button), getString(C1992R$string.cancel));
    }
}
