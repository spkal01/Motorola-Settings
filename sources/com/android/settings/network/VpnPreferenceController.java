...
public class VpnPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, LifecycleObserver, OnResume, OnPause {
    ...
    public boolean isAvailable() {
        if (RestrictedLockUtilsInternal.hasBaseUserRestriction(this.mContext, "no_moto_config_vpn", UserHandle.myUserId())) {
            return false;
        }
    ...
    public void updateSummary() {
        ...
        if (this.mPreference != null) {
            ...
            if (vpnConfig2 == null) {
                str = this.mContext.getString(C1992R$string.vpn_disconnected_summary);
            } else {
            ...
            if (Utils.isProviderModelEnabled(this.mContext) && (this.mPreference instanceof VpnInfoPreference)) {
                ...
                if (z) {
                    if ((sparseArray.size() + LegacyVpnProfileStore.list("VPN_").length) - i2 == 1) {
                        str = this.mContext.getString(C1992R$string.vpn_settings_insecure_single);
                    } else if (insecureVpnCount == 1) {
                        str = this.mContext.getString(C1992R$string.vpn_settings_single_insecure_multiple_total, new Object[]{Integer.valueOf(insecureVpnCount)});
                    } else {
                        str = this.mContext.getString(C1992R$string.vpn_settings_multiple_insecure_multiple_total, new Object[]{Integer.valueOf(insecureVpnCount)});
                    }
                }
            }
    ...
    public String getNameForVpnConfig(VpnConfig vpnConfig, UserHandle userHandle) {
        if (vpnConfig.legacy) {
            return this.mContext.getString(C1992R$string.wifi_display_status_connected);
        }
