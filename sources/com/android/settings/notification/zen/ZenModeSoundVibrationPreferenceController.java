...
public class ZenModeSoundVibrationPreferenceController extends AbstractZenModePreferenceController {
    ...
    public void updateState(Preference preference) {
        ...
        if (zenMode == 2) {
            ...
            preference.setSummary((CharSequence) this.mContext.getString(C1992R$string.zen_mode_sounds_none));
        } else if (zenMode != 3) {
            preference.setEnabled(true);
            preference.setSummary((CharSequence) this.mSummaryBuilder.getOtherSoundCategoriesSummary(getPolicy()));
        } else {
            preference.setEnabled(false);
            preference.setSummary((CharSequence) this.mContext.getString(C1992R$string.zen_mode_behavior_alarms_only));
        }
    }
}
