...
public class ZenAccessSettings extends EmptyTextSettings implements ZenAccessSettingObserverMixin.Listener {
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER = new BaseSearchIndexProvider(C1994R$xml.zen_access_settings);
    ...
    public void onViewCreated(View view, Bundle bundle) {
        ...
        setEmptyText(C1992R$string.zen_access_empty_text);
    }

    /* access modifiers changed from: protected */
    public int getPreferenceScreenResId() {
        return C1994R$xml.zen_access_settings;
    }
    ...
    private void reloadList() {
        ...
        if (!packagesRequestingNotificationPolicyAccess.isEmpty() && (installedApplications = this.mPkgMan.getInstalledApplications(0)) != null) {
            for (ApplicationInfo next : installedApplications) {
                if (!CloneAppUtils.isCloneAppByUid(next.uid) && packagesRequestingNotificationPolicyAccess.contains(next.packageName)) {
                    arrayList.add(next);
                }
            }
        }
        ...
        while (it.hasNext()) {
            ...
            if (arraySet.contains(str)) {
                ...
                appPreference.setSummary((CharSequence) getString(C1992R$string.zen_access_disabled_package_warning));
            } else {
    ...
    public /* synthetic */ boolean lambda$reloadList$0(String str, ApplicationInfo applicationInfo, Preference preference) {
        AppInfoBase.startAppInfoFragment(ZenAccessDetails.class, C1992R$string.manage_zen_access_title, str, applicationInfo.uid, this, -1, getMetricsCategory());
        return true;
    }

    private int getPreferenceSummary(String str) {
        if (ZenAccessController.hasAccess(getContext(), str)) {
            return C1992R$string.app_permission_summary_allowed;
        }
        return C1992R$string.app_permission_summary_not_allowed;
    }
}
