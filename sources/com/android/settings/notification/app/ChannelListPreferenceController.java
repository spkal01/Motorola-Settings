...
public class ChannelListPreferenceController extends NotificationPreferenceController {
    ...
    public void updateFullList(PreferenceCategory preferenceCategory, List<NotificationChannelGroup> list) {
        if (!list.isEmpty()) {
            ...
            preferenceCategory2.setTitle(C1992R$string.notification_channels);
            preferenceCategory2.setKey("zeroCategories");
            preferenceCategory.addPreference(preferenceCategory2);
            Preference preference = new Preference(this.mContext);
            preference.setTitle(C1992R$string.no_channels);
            preference.setEnabled(false);
            preferenceCategory2.addPreference(preference);
        } else {
            PreferenceGroup preferenceGroup = (PreferenceGroup) preferenceCategory.getPreference(0);
            preferenceGroup.setTitle(C1992R$string.notification_channels);
            preferenceGroup.getPreference(0).setTitle(C1992R$string.no_channels);
        }
    }
    ...
    private void updateGroupPreferences(NotificationChannelGroup notificationChannelGroup, PreferenceGroup preferenceGroup) {
        ...
        if (notificationChannelGroup.getId() == null) {
            preferenceGroup.setTitle(C1992R$string.notification_channels_other);
        } else {
    ...
    private Preference addOrUpdateGroupToggle(PreferenceGroup preferenceGroup, NotificationChannelGroup notificationChannelGroup) {
        ...
        restrictedSwitchPreference.setTitle((CharSequence) this.mContext.getString(C1992R$string.notification_switch_label, new Object[]{notificationChannelGroup.getName()}));
    ...
    private void updateSingleChannelPrefs(PrimarySwitchPreference primarySwitchPreference, NotificationChannel notificationChannel, boolean z) {
        ...
        if (notificationChannel.getImportance() > 2) {
            ...
            primarySwitchPreference.setIcon(C1983R$drawable.empty_icon);
        }
        ...
        primarySwitchPreference.setIntent(new SubSettingLauncher(this.mContext).setDestination(ChannelNotificationSettings.class.getName()).setArguments(bundle).setTitleRes(C1992R$string.notification_channel_title).setSourceMetricsCategory(72).toIntent());
    ...
    private Drawable getAlertingIcon() {
        Drawable drawable = this.mContext.getDrawable(C1983R$drawable.ic_notifications_alert);
