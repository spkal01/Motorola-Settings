...
public abstract class NotificationSettings extends DashboardFragment {
    ...
    public void animatePanel() {
        if (this.mPreferenceFilter != null) {
            ViewGroup viewGroup = (ViewGroup) getActivity().findViewById(C1985R$id.main_content);
    ...
    public void toastAndFinish() {
        Toast.makeText(this.mContext, C1992R$string.app_not_found_dlg_text, 0).show();
        getActivity().finish();
    }
                ...
                if (str.equals(packagesForUid[i2])) {
                    try {
                        if (CloneAppUtils.isCloneAppByUid(i)) {
                            return this.mPm.getPackageInfoAsUser(str, 64, UserHandle.getUserId(i));
                        }
