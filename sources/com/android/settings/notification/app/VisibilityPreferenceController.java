...
public class VisibilityPreferenceController extends NotificationPreferenceController implements PreferenceControllerMixin, Preference.OnPreferenceChangeListener {
    ...
    public boolean isAvailable() {
        if (super.isAvailable() && this.mChannel != null && !this.mAppRow.banned && !InstalledAppUtils.get(this.mContext).isPkgHideSensitiveContentItem(this.mAppRow.pkg) && checkCanBeVisible(2) && isLockScreenSecure()) {
            return true;
        }
        return false;
    }
    ...
    public void updateState(Preference preference) {
        if (this.mChannel != null && this.mAppRow != null) {
            ...
            if (getLockscreenNotificationsEnabled() && getLockscreenAllowPrivateNotifications()) {
                String string = this.mContext.getString(C1992R$string.lock_screen_notifications_summary_show);
            ...
            if (getLockscreenNotificationsEnabled()) {
                String string2 = this.mContext.getString(C1992R$string.lock_screen_notifications_summary_hide);
                String num2 = Integer.toString(0);
                arrayList.add(string2);
                arrayList2.add(num2);
                setRestrictedIfNotificationFeaturesDisabled(restrictedListPreference, string2, num2, 4);
            }
            arrayList.add(this.mContext.getString(C1992R$string.lock_screen_notifications_summary_disable));
