...
public class AppBubbleListPreferenceController extends AppConversationListPreferenceController {
    ...
    public int getTitleResId() {
        if (this.mAppRow.bubblePreference == 2) {
            return C1992R$string.bubble_app_setting_selected_conversation_title;
        }
        return C1992R$string.bubble_app_setting_excluded_conversation_title;
    }
    ...
    public static class ConversationPreference extends Preference implements View.OnClickListener {
        ...
        ConversationPreference(Context context) {
            super(context);
            setWidgetLayoutResource(C1987R$layout.bubble_conversation_remove_button);
        }

        public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
            String str;
            super.onBindViewHolder(preferenceViewHolder);
            ImageView imageView = (ImageView) preferenceViewHolder.itemView.findViewById(C1985R$id.button);
            if (this.mOnClickBubbles) {
                str = getContext().getString(C1992R$string.bubble_app_setting_bubble_conversation);
            } else {
                str = getContext().getString(C1992R$string.bubble_app_setting_unbubble_conversation);
            }
