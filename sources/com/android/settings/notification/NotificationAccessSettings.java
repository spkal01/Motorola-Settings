...
public class NotificationAccessSettings extends EmptyTextSettings {
    private static final ManagedServiceSettings.Config CONFIG = new ManagedServiceSettings.Config.Builder().setTag("NotifAccessSettings").setSetting("enabled_notification_listeners").setIntentAction("android.service.notification.NotificationListenerService").setPermission("android.permission.BIND_NOTIFICATION_LISTENER_SERVICE").setNoun("notification listener").setWarningDialogTitle(C1992R$string.notification_listener_security_warning_title).setWarningDialogSummary(C1992R$string.notification_listener_security_warning_summary).setEmptyText(C1992R$string.no_notification_listeners).build();
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER = new BaseSearchIndexProvider(C1994R$xml.notification_access_settings);
    ...
    public void onCreate(Bundle bundle) {
        ...
        if (UserManager.get(this.mContext).isManagedProfile()) {
            Toast.makeText(this.mContext, C1992R$string.notification_settings_work_profile, 0).show();
            finish();
        }
    }
    ...
    public void updateList(List<ServiceInfo> list) {
        ...
        for (ServiceInfo next : list) {
            ...
            try {
                ...
                if (!Build.IS_PRC_PRODUCT || !UserHandle.isAppCloneUser(userId)) {
                    charSequence = this.mPm.getApplicationInfoAsUser(next.packageName, 0, UserHandle.myUserId()).loadLabel(this.mPm);
                } else {
                    charSequence = this.mPm.getApplicationInfoAsUser(next.packageName, 0, userId).loadLabel(this.mPm);
                }
            ...
            if (managedProfileId != -10000 && !this.mDpm.isNotificationListenerServicePermitted(next.packageName, managedProfileId)) {
                appPreference.setSummary(C1992R$string.work_profile_notification_access_blocked_summary);
            }
    ...
    public /* synthetic */ boolean lambda$updateList$0(ComponentName componentName, ServiceInfo serviceInfo, Preference preference) {
        ...
        new SubSettingLauncher(getContext()).setDestination(NotificationAccessDetails.class.getName()).setSourceMetricsCategory(getMetricsCategory()).setTitleRes(C1992R$string.manage_notification_access_title).setArguments(bundle).setExtras(bundle2).setUserHandle(UserHandle.getUserHandleForUid(serviceInfo.applicationInfo.uid)).launch();
        return true;
    }
    ...
    public int getPreferenceScreenResId() {
        return C1994R$xml.notification_access_settings;
    }
}
