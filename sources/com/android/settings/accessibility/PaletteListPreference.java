...
public final class PaletteListPreference extends Preference {
    ...
    public PaletteListPreference(Context context, AttributeSet attributeSet, int i) {
        ...
        setLayoutResource(C1987R$layout.daltonizer_preview);
    }

    public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
        super.onBindViewHolder(preferenceViewHolder);
        initPaletteAttributes(getContext());
        initPaletteView(getContext(), (ViewGroup) preferenceViewHolder.itemView.findViewById(C1985R$id.palette_view));
    }

    private void initPaletteAttributes(Context context) {
        int color = context.getColor(C1981R$color.palette_list_gradient_background);
    ...
    private void initPaletteView(Context context, ViewGroup viewGroup) {
        ...
        float dimension = context.getResources().getDimension(C1982R$dimen.accessibility_layout_margin_start_end);
    ...
    private List<Integer> getPaletteColors(Context context) {
        return (List) Arrays.stream(context.getResources().getIntArray(C1978R$array.setting_palette_colors)).boxed().collect(Collectors.toList());
    }

    private List<String> getPaletteData(Context context) {
        return Arrays.asList(context.getResources().getStringArray(C1978R$array.setting_palette_data));
    }
