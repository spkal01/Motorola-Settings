...
public abstract class ToggleFeaturePreferenceFragment extends SettingsPreferenceFragment implements ShortcutPreference.OnClickCallback, OnMainSwitchChangeListener {
    ...
    public Dialog onCreateDialog(int i) {
        if (i == 1) {
            String string = getPrefContext().getString(C1992R$string.accessibility_shortcut_title, new Object[]{this.mPackageName});
    ...
    public void updateToggleServiceTitle(SettingsMainSwitchPreference settingsMainSwitchPreference) {
        settingsMainSwitchPreference.setTitle(getString(C1992R$string.accessibility_service_primary_switch_title, this.mPackageName));
    }

    /* access modifiers changed from: protected */
    public void updateShortcutTitle(ShortcutPreference shortcutPreference) {
        shortcutPreference.setTitle((CharSequence) getString(C1992R$string.accessibility_shortcut_title, this.mPackageName));
    }
    ...
    private void initGeneralCategory() {
        ...
        preferenceCategory.setTitle(C1992R$string.accessibility_screen_option);
    ...
    private void initHtmlTextPreference() {
        if (!TextUtils.isEmpty(this.mHtmlDescription)) {
            ...
            String string = getString(C1992R$string.accessibility_introduction_title, this.mPackageName);
    ...
    private void initFooterPreference() {
        if (!TextUtils.isEmpty(this.mDescription)) {
            createFooterPreference(getPreferenceScreen(), this.mDescription, getString(C1992R$string.accessibility_introduction_title, this.mPackageName));
        }
        if (TextUtils.isEmpty(this.mHtmlDescription) && TextUtils.isEmpty(this.mDescription)) {
            CharSequence text = getText(C1992R$string.accessibility_service_default_description);
            createFooterPreference(getPreferenceScreen(), text, getString(C1992R$string.accessibility_introduction_title, this.mPackageName));
        }
    }
    ...
    private void setDialogTextAreaClickListener(View view, CheckBox checkBox) {
        view.findViewById(C1985R$id.container).setOnClickListener(new ToggleFeaturePreferenceFragment$$ExternalSyntheticLambda2(checkBox));
    }
