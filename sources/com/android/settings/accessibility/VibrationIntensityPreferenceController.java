...
public abstract class VibrationIntensityPreferenceController extends BasePreferenceController implements LifecycleObserver, OnStart, OnStop {
    ...
    public static CharSequence getIntensityString(Context context, int i) {
        if (context.getResources().getBoolean(C1980R$bool.config_vibration_supports_multiple_intensities)) {
            if (i == 0) {
                return context.getString(C1992R$string.accessibility_vibration_intensity_off);
            }
            if (i == 1) {
                return context.getString(C1992R$string.accessibility_vibration_intensity_low);
            }
            if (i != 2) {
                return i != 3 ? "" : context.getString(C1992R$string.accessibility_vibration_intensity_high);
            }
            return context.getString(C1992R$string.accessibility_vibration_intensity_medium);
        } else if (i == 0) {
            return context.getString(C1992R$string.switch_off_text);
        } else {
            return context.getString(C1992R$string.switch_on_text);
        }
    }
