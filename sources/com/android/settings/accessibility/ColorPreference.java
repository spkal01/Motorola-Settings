...
public class ColorPreference extends ListDialogPreference {
    ...
    public ColorPreference(Context context, AttributeSet attributeSet) {
        ...
        setListItemLayoutResource(C1987R$layout.color_picker_item);
    }
    ...
    public CharSequence getTitleAt(int i) {
        ...
        return getContext().getString(C1992R$string.color_custom, new Object[]{Integer.valueOf(red), Integer.valueOf(green), Integer.valueOf(blue)});
    }

    public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
        super.onBindViewHolder(preferenceViewHolder);
        if (this.mPreviewEnabled) {
            ImageView imageView = (ImageView) preferenceViewHolder.findViewById(C1985R$id.color_preview);
            int value = getValue();
            if (Color.alpha(value) < 255) {
                imageView.setBackgroundResource(C1983R$drawable.transparency_tileable);
            } else {
    ...
    public void onBindListItem(View view, int i) {
        ...
        ImageView imageView = (ImageView) view.findViewById(C1985R$id.color_swatch);
        if (alpha < 255) {
            imageView.setBackgroundResource(C1983R$drawable.transparency_tileable);
        } else {
            imageView.setBackground((Drawable) null);
