...
public class EmergencyInfoPreferenceController extends BasePreferenceController {
    ...
    public void updateRawDataToIndex(List<SearchIndexableRaw> list) {
        if (isAvailable()) {
            ...
            int i = C1992R$string.emergency_info_title;
    ...
    public void updateState(Preference preference) {
        ...
        preference.setSummary((CharSequence) this.mContext.getString(C1992R$string.emergency_info_summary, new Object[]{userInfo.name}));
    }
    ...
    public int getAvailabilityStatus() {
        if (!this.mContext.getResources().getBoolean(C1980R$bool.config_show_emergency_info_in_device_info)) {
            return 3;
        }
    ...
    private boolean isEmergencyInfoSupported() {
        this.mIntent = new Intent(this.mContext.getResources().getString(C1992R$string.config_emergency_intent_action)).setPackage(this.mContext.getResources().getString(C1992R$string.config_emergency_package_name));
    ...
    private boolean isAOSPVersionSupported() {
        this.mIntent = new Intent(this.mContext.getResources().getString(C1992R$string.config_aosp_emergency_intent_action)).setPackage(this.mContext.getResources().getString(C1992R$string.config_aosp_emergency_package_name));
        List<ResolveInfo> queryIntentActivities = this.mContext.getPackageManager().queryIntentActivities(this.mIntent, 0);
        if (queryIntentActivities == null || queryIntentActivities.isEmpty()) {
            return false;
        }
