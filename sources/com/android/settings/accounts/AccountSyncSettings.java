...
public class AccountSyncSettings extends AccountPreferenceBase {
    ...
    public Dialog onCreateDialog(int i) {
        if (i == 102) {
            return new AlertDialog.Builder(getActivity()).setTitle(C1992R$string.cant_sync_dialog_title).setMessage(C1992R$string.cant_sync_dialog_message).setPositiveButton(17039370, (DialogInterface.OnClickListener) null).create();
        }
        return null;
    }
    ...
    public void onCreate(Bundle bundle) {
        ...
        addPreferencesFromResource(C1994R$xml.account_sync_settings);
    ...
    public void onActivityCreated(Bundle bundle) {
        ...
        this.mDisplayName = MotoAccountsHelper.getAccountDisplayName(activity, this.mAccount);
        LayoutPreference done = EntityHeaderController.newInstance(activity, this, (View) null).setRecyclerView(getListView(), getSettingsLifecycle()).setIcon(getDrawableForType(this.mAccount.type)).setLabel((CharSequence) this.mDisplayName).setSummary(getLabelForType(this.mAccount.type)).done((Activity) activity, getPrefContext());
        done.setOrder(0);
    ...
    private void setAccessibilityTitle() {
        ...
        if (isManagedProfile) {
            i = C1992R$string.accessibility_work_account_title;
        } else {
            i = C1992R$string.accessibility_personal_account_title;
        }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater menuInflater) {
        MenuItem add = menu.add(0, 1, 0, getString(C1992R$string.sync_menu_sync_now));
        MenuItem add2 = menu.add(0, 2, 0, getString(C1992R$string.sync_menu_sync_cancel));
    ...
    private void setFeedsState() {
        ...
        while (i3 < preferenceCount) {
            ...
            if (!(preference instanceof SyncStateSwitchPreference)) {
                ...
                if (!syncAutomaticallyAsUser) {
                    syncStateSwitchPreference.setSummary(C1992R$string.sync_disabled);
                } else if (isSyncing) {
                    syncStateSwitchPreference.setSummary(C1992R$string.sync_in_progress);
                } else {
                    if (j != 0) {
                        date.setTime(j);
                        i2 = i4;
                        z3 = false;
                        syncStateSwitchPreference.setSummary((CharSequence) getResources().getString(C1992R$string.last_synced, new Object[]{formatSyncDate(getContext(), date)}));
                    } else {
                        i2 = i4;
                        z3 = false;
        ...
        if (z4) {
            getPreferenceScreen().addPreference(new FooterPreference.Builder(getActivity()).setTitle(C1992R$string.sync_is_failing).build());
        }
    }
    ...
    public int getHelpResource() {
        return C1992R$string.help_url_accounts;
    }
