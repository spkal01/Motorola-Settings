...
public class AccountHeaderPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, LifecycleObserver, OnResume {
    ...
    public void onResume() {
        EntityHeaderController.newInstance(this.mActivity, this.mHost, this.mHeaderPreference.findViewById(C1985R$id.entity_header)).setLabel((CharSequence) this.mAccount.name).setIcon(new AuthenticatorHelper(this.mContext, this.mUserHandle, (AuthenticatorHelper.OnAccountsUpdateListener) null).getDrawableForType(this.mContext, this.mAccount.type)).done(this.mActivity, true);
    }
}
