...
public class AccountPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, AuthenticatorHelper.OnAccountsUpdateListener, Preference.OnPreferenceClickListener, LifecycleObserver, OnPause, OnResume {
    ...
    public void updateDynamicRawDataToIndex(List<SearchIndexableRaw> list) {
        if (isAvailable()) {
            ...
            String string = resources.getString(C1992R$string.account_settings_title);
            for (UserInfo userInfo : this.mUm.getProfiles(UserHandle.myUserId())) {
                if (userInfo.isEnabled() && userInfo.isManagedProfile()) {
                    if (!this.mHelper.hasBaseUserRestriction("no_remove_managed_profile", UserHandle.myUserId())) {
                        ...
                        searchIndexableRaw.title = resources.getString(C1992R$string.remove_managed_profile_label);
                    ...
                    searchIndexableRaw2.title = resources.getString(C1992R$string.managed_profile_settings_title);
    ...
    public boolean onPreferenceClick(Preference preference) {
        ...
        while (i < size) {
            ...
            if (preference == valueAt.addAccountPreference) {
                ...
                new SubSettingLauncher(this.mContext).setSourceMetricsCategory(metricsCategory).setDestination(ManagedProfileSettings.class.getName()).setTitleRes(C1992R$string.managed_profile_settings_title).setArguments(bundle).launch();
                return true;
            } else {
                i++;
            }
    ...
    private void updateProfileUi(UserInfo userInfo) {
        if (this.mFragment.getPreferenceManager() != null) {
            if (!Build.IS_PRC_PRODUCT || !userInfo.isCloneProfile()) {
                ...
                createAccessiblePreferenceCategory.setTitle(C1992R$string.account_settings);
                if (isSingleProfile()) {
                    String string = context.getString(C1992R$string.account_for_section_header, new Object[]{BidiFormatter.getInstance().unicodeWrap(userInfo.name)});
                    ...
                    if (this.mType == 3) {
                        createAccessiblePreferenceCategory.setTitle(C1992R$string.category_work);
                        ...
                        createAccessiblePreferenceCategory.setContentDescription(this.mContext.getString(C1992R$string.accessibility_category_work, new Object[]{workGroupSummary}));
                    }
                    ...
                    createAccessiblePreferenceCategory.setTitle(C1992R$string.category_personal);
                    createAccessiblePreferenceCategory.setContentDescription(this.mContext.getString(C1992R$string.accessibility_category_personal));
                }
    ...
    private RestrictedPreference newAddAccountPreference() {
        ...
        restrictedPreference.setTitle(C1992R$string.add_account_label);
        restrictedPreference.setIcon(C1983R$drawable.ic_add_24dp);
    ...
    private RestrictedPreference newRemoveWorkProfilePreference() {
        ...
        restrictedPreference.setTitle(C1992R$string.remove_managed_profile_label);
        restrictedPreference.setIcon(C1983R$drawable.ic_delete);
        restrictedPreference.setOnPreferenceClickListener(this);
    ...
    private Preference newManagedProfileSettings() {
        ...
        preference.setTitle(C1992R$string.managed_profile_settings_title);
        preference.setIcon(C1983R$drawable.ic_settings_24dp);
    ...
    private String getWorkGroupSummary(Context context, UserInfo userInfo) {
        ...
        return this.mContext.getString(C1992R$string.managing_admin, new Object[]{applicationLabel});
    }
    ...
    private void updateAccountTypes(ProfileData profileData) {
        if (this.mFragment.getPreferenceManager() != null && profileData.preferenceGroup.getPreferenceManager() != null) {
            if (profileData.userInfo.isEnabled()) {
                ...
                this.mProfileNotAvailablePreference.setIcon(C1983R$drawable.empty_icon);
                this.mProfileNotAvailablePreference.setTitle((CharSequence) null);
                this.mProfileNotAvailablePreference.setSummary(C1992R$string.managed_profile_not_available_label);
    ...
    private ArrayList<AccountTypePreference> getAccountTypePreferences(AuthenticatorHelper authenticatorHelper, UserHandle userHandle, ArrayMap<String, AccountTypePreference> arrayMap) {
        ...
        while (i4 < enabledAccountTypes.length) {
            ...
            if (!MotoAccountsHelper.isHidden(this.mContext, str) && accountTypeHasAnyRequestedAuthorities(authenticatorHelper2, str) && (labelForType = authenticatorHelper2.getLabelForType(this.mContext, str)) != null) {
    ...
    private boolean isSingleProfile() {
        if (Build.IS_PRC_PRODUCT) {
            List profiles = this.mUm.getProfiles(UserHandle.myUserId());
            List list = (List) profiles.stream().filter(AccountPreferenceController$$ExternalSyntheticLambda0.INSTANCE).collect(Collectors.toList());
            if (this.mUm.isLinkedUser() || profiles.size() == 1 || list.size() == 1) {
                return true;
            }
            return false;
        } else if (this.mUm.isLinkedUser() || this.mUm.getProfiles(UserHandle.myUserId()).size() == 1) {
            return true;
        } else {
            return false;
        }
    ...
    /* access modifiers changed from: private */
    public static /* synthetic */ boolean lambda$isSingleProfile$0(UserInfo userInfo) {
