...
public class StorageWizardMoveProgress extends StorageWizardBase {
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView(C1987R$layout.storage_wizard_progress);
        ...
        setHeaderText(C1992R$string.storage_wizard_move_progress_title, stringExtra);
        setBodyText(C1992R$string.storage_wizard_move_progress_body, bestVolumeDescription, stringExtra);
    ...
    public CharSequence moveStatusToMessage(int i) {
        if (i == -8) {
            return getString(C1992R$string.move_error_device_admin);
        }
        if (i == -5) {
            return getString(C1992R$string.invalid_location);
        }
        if (i == -3) {
            return getString(C1992R$string.system_package);
        }
        if (i != -2) {
            return i != -1 ? getString(C1992R$string.insufficient_storage) : getString(C1992R$string.insufficient_storage);
        }
        return getString(C1992R$string.does_not_exist);
    }
}
