...
public class PhoneNumberPreferenceController extends BasePreferenceController {
    ...
    public void copy() {
        ...
        Toast.makeText(this.mContext, context.getString(C1992R$string.copyable_slice_toast, new Object[]{context.getText(C1992R$string.status_number)}), 0).show();
    }

    private CharSequence getFirstPhoneNumber() {
        List<SubscriptionInfo> activeSubscriptionInfoList = this.mSubscriptionManager.getActiveSubscriptionInfoList();
        if (activeSubscriptionInfoList == null || activeSubscriptionInfoList.isEmpty()) {
            return this.mContext.getText(C1992R$string.device_info_default);
        }
    ...
    private CharSequence getPreferenceTitle(int i) {
        if (this.mTelephonyManager.getPhoneCount() <= 1) {
            return this.mContext.getString(C1992R$string.status_number);
        }
        return this.mContext.getString(C1992R$string.status_number_sim_slot, new Object[]{Integer.valueOf(i + 1)});
    }
    ...
    public CharSequence getFormattedPhoneNumber(SubscriptionInfo subscriptionInfo) {
        ...
        return TextUtils.isEmpty(bidiFormattedPhoneNumber) ? this.mContext.getString(C1992R$string.device_info_default) : bidiFormattedPhoneNumber;
    }
