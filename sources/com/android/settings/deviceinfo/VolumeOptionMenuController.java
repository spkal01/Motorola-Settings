...
public class VolumeOptionMenuController implements LifecycleObserver, OnCreateOptionsMenu, OnPrepareOptionsMenu, OnOptionsItemSelected {
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater menuInflater) {
        menuInflater.inflate(C1988R$menu.storage_volume, menu);
    }

    public void onPrepareOptionsMenu(Menu menu) {
        this.mRename = menu.findItem(C1985R$id.storage_rename);
        this.mMount = menu.findItem(C1985R$id.storage_mount);
        this.mUnmount = menu.findItem(C1985R$id.storage_unmount);
        this.mFormat = menu.findItem(C1985R$id.storage_format);
        this.mFormatAsPortable = menu.findItem(C1985R$id.storage_format_as_portable);
        this.mFormatAsInternal = menu.findItem(C1985R$id.storage_format_as_internal);
        this.mMigrate = menu.findItem(C1985R$id.storage_migrate);
        this.mFree = menu.findItem(C1985R$id.storage_free);
        this.mForget = menu.findItem(C1985R$id.storage_forget);
        boolean z = false;
    ...
    public boolean onOptionsItemSelected(MenuItem menuItem) {
        ...
        if (itemId == C1985R$id.storage_mount) {
            ...
            return true;
        } else if (itemId == C1985R$id.storage_unmount) {
            if (this.mStorageEntry.isMounted()) {
                if (this.mStorageEntry.isPublic()) {
                    ...
                    new SubSettingLauncher(this.mContext).setDestination(PrivateVolumeUnmount.class.getCanonicalName()).setTitleRes(C1992R$string.storage_menu_unmount).setSourceMetricsCategory(42).setArguments(bundle).launch();
                    return true;
                }
            }
            return false;
        } else if (itemId == C1985R$id.storage_rename) {
            ...
            return true;
        } else if (itemId == C1985R$id.storage_format) {
            ...
            return true;
        } else if (itemId == C1985R$id.storage_format_as_portable) {
            if (!this.mStorageEntry.isPrivate()) {
                return false;
            }
            Bundle bundle2 = new Bundle();
            bundle2.putString("android.os.storage.extra.VOLUME_ID", this.mStorageEntry.getId());
            new SubSettingLauncher(this.mContext).setDestination(PrivateVolumeFormat.class.getCanonicalName()).setTitleRes(C1992R$string.storage_menu_format).setSourceMetricsCategory(42).setArguments(bundle2).launch();
            return true;
        } else if (itemId == C1985R$id.storage_format_as_internal) {
            if (!this.mStorageEntry.isPublic()) {
                return false;
            }
            StorageWizardFormatConfirm.showPrivate(this.mFragment.getActivity(), this.mStorageEntry.getDiskId());
            return true;
        } else if (itemId == C1985R$id.storage_migrate) {
            if (!this.mStorageEntry.isPrivate()) {
                return false;
            }
            ...
            return true;
        } else if (itemId != C1985R$id.storage_forget || !this.mStorageEntry.isVolumeRecordMissed()) {
            return false;
        } else {
