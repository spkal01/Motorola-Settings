...
public class PublicVolumeSettings extends SettingsPreferenceFragment {
    ...
    public void onCreate(Bundle bundle) {
        ...
        addPreferencesFromResource(C1994R$xml.device_info_storage_volume);
        getPreferenceScreen().setOrderingAsAdded(true);
        this.mSummary = new StorageSummaryPreference(getPrefContext());
        this.mMount = buildAction(C1992R$string.storage_menu_mount);
        Button button = new Button(getActivity());
        this.mUnmount = button;
        button.setText(C1992R$string.storage_menu_unmount);
        this.mUnmount.setOnClickListener(this.mUnmountListener);
        this.mFormatPublic = buildAction(C1992R$string.storage_menu_format);
        if (this.mIsPermittedToAdopt) {
            this.mFormatPrivate = buildAction(C1992R$string.storage_menu_format_private);
        }
    }
    ...
    public void onActivityCreated(Bundle bundle) {
        ...
        if (isVolumeValid()) {
            ((ViewGroup) getActivity().findViewById(C1985R$id.container_material)).addView(this.mUnmount, new ViewGroup.LayoutParams(-1, -2));
        }
    }
    ...
    public void update() {
        ...
        if (this.mVolume.isMountedReadable()) {
            ...
            this.mSummary.setTitle(TextUtils.expandTemplate(getText(C1992R$string.storage_size_large), new CharSequence[]{formatBytes.value, formatBytes.units}));
            this.mSummary.setSummary((CharSequence) getString(C1992R$string.storage_volume_used, Formatter.formatFileSize(activity, totalSpace)));
