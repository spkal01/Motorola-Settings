...
public class StorageItemPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, EmptyTrashFragment.OnEmptyTrashCompleteListener {
    ...
    public StorageItemPreferenceController(Context context, Fragment fragment, VolumeInfo volumeInfo, StorageVolumeProvider storageVolumeProvider, boolean z) {
        ...
        this.mImagesUri = Uri.parse(context.getResources().getString(C1992R$string.config_images_storage_category_uri));
        this.mVideosUri = Uri.parse(context.getResources().getString(C1992R$string.config_videos_storage_category_uri));
        this.mAudioUri = Uri.parse(context.getResources().getString(C1992R$string.config_audio_storage_category_uri));
        this.mDocumentsAndOtherUri = Uri.parse(context.getResources().getString(C1992R$string.config_documents_and_other_storage_category_uri));
    }
    ...
    private void launchAppsIntent() {
        ...
        Intent intent = new SubSettingLauncher(this.mContext).setDestination(ManageApplications.class.getName()).setTitleRes(C1992R$string.apps_storage).setArguments(workAnnotatedBundle).setSourceMetricsCategory(this.mMetricsFeatureProvider.getMetricsCategory(this.mFragment)).toIntent();
    ...
    private void launchGamesIntent() {
        ...
        Intent intent = new SubSettingLauncher(this.mContext).setDestination(ManageApplications.class.getName()).setTitleRes(C1992R$string.game_storage_settings).setArguments(workAnnotatedBundle).setSourceMetricsCategory(this.mMetricsFeatureProvider.getMetricsCategory(this.mFragment)).toIntent();
        intent.putExtra("android.intent.extra.USER_ID", this.mUserId);
        Utils.launchIntent(this.mFragment, intent);
    }
    ...
    private void launchTrashIntent() {
        ...
        if (this.mPackageManager.resolveActivityAsUser(intent, 0, this.mUserId) == null || Build.IS_PRC_PRODUCT) {
            ...
            if (storageSize > 0) {
                ...
                Toast.makeText(this.mContext, C1992R$string.storage_trash_dialog_empty_message, 0).show();
            }
        } else {
            this.mContext.startActivityAsUser(intent, new UserHandle(this.mUserId));
        }
    ...
    public void onEmptyTrashComplete() {
        ...
        if (storageItemPreference != null) {
            storageItemPreference.setStorageSize(0, this.mTotalSize);
            updatePrivateStorageCategoryPreferencesOrder();
        }
    }
}
