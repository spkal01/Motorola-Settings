...
public class StorageWizardMigrateConfirm extends StorageWizardBase {
    ...
    public void onCreate(Bundle bundle) {
        ...
        setHeaderText(C1992R$string.storage_wizard_migrate_v2_title, getDiskShortDescription());
        setBodyText(C1992R$string.memory_calculating_size, new CharSequence[0]);
        setAuxChecklist();
        C08831 r4 = new MigrateEstimateTask(this) {
            public void onPostExecute(String str, String str2) {
                ...
                storageWizardMigrateConfirm.setBodyText(C1992R$string.storage_wizard_migrate_v2_body, storageWizardMigrateConfirm.getDiskDescription(), str, str2);
            }
        };
        this.mEstimate = r4;
        r4.copyFrom(getIntent());
        this.mEstimate.execute(new Void[0]);
        setBackButtonText(C1992R$string.storage_wizard_migrate_v2_later, new CharSequence[0]);
        setNextButtonText(C1992R$string.storage_wizard_migrate_v2_now, new CharSequence[0]);
    }
    ...
    public void onNavigateNext(View view) {
        if (StorageManager.isFileEncryptedNativeOrEmulated()) {
            for (UserInfo userInfo : ((UserManager) getSystemService(UserManager.class)).getUsers()) {
                if (!StorageManager.isUserKeyUnlocked(userInfo.id)) {
                    ...
                    new ChooseLockSettingsHelper.Builder(this).setRequestCode(100).setDescription(TextUtils.expandTemplate(getText(C1992R$string.storage_wizard_move_unlock), new CharSequence[]{userInfo.name})).setUserId(userInfo.id).setAllowAnyUserId(true).setForceVerifyPath(true).show();
                    return;
                }
            }
        }
        try {
            ...
            Toast.makeText(this, getString(C1992R$string.another_migration_already_in_progress), 1).show();
            finishAffinity();
        }
    }
