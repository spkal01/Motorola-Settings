...
public abstract class StorageWizardBase extends FragmentActivity {
    ...
    public void setContentView(int i) {
        ...
        footerBarMixin.setSecondaryButton(new FooterButton.Builder(this).setText(C1992R$string.wizard_back).setListener(new StorageWizardBase$$ExternalSyntheticLambda0(this)).setButtonType(0).setTheme(C1993R$style.SudGlifButton_Secondary).build());
        this.mFooterBarMixin.setPrimaryButton(new FooterButton.Builder(this).setText(C1992R$string.wizard_next).setListener(new StorageWizardBase$$ExternalSyntheticLambda1(this)).setButtonType(5).setTheme(C1993R$style.SudGlifButton_Primary).build());
    ...
    public GlifLayout getGlifLayout() {
        return (GlifLayout) requireViewById(C1985R$id.setup_wizard_layout);
    }

    /* access modifiers changed from: protected */
    public ProgressBar getProgressBar() {
        return (ProgressBar) requireViewById(C1985R$id.storage_wizard_progress);
    }

    /* access modifiers changed from: protected */
    public void setCurrentProgress(int i) {
        getProgressBar().setProgress(i);
        ((TextView) requireViewById(C1985R$id.storage_wizard_progress_summary)).setText(NumberFormat.getPercentInstance().format(((double) i) / 100.0d));
    }
    ...
    public void setBodyText(int i, CharSequence... charSequenceArr) {
        TextView textView = (TextView) requireViewById(C1985R$id.storage_wizard_body);
    ...
    public void setAuxChecklist() {
        FrameLayout frameLayout = (FrameLayout) requireViewById(C1985R$id.storage_wizard_aux);
        frameLayout.addView(LayoutInflater.from(frameLayout.getContext()).inflate(C1987R$layout.storage_wizard_checklist, frameLayout, false));
        frameLayout.setVisibility(0);
        ((TextView) frameLayout.requireViewById(C1985R$id.storage_wizard_migrate_v2_checklist_media)).setText(TextUtils.expandTemplate(getText(C1992R$string.storage_wizard_migrate_v2_checklist_media), new CharSequence[]{getDiskShortDescription()}));
    }
    ...
    public CharSequence getDiskDescription() {
        ...
        return getText(C1992R$string.unknown);
    }
