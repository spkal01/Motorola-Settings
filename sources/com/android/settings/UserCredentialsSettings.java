...
public class UserCredentialsSettings extends SettingsPreferenceFragment implements View.OnClickListener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        getActivity().setTitle(C1992R$string.user_credentials);
    }

    /* access modifiers changed from: protected */
    public void announceRemoval(String str) {
        if (isAdded()) {
            getListView().announceForAccessibility(getString(C1992R$string.user_credential_removed, str));
        }
    }
    ...
    public static class CredentialDialogFragment extends InstrumentedDialogFragment {
        ...
        public Dialog onCreateDialog(Bundle bundle) {
            ...
            View inflate = getActivity().getLayoutInflater().inflate(C1987R$layout.user_credential_dialog, (ViewGroup) null);
            ViewGroup viewGroup = (ViewGroup) inflate.findViewById(C1985R$id.credential_container);
            viewGroup.addView(UserCredentialsSettings.getCredentialView(credential, C1987R$layout.user_credential, (View) null, viewGroup, true));
            AlertDialog.Builder positiveButton = new AlertDialog.Builder(getActivity()).setView(inflate).setTitle(C1992R$string.user_credential_title).setPositiveButton(C1992R$string.done, (DialogInterface.OnClickListener) null);
            final int myUserId = UserHandle.myUserId();
            if (!RestrictedLockUtilsInternal.hasBaseUserRestriction(getContext(), "no_config_credentials", myUserId)) {
                positiveButton.setNegativeButton(C1992R$string.trusted_credentials_remove_label, (DialogInterface.OnClickListener) new DialogInterface.OnClickListener() {
        ...
        public void onPostExecute(List<Credential> list) {
            if (UserCredentialsSettings.this.isAdded()) {
                if (list == null || list.size() == 0) {
                    ...
                    textView.setText(C1992R$string.user_credential_none_installed);
    ...
    private static class CredentialAdapter extends RecyclerView.Adapter<ViewHolder> {
        private static final int LAYOUT_RESOURCE = C1987R$layout.user_credential_preference;
    ...
    static {
        ...
        sparseArray.put(C1985R$id.contents_userkey, Credential.Type.USER_KEY);
        sparseArray.put(C1985R$id.contents_usercrt, Credential.Type.USER_CERTIFICATE);
        sparseArray.put(C1985R$id.contents_cacrt, Credential.Type.CA_CERTIFICATE);
    }
    ...
    protected static View getCredentialView(Credential credential, int i, View view, ViewGroup viewGroup, boolean z) {
        ...
        ((TextView) view.findViewById(C1985R$id.alias)).setText(credential.alias);
        TextView textView = (TextView) view.findViewById(C1985R$id.purpose);
        if (credential.isSystem()) {
            i2 = C1992R$string.credential_for_vpn_and_apps;
        } else {
            i2 = C1992R$string.credential_for_wifi;
        }
        textView.setText(i2);
        view.findViewById(C1985R$id.contents).setVisibility(z ? 0 : 8);
        if (z) {
            int i3 = 0;
            while (true) {
