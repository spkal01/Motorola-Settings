...
public class AdvancedBluetoothDetailsHeaderController extends BasePreferenceController implements LifecycleObserver, OnStart, OnStop, OnDestroy, CachedBluetoothDevice.Callback {
    ...
    public void refresh() {
        ...
        if (layoutPreference != null && this.mCachedDevice != null) {
            ((TextView) layoutPreference.findViewById(C1985R$id.entity_header_title)).setText(this.mCachedDevice.getName());
            ((TextView) this.mLayoutPreference.findViewById(C1985R$id.entity_header_summary)).setText(this.mCachedDevice.getConnectionSummary(true));
            ...
            if (TextUtils.equals(stringMetaData, "Watch") || TextUtils.equals(stringMetaData, "Default")) {
                this.mLayoutPreference.findViewById(C1985R$id.layout_left).setVisibility(8);
                this.mLayoutPreference.findViewById(C1985R$id.layout_right).setVisibility(8);
                updateSubLayout((LinearLayout) this.mLayoutPreference.findViewById(C1985R$id.layout_middle), 5, 18, 20, 19, 0, 4);
            } else if (TextUtils.equals(stringMetaData, "Untethered Headset") || BluetoothUtils.getBooleanMetaData(device, 6)) {
                updateSubLayout((LinearLayout) this.mLayoutPreference.findViewById(C1985R$id.layout_left), 7, 10, 21, 13, C1992R$string.bluetooth_left_name, 1);
                updateSubLayout((LinearLayout) this.mLayoutPreference.findViewById(C1985R$id.layout_middle), 9, 12, 23, 15, C1992R$string.bluetooth_middle_name, 3);
                updateSubLayout((LinearLayout) this.mLayoutPreference.findViewById(C1985R$id.layout_right), 8, 11, 22, 14, C1992R$string.bluetooth_right_name, 2);
            }
        }
    }
    ...
    public Drawable createBtBatteryIcon(Context context, int i, boolean z) {
        BatteryMeterView.BatteryMeterDrawable batteryMeterDrawable = new BatteryMeterView.BatteryMeterDrawable(context, context.getColor(C1981R$color.meter_background_color), context.getResources().getDimensionPixelSize(C1982R$dimen.advanced_bluetooth_battery_meter_width), context.getResources().getDimensionPixelSize(C1982R$dimen.advanced_bluetooth_battery_meter_height));
    ...
    private void updateSubLayout(LinearLayout linearLayout, int i, int i2, int i3, int i4, int i5, int i6) {
        if (linearLayout != null) {
            ...
            ImageView imageView = (ImageView) linearLayout.findViewById(C1985R$id.header_icon);
            ...
            TextView textView = (TextView) linearLayout.findViewById(C1985R$id.bt_battery_summary);
            if (!isUntetheredHeadset(device)) {
                ...
                linearLayout.setVisibility(0);
                linearLayout.findViewById(C1985R$id.bt_battery_icon).setVisibility(8);
            ...
            TextView textView2 = (TextView) linearLayout.findViewById(C1985R$id.header_title);
            if (i6 == 4) {
    ...
    public void showBatteryPredictionIfNecessary(int i, long j, LinearLayout linearLayout) {
        ThreadUtils.postOnMainThread(new C0790xd65cca4f(this, linearLayout, i, j));
    }

    /* access modifiers changed from: private */
    public /* synthetic */ void lambda$showBatteryPredictionIfNecessary$1(LinearLayout linearLayout, int i, long j) {
        TextView textView = (TextView) linearLayout.findViewById(C1985R$id.bt_battery_prediction);
        if (i == 1) {
            textView.setVisibility(0);
    ...
    private void showBatteryIcon(LinearLayout linearLayout, int i, int i2, boolean z) {
        boolean z2 = i <= i2 && !z;
        ImageView imageView = (ImageView) linearLayout.findViewById(C1985R$id.bt_battery_icon);
        if (z2) {
            imageView.setImageDrawable(this.mContext.getDrawable(C1983R$drawable.ic_battery_alert_24dp));
            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(this.mContext.getResources().getDimensionPixelSize(C1982R$dimen.advanced_bluetooth_battery_width), this.mContext.getResources().getDimensionPixelSize(C1982R$dimen.advanced_bluetooth_battery_height));
            layoutParams.rightMargin = this.mContext.getResources().getDimensionPixelSize(C1982R$dimen.advanced_bluetooth_battery_right_margin);
    ...
    private void updateDisconnectLayout() {
        this.mLayoutPreference.findViewById(C1985R$id.layout_left).setVisibility(8);
        this.mLayoutPreference.findViewById(C1985R$id.layout_right).setVisibility(8);
        LinearLayout linearLayout = (LinearLayout) this.mLayoutPreference.findViewById(C1985R$id.layout_middle);
        linearLayout.setVisibility(0);
        linearLayout.findViewById(C1985R$id.header_title).setVisibility(8);
        linearLayout.findViewById(C1985R$id.bt_battery_summary).setVisibility(8);
        linearLayout.findViewById(C1985R$id.bt_battery_icon).setVisibility(8);
        ...
        if (stringMetaData != null) {
            updateIcon((ImageView) linearLayout.findViewById(C1985R$id.header_icon), stringMetaData);
        }
    }
    ...
    public void updateIcon(ImageView imageView, String str) {
        ...
        ThreadUtils.postOnBackgroundThread((Runnable) new C0792xd65cca51(this, str, imageView));
    }
    ...
    public /* synthetic */ void lambda$updateIcon$3(String str, ImageView imageView) {
        ...
        try {
            ...
            ThreadUtils.postOnMainThread(new C0791xd65cca50(this, str, MediaStore.Images.Media.getBitmap(this.mContext.getContentResolver(), parse), imageView));
        } catch (IOException e) {
