            ...
            if (!((UserManager) context.getSystemService("user")).isManagedProfile()) {
                ...
                if (!checkUserChoice()) {
                    ...
                    if (!((PowerManager) context.getSystemService("power")).isScreenOn() || !LocalBluetoothPreferences.shouldShowDialogInForeground(context, address, str3)) {
                        ...
                        if (i == 2) {
                            str = context.getString(C1992R$string.bluetooth_phonebook_request);
                            str2 = context.getString(C1992R$string.bluetooth_phonebook_access_notification_content);
                        } else if (i == 3) {
                            str = context.getString(C1992R$string.bluetooth_map_request);
                            str2 = context.getString(C1992R$string.bluetooth_message_access_notification_content);
                        } else if (i != 4) {
                            str = context.getString(C1992R$string.bluetooth_connect_access_notification_title);
                            str2 = context.getString(C1992R$string.bluetooth_connect_access_notification_content, new Object[]{createRemoteName, createRemoteName});
                        } else {
                            str = context.getString(C1992R$string.bluetooth_sim_card_access_notification_title);
                            str2 = context.getString(C1992R$string.bluetooth_sim_card_access_notification_content, new Object[]{createRemoteName, createRemoteName});
                        }
                        NotificationManager notificationManager = (NotificationManager) context.getSystemService("notification");
                        if (this.mNotificationChannel == null) {
                            NotificationChannel notificationChannel = new NotificationChannel("bluetooth_notification_channel", context.getString(C1992R$string.bluetooth), 4);
