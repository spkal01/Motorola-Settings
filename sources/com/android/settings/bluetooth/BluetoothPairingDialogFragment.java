...
public class BluetoothPairingDialogFragment extends InstrumentedDialogFragment implements TextWatcher, DialogInterface.OnClickListener {
    ...
    private AlertDialog createUserEntryDialog() {
        this.mBuilder.setTitle((CharSequence) getString(C1992R$string.bluetooth_pairing_request, this.mPairingController.getDeviceName()));
    ...
    private View createPinEntryView() {
        View inflate = getActivity().getLayoutInflater().inflate(C1987R$layout.bluetooth_pin_entry, (ViewGroup) null);
        TextView textView = (TextView) inflate.findViewById(C1985R$id.pin_values_hint);
        TextView textView2 = (TextView) inflate.findViewById(C1985R$id.message_below_pin);
        CheckBox checkBox = (CheckBox) inflate.findViewById(C1985R$id.alphanumeric_pin);
        CheckBox checkBox2 = (CheckBox) inflate.findViewById(C1985R$id.phonebook_sharing_message_entry_pin);
        checkBox2.setText(getString(C1992R$string.bluetooth_pairing_shares_phonebook, this.mPairingController.getDeviceName()));
        EditText editText = (EditText) inflate.findViewById(C1985R$id.text);
        checkBox2.setVisibility((!this.mPairingController.isHeadSetDevices() || this.mPairingController.isProfileReady()) ? 8 : 0);
    ...
    private AlertDialog createConfirmationDialog() {
        ...
        this.mBuilder.setPositiveButton((CharSequence) getString(C1992R$string.bluetooth_pairing_accept), (DialogInterface.OnClickListener) this);
        this.mBuilder.setNegativeButton((CharSequence) getString(C1992R$string.bluetooth_pairing_decline), (DialogInterface.OnClickListener) this);
    ...
    private View createView() {
        View inflate = getActivity().getLayoutInflater().inflate(C1987R$layout.bluetooth_pin_confirm, (ViewGroup) null);
        TextView textView = (TextView) inflate.findViewById(C1985R$id.pairing_caption);
        TextView textView2 = (TextView) inflate.findViewById(C1985R$id.pairing_subhead);
        TextView textView3 = (TextView) inflate.findViewById(C1985R$id.pairing_code_message);
        CheckBox checkBox = (CheckBox) inflate.findViewById(C1985R$id.phonebook_sharing_message_confirm_pin);
        checkBox.setText(getString(C1992R$string.bluetooth_pairing_shares_phonebook, this.mPairingController.getDeviceName()));
        int i = 8;
        checkBox.setVisibility((!this.mPairingController.isHeadSetDevices() || this.mPairingController.isProfileReady()) ? 8 : 0);
        this.mPairingController.setContactSharingState();
