...
public class BluetoothPairingController implements CompoundButton.OnCheckedChangeListener {
    ...
    public int getDeviceVariantMessageId() {
        ...
        if (i != 0) {
            if (i == 1) {
                return C1992R$string.bluetooth_enter_passkey_other_device;
            }
            if (i != 7) {
                return -1;
            }
        }
        return C1992R$string.bluetooth_enter_pin_other_device;
    }

    public int getDeviceVariantMessageHintId() {
        int i = this.mType;
        if (i == 0 || i == 1) {
            return C1992R$string.bluetooth_pin_values_hint;
        }
        if (i != 7) {
            return -1;
        }
        return C1992R$string.bluetooth_pin_values_hint_16_digits;
    }
    ...
    public boolean isHeadSetDevices() {
        if (this.mDevice.getBluetoothClass() != null && this.mDevice.getBluetoothClass().getDeviceClass() == 1032) {
            return true;
        }
        if (this.mDevice.getBluetoothClass() == null || this.mDevice.getBluetoothClass().getMajorDeviceClass() != 1024) {
            return false;
        }
        return true;
    }
