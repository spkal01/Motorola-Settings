...
public class UsbBackend {
    ...
    public UsbBackend(Context context, UserManager userManager) {
        ...
        this.mTetheringSupported = ((TetheringManager) context.getSystemService(TetheringManager.class)).isTetheringSupported();
        updatePorts();
    }
    ...
    public boolean areFunctionsSupported(long j) {
        if (!MotoCdromUtils.isCdromFeatureSupported(this.mContext) && (MotoCdromUtils.FUNCTION_CDROM & j) != 0) {
            return false;
        }
        if (!MotoWebcamUtils.isWebcamFeatureSupported(this.mContext) && (MotoWebcamUtils.FUNCTION_WEBCAM & j) != 0) {
            return false;
        }
        if (!MotoReadyForUtils.isReadyForFeatureSupported(this.mContext) && (MotoReadyForUtils.FUNCTION_READYFOR & j) != 0) {
            return false;
        }
