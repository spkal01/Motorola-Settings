...
public class UsbConnectionBroadcastReceiver extends BroadcastReceiver implements LifecycleObserver, OnResume, OnPause {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if ("android.hardware.usb.action.USB_STATE".equals(intent.getAction())) {
            ...
            if (MotoCdromUtils.isCdromFeatureSupported(context) && intent.getExtras().getBoolean(MotoCdromUtils.USB_FUNCTION_CDROM)) {
                j |= MotoCdromUtils.FUNCTION_CDROM;
            }
            if (MotoWebcamUtils.isWebcamFeatureSupported(context) && intent.getExtras().getBoolean(MotoWebcamUtils.USB_FUNCTION_WEBCAM)) {
                j |= MotoWebcamUtils.FUNCTION_WEBCAM;
            }
            if (MotoReadyForUtils.isReadyForFeatureSupported(context) && intent.getExtras().getBoolean(MotoReadyForUtils.USB_FUNCTION_READYFOR)) {
                j |= MotoReadyForUtils.FUNCTION_READYFOR;
            }
            this.mFunctions = j;
