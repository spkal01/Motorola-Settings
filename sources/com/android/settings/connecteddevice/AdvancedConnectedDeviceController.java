    ...
    static int getConnectedDevicesSummaryResourceId(NfcPreferenceController nfcPreferenceController, boolean z, boolean z2) {
        if (z2) {
            if (nfcPreferenceController.isAvailable()) {
                if (z) {
                    return C1992R$string.connected_devices_dashboard_android_auto_summary;
                }
                return C1992R$string.connected_devices_dashboard_android_auto_no_driving_mode_summary;
            } else if (z) {
                return C1992R$string.connected_devices_dashboard_android_auto_no_nfc_summary;
            } else {
                return C1992R$string.connected_devices_dashboard_android_auto_no_nfc_no_driving_mode;
            }
        } else if (nfcPreferenceController.isAvailable()) {
            if (z) {
                return C1992R$string.connected_devices_dashboard_summary;
            }
            return C1992R$string.connected_devices_dashboard_no_driving_mode_summary;
        } else if (z) {
            return C1992R$string.connected_devices_dashboard_no_nfc_summary;
        } else {
            return C1992R$string.connected_devices_dashboard_no_driving_mode_no_nfc_summary;
        }
    }
}
