...
public class PrintJobSettingsFragment extends DashboardFragment {
    ...
    public int getPreferenceScreenResId() {
        return C1994R$xml.print_job_settings;
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater menuInflater) {
        ...
        if (printJob != null) {
            if (!printJob.getInfo().isCancelling()) {
                menu.add(0, 1, 0, getString(C1992R$string.print_cancel)).setShowAsAction(1);
            }
            if (printJob.isFailed()) {
                menu.add(0, 2, 0, getString(C1992R$string.print_restart)).setShowAsAction(1);
            }
        }
    }
