...
public class PhoneLanguagePreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin {
    ...
    public boolean isAvailable() {
        if (!this.mContext.getResources().getBoolean(C1980R$bool.config_show_phone_language) || this.mContext.getAssets().getLocales().length <= 1) {
            return false;
        }
        return true;
    }
    ...
    public boolean handlePreferenceTreeClick(Preference preference) {
        ...
        new SubSettingLauncher(this.mContext).setDestination(LocaleListEditor.class.getName()).setSourceMetricsCategory(750).setTitleRes(C1992R$string.language_picker_title).launch();
        return true;
    }
}
