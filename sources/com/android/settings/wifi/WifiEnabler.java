...
import com.motorola.settings.tether.WifiHotspotConflictWarningDialog;
...
public class WifiEnabler implements SwitchWidgetController.OnSwitchChangeListener {
    ...
    public boolean onSwitchToggled(boolean z) {
        ...
        if (!z || WirelessUtils.isRadioAllowed(this.mContext, "wifi")) {
            ...
            if (!z || !WifiHotspotConflictWarningDialog.shouldShow(this.mContext)) {
                toggleWifi(z);
                return true;
            }
            WifiHotspotConflictWarningDialog.show(((Activity) this.mContext).getFragmentManager(), getWarningDialogCallback());
            return true;
        }
        Toast.makeText(this.mContext, C1992R$string.wifi_in_airplane_mode, 0).show();
    ...
    private WarningDialogCallback getWarningDialogCallback() {
        return new WarningDialogCallback() {
            public void onPositive() {
                WifiEnabler.this.toggleWifi(true);
            }
    ...
    public void toggleWifi(boolean z) {
        if (!this.mWifiManager.setWifiEnabled(z)) {
            this.mSwitchWidget.setEnabled(true);
            Toast.makeText(this.mContext, C1992R$string.wifi_error, 0).show();
        }
    }
}
