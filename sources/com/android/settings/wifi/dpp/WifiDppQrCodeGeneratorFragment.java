...
public class WifiDppQrCodeGeneratorFragment extends WifiDppQrCodeBaseFragment {
    ...
    public void onActivityCreated(Bundle bundle) {
        ...
        if (wifiNetworkConfigFromHostActivity.isHotspot()) {
            getActivity().setTitle(C1992R$string.wifi_dpp_share_hotspot);
        } else {
            getActivity().setTitle(C1992R$string.wifi_dpp_share_wifi);
        }
    }
    ...
    public final View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        return layoutInflater.inflate(C1987R$layout.wifi_dpp_qrcode_generator_fragment, viewGroup, false);
    }

    public void onViewCreated(View view, Bundle bundle) {
        super.onViewCreated(view, bundle);
        this.mQrCodeView = (ImageView) view.findViewById(C1985R$id.qrcode_view);
        WifiNetworkConfig wifiNetworkConfigFromHostActivity = getWifiNetworkConfigFromHostActivity();
        if (wifiNetworkConfigFromHostActivity.isHotspot()) {
            setHeaderTitle(C1992R$string.wifi_dpp_share_hotspot, new Object[0]);
        } else {
            setHeaderTitle(C1992R$string.wifi_dpp_share_wifi, new Object[0]);
        }
        String preSharedKey = wifiNetworkConfigFromHostActivity.getPreSharedKey();
        TextView textView = (TextView) view.findViewById(C1985R$id.password);
        if (TextUtils.isEmpty(preSharedKey)) {
            this.mSummary.setText(getString(C1992R$string.wifi_dpp_scan_open_network_qr_code_with_another_device, wifiNetworkConfigFromHostActivity.getSsid()));
            textView.setVisibility(8);
        } else {
            this.mSummary.setText(getString(C1992R$string.wifi_dpp_scan_qr_code_with_another_device, wifiNetworkConfigFromHostActivity.getSsid()));
            if (wifiNetworkConfigFromHostActivity.isHotspot()) {
                textView.setText(getString(C1992R$string.wifi_dpp_hotspot_password, preSharedKey));
            } else {
                textView.setText(getString(C1992R$string.wifi_dpp_wifi_password, preSharedKey));
            }
        }
        Intent component = new Intent().setComponent(getNearbySharingComponent());
        addActionButton((ViewGroup) view.findViewById(C1985R$id.wifi_dpp_layout), createNearbyButton(component, new WifiDppQrCodeGeneratorFragment$$ExternalSyntheticLambda0(this, component, wifiNetworkConfigFromHostActivity)));
    ...
    private void addActionButton(ViewGroup viewGroup, Button button) {
        if (button != null) {
            ...
            int dimensionPixelSize = getResources().getDimensionPixelSize(17105493) / 2;
    ...
    private void setQrCode() {
        try {
            this.mQrCodeView.setImageBitmap(QrCodeGenerator.encodeQrCode(this.mQrCode, getContext().getResources().getDimensionPixelSize(C1982R$dimen.qrcode_size)));
