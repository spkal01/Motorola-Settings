...
public abstract class WifiDppQrCodeBaseFragment extends InstrumentedFragment {
    ...
    public void onViewCreated(View view, Bundle bundle) {
        ...
        if (isFooterAvailable()) {
            this.mLeftButton = new FooterButton.Builder(getContext()).setButtonType(2).setTheme(C1993R$style.SudGlifButton_Secondary).build();
            ((FooterBarMixin) this.mGlifLayout.getMixin(cls)).setSecondaryButton(this.mLeftButton);
            this.mRightButton = new FooterButton.Builder(getContext()).setButtonType(5).setTheme(C1993R$style.SudGlifButton_Primary).build();
