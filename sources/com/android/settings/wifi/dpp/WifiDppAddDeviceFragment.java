...
public class WifiDppAddDeviceFragment extends WifiDppQrCodeBaseFragment {
    ...
    public void showSuccessUi(boolean z) {
        setHeaderIconImageResource(C1983R$drawable.ic_devices_check_circle_green_32dp);
        setHeaderTitle(C1992R$string.wifi_dpp_wifi_shared_with_device, new Object[0]);
        setProgressBarShown(isEasyConnectHandshaking());
        this.mSummary.setVisibility(4);
        this.mWifiApPictureView.setImageResource(C1983R$drawable.wifi_dpp_success);
        this.mChooseDifferentNetwork.setVisibility(4);
        this.mLeftButton.setText(getContext(), C1992R$string.wifi_dpp_add_another_device);
        this.mLeftButton.setOnClickListener(new WifiDppAddDeviceFragment$$ExternalSyntheticLambda1(this));
        this.mRightButton.setText(getContext(), C1992R$string.done);
    ...
    public void showErrorUi(int i, Intent intent, boolean z) {
        ...
        switch (i) {
            case -12:
                charSequence = getText(C1992R$string.wifi_dpp_failure_enrollee_rejected_configuration);
                break;
            case -11:
                charSequence = getText(C1992R$string.wifi_dpp_failure_enrollee_authentication);
                break;
            case -10:
                charSequence = getText(C1992R$string.wifi_dpp_failure_cannot_find_network);
                break;
            case -9:
                throw new IllegalStateException("Wi-Fi DPP configurator used a non-PSK/non-SAEnetwork to handshake");
            case -8:
                charSequence = getString(C1992R$string.wifi_dpp_failure_not_supported, getSsid());
                break;
            case -7:
                charSequence = getText(C1992R$string.wifi_dpp_failure_generic);
                break;
            case -6:
                charSequence = getText(C1992R$string.wifi_dpp_failure_timeout);
                break;
            case -5:
                if (!z) {
                ...
                return;
            case -4:
                charSequence = getText(C1992R$string.wifi_dpp_failure_authentication_or_configuration);
                break;
            case -3:
                charSequence = getText(C1992R$string.wifi_dpp_failure_not_compatible);
                break;
            case -2:
                charSequence = getText(C1992R$string.wifi_dpp_failure_authentication_or_configuration);
                break;
            case -1:
                charSequence = getText(C1992R$string.wifi_dpp_qr_code_is_not_valid_format);
                break;
            default:
                throw new IllegalStateException("Unexpected Wi-Fi DPP error");
        }
        setHeaderTitle(C1992R$string.wifi_dpp_could_not_add_device, new Object[0]);
        this.mSummary.setText(charSequence);
        this.mWifiApPictureView.setImageResource(C1983R$drawable.wifi_dpp_error);
        this.mChooseDifferentNetwork.setVisibility(4);
        FooterButton footerButton = this.mLeftButton;
        if (hasRetryButton(i)) {
            this.mRightButton.setText(getContext(), C1992R$string.retry);
        } else {
            this.mRightButton.setText(getContext(), C1992R$string.done);
        ...
        if (isEasyConnectHandshaking()) {
            this.mSummary.setText(C1992R$string.wifi_dpp_sharing_wifi_with_this_device);
        }
        setProgressBarShown(isEasyConnectHandshaking());
    ...
    public final View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        return layoutInflater.inflate(C1987R$layout.wifi_dpp_add_device_fragment, viewGroup, false);
    }

    public void onViewCreated(View view, Bundle bundle) {
        super.onViewCreated(view, bundle);
        setHeaderIconImageResource(C1983R$drawable.ic_devices_other_32dp);
        String information = ((WifiDppConfiguratorActivity) getActivity()).getWifiDppQrCode().getInformation();
        int i = 0;
        if (TextUtils.isEmpty(information)) {
            setHeaderTitle(C1992R$string.wifi_dpp_device_found, new Object[0]);
        } else {
            setHeaderTitle(information);
        }
        updateSummary();
        this.mWifiApPictureView = (ImageView) view.findViewById(C1985R$id.wifi_ap_picture_view);
        Button button = (Button) view.findViewById(C1985R$id.choose_different_network);
        this.mChooseDifferentNetwork = button;
        button.setOnClickListener(new WifiDppAddDeviceFragment$$ExternalSyntheticLambda4(this));
        this.mLeftButton.setText(getContext(), C1992R$string.cancel);
        this.mLeftButton.setOnClickListener(new WifiDppAddDeviceFragment$$ExternalSyntheticLambda0(this));
        this.mRightButton.setText(getContext(), C1992R$string.wifi_dpp_share_wifi);
    ...
    private void updateSummary() {
        ...
        this.mSummary.setText(getString(C1992R$string.wifi_dpp_add_device_to_wifi, getSsid()));
    }
}
