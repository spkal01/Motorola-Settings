...
public class WifiP2pPeer extends Preference {
    ...
    private static final int[] STATE_SECURED = {C1979R$attr.state_encrypted};
    ...
    public WifiP2pPeer(Context context, WifiP2pDevice wifiP2pDevice) {
        ...
        setWidgetLayoutResource(C1987R$layout.preference_widget_wifi_signal);
        ...
        setSummary((CharSequence) context.getResources().getStringArray(C1978R$array.wifi_p2p_status)[this.device.status]);
    }

    public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
        super.onBindViewHolder(preferenceViewHolder);
        ImageView imageView = (ImageView) preferenceViewHolder.findViewById(C1985R$id.signal);
        this.mSignal = imageView;
        if (this.mRssi == Integer.MAX_VALUE) {
            imageView.setImageDrawable((Drawable) null);
        } else {
            imageView.setImageResource(C1983R$drawable.wifi_signal);
