...
public class WifiStatusTest extends Activity {
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView(C1987R$layout.wifi_status_test);
        Button button = (Button) findViewById(C1985R$id.update);
        this.updateButton = button;
        button.setOnClickListener(this.updateButtonHandler);
        this.mWifiState = (TextView) findViewById(C1985R$id.wifi_state);
        this.mNetworkState = (TextView) findViewById(C1985R$id.network_state);
        this.mSupplicantState = (TextView) findViewById(C1985R$id.supplicant_state);
        this.mRSSI = (TextView) findViewById(C1985R$id.rssi);
        this.mBSSID = (TextView) findViewById(C1985R$id.bssid);
        this.mSSID = (TextView) findViewById(C1985R$id.ssid);
        this.mHiddenSSID = (TextView) findViewById(C1985R$id.hidden_ssid);
        this.mIPAddr = (TextView) findViewById(C1985R$id.ipaddr);
        this.mMACAddr = (TextView) findViewById(C1985R$id.macaddr);
        this.mNetworkId = (TextView) findViewById(C1985R$id.networkid);
        this.mTxLinkSpeed = (TextView) findViewById(C1985R$id.tx_link_speed);
        this.mRxLinkSpeed = (TextView) findViewById(C1985R$id.rx_link_speed);
        this.mScanList = (TextView) findViewById(C1985R$id.scan_list);
        this.mPingHostname = (TextView) findViewById(C1985R$id.pingHostname);
        this.mHttpClientTest = (TextView) findViewById(C1985R$id.httpClientTest);
        Button button2 = (Button) findViewById(C1985R$id.ping_test);
    ...
    public void setWifiStateText(int i) {
        String str;
        if (i == 0) {
            str = getString(C1992R$string.wifi_state_disabling);
        } else if (i == 1) {
            str = getString(C1992R$string.wifi_state_disabled);
        } else if (i == 2) {
            str = getString(C1992R$string.wifi_state_enabling);
        } else if (i == 3) {
            str = getString(C1992R$string.wifi_state_enabled);
        } else if (i != 4) {
            Log.e("WifiStatusTest", "wifi state is bad");
            str = "BAD";
        } else {
            str = getString(C1992R$string.wifi_state_unknown);
        }
    ...
    public final void updatePingState() {
        ...
        int i = C1992R$string.radioInfo_unknown;
