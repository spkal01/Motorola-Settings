...
public class WifiCallingSliceHelper {
    ...
    public Slice createWifiCallingSlice(Uri uri) {
        ...
        if (!SubscriptionManager.isValidSubscriptionId(defaultVoiceSubId)) {
            ...
            return getNonActionableWifiCallingSlice(resourcesForSubId.getText(C1992R$string.wifi_calling_settings_title), resourcesForSubId.getText(C1992R$string.wifi_calling_settings_activation_instructions), uri, getActivityIntent("android.settings.WIFI_CALLING_SETTINGS"));
        }
    }
    ...
    private Slice getWifiCallingSlice(Uri uri, boolean z, int i) {
        IconCompat createWithResource = IconCompat.createWithResource(this.mContext, C1983R$drawable.wifi_signal);
        ...
        int i2 = C1992R$string.wifi_calling_settings_title;
        return accentColor.addRow(rowBuilder.setTitle(resourcesForSubId.getText(i2)).addEndItem(SliceAction.createToggle(getBroadcastIntent("com.android.settings.wifi.calling.action.WIFI_CALLING_CHANGED"), (CharSequence) null, z)).setPrimaryAction(SliceAction.createDeeplink(getActivityIntent("android.settings.WIFI_CALLING_SETTINGS"), createWithResource, 0, resourcesForSubId.getText(i2)))).build();
    }
    ...
    public Slice createWifiCallingPreferenceSlice(Uri uri) {
        ...
        if (!isCarrierConfigManagerKeyEnabled) {
            ...
            try {
                ...
                Resources resourcesForSubId = getResourcesForSubId(defaultVoiceSubId);
                return getNonActionableWifiCallingSlice(resourcesForSubId.getText(C1992R$string.wifi_calling_mode_title), resourcesForSubId.getText(C1992R$string.wifi_calling_turn_on), uri, getActivityIntent("android.settings.WIFI_CALLING_SETTINGS"));
    ...
    private Slice getWifiCallingPreferenceSlice(boolean z, int i, Uri uri, int i2) {
        ...
        int i3 = C1992R$string.wifi_calling_mode_title;
        ...
        if (z) {
            accentColor.addRow(wifiPreferenceRowBuilder(accentColor, 17041681, "com.android.settings.slice.action.WIFI_CALLING_PREFERENCE_WIFI_ONLY", i == 0, i2));
        }
        accentColor.addRow(wifiPreferenceRowBuilder(accentColor, 17041682, "com.android.settings.slice.action.WIFI_CALLING_PREFERENCE_WIFI_PREFERRED", i == 2, i2));
        accentColor.addRow(wifiPreferenceRowBuilder(accentColor, 17041679, "com.android.settings.slice.action.WIFI_CALLING_PREFERENCE_CELLULAR_PREFERRED", i == 1, i2));
    ...
    private ListBuilder.RowBuilder wifiPreferenceRowBuilder(ListBuilder listBuilder, int i, String str, boolean z, int i2) {
        IconCompat createWithResource = IconCompat.createWithResource(this.mContext, C1983R$drawable.radio_button_check);
        Resources resourcesForSubId = getResourcesForSubId(i2);
    ...
    private CharSequence getWifiCallingPreferenceSummary(int i, int i2) {
        Resources resourcesForSubId = getResourcesForSubId(i2);
        if (i == 0) {
            return resourcesForSubId.getText(17041681);
        }
        if (i == 1) {
            return resourcesForSubId.getText(17041679);
        }
        if (i != 2) {
            return null;
        }
        return resourcesForSubId.getText(17041682);
    }
    ...
    private Slice getNonActionableWifiCallingSlice(CharSequence charSequence, CharSequence charSequence2, Uri uri, PendingIntent pendingIntent) {
        ListBuilder.RowBuilder primaryAction = new ListBuilder.RowBuilder().setTitle(charSequence).setPrimaryAction(SliceAction.createDeeplink(pendingIntent, IconCompat.createWithResource(this.mContext, C1983R$drawable.wifi_signal), 1, charSequence));
        if (!com.android.settings.Utils.isSettingsIntelligence(this.mContext)) {
