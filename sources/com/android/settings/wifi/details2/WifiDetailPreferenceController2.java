...
public class WifiDetailPreferenceController2 extends AbstractPreferenceController implements PreferenceControllerMixin, WifiDialog2.WifiDialog2Listener, LifecycleObserver, OnPause, OnResume, WifiEntry.WifiEntryCallback, WifiEntry.ConnectCallback, WifiEntry.DisconnectCallback, WifiEntry.ForgetCallback, WifiEntry.SignInCallback {
    ...
    private class CarrierIdAsyncQueryHandler extends AsyncQueryHandler {
        ...
        public void onQueryComplete(int i, Object obj, Cursor cursor) {
            ...
            if (WifiDetailPreferenceController2.this.mContext == null || cursor == null || !cursor.moveToFirst()) {
                ...
                WifiDetailPreferenceController2.this.mEapSimSubscriptionPref.setSummary(C1992R$string.wifi_require_sim_card_to_connect);
                return;
            }
            WifiDetailPreferenceController2.this.mEapSimSubscriptionPref.setSummary((CharSequence) WifiDetailPreferenceController2.this.mContext.getString(C1992R$string.wifi_require_specific_sim_card_to_connect, new Object[]{cursor.getString(0)}));
            cursor.close();
        }
    }
    ...
    WifiDetailPreferenceController2(WifiEntry wifiEntry, ConnectivityManager connectivityManager, Context context, PreferenceFragmentCompat preferenceFragmentCompat, Handler handler, Lifecycle lifecycle, WifiManager wifiManager, MetricsFeatureProvider metricsFeatureProvider, IconInjector iconInjector, Clock clock) {
        ...
        this.mSignalStr = context.getResources().getStringArray(C1978R$array.wifi_signal);
    ...
    public void displayPreference(PreferenceScreen preferenceScreen) {
        ...
        this.mButtonsPref = ((ActionButtonsPreference) preferenceScreen.findPreference(KEY_BUTTONS_PREF)).setButton1Text(C1992R$string.forget).setButton1Icon(C1983R$drawable.ic_settings_delete).setButton1OnClickListener(new WifiDetailPreferenceController2$$ExternalSyntheticLambda3(this)).setButton2Text(C1992R$string.wifi_sign_in_button_text).setButton2Icon(C1983R$drawable.ic_settings_sign_in).setButton2OnClickListener(new WifiDetailPreferenceController2$$ExternalSyntheticLambda5(this)).setButton3Text(getConnectDisconnectButtonTextResource()).setButton3Icon(getConnectDisconnectButtonIconResource()).setButton3OnClickListener(new WifiDetailPreferenceController2$$ExternalSyntheticLambda4(this)).setButton4Text(C1992R$string.share).setButton4Icon(C1983R$drawable.ic_qrcode_24dp).setButton4OnClickListener(new WifiDetailPreferenceController2$$ExternalSyntheticLambda2(this));
    ...
    private boolean updateCaptivePortalButton() {
        ...
        if (captivePortalVenueInfoUrl == null) {
            this.mButtonsPref.setButton2Text(C1992R$string.wifi_sign_in_button_text).setButton2Icon(C1983R$drawable.ic_settings_sign_in).setButton2OnClickListener(new WifiDetailPreferenceController2$$ExternalSyntheticLambda1(this));
            return canSignIntoNetwork();
        }
        this.mButtonsPref.setButton2Text(C1992R$string.wifi_venue_website_button_text).setButton2Icon(C1983R$drawable.ic_settings_sign_in).setButton2OnClickListener(new WifiDetailPreferenceController2$$ExternalSyntheticLambda6(this, captivePortalVenueInfoUrl));
    ...
    private void setupEntityHeader(PreferenceScreen preferenceScreen) {
        ...
        this.mEntityHeaderController = EntityHeaderController.newInstance(this.mFragment.getActivity(), this.mFragment, layoutPreference.findViewById(C1985R$id.entity_header));
        ((ImageView) layoutPreference.findViewById(C1985R$id.entity_header_icon)).setScaleType(ImageView.ScaleType.CENTER_INSIDE);
    ...
    private String getExpiryTimeSummary() {
        ...
        if (now.plusDays(2).isAfter(ofInstant)) {
            ...
            return context.getString(C1992R$string.wifi_time_remaining, new Object[]{StringUtil.formatElapsedTime(context, (double) (Duration.between(now, ofInstant).getSeconds() * 1000), false, false)});
        }
        return this.mContext.getString(C1992R$string.wifi_expiry_time, new Object[]{DateTimeFormatter.ofLocalizedDateTime(FormatStyle.SHORT).format(ofInstant)});
    }
    ...
    private Drawable redrawIconForHeader(Drawable drawable) {
        int dimensionPixelSize = this.mContext.getResources().getDimensionPixelSize(C1982R$dimen.wifi_detail_page_header_image_size);
    ...
    private void refreshFrequency() {
        ...
        if (i >= 2400 && i < 2500) {
            str = this.mContext.getResources().getString(C1992R$string.wifi_band_24ghz);
        } else if (i >= 4900 && i < 5900) {
            str = this.mContext.getResources().getString(C1992R$string.wifi_band_5ghz);
        } else if (i >= 5925 && i < 7125) {
            str = this.mContext.getResources().getString(C1992R$string.wifi_band_6ghz);
    ...
    private void refreshTxSpeed() {
        ...
        this.mTxLinkSpeedPref.setSummary((CharSequence) this.mContext.getString(C1992R$string.tx_link_speed, new Object[]{Integer.valueOf(this.mWifiInfo.getTxLinkSpeedMbps())}));
    }
    ...
    private void refreshRxSpeed() {
        ...
        this.mRxLinkSpeedPref.setSummary((CharSequence) this.mContext.getString(C1992R$string.rx_link_speed, new Object[]{Integer.valueOf(this.mWifiInfo.getRxLinkSpeedMbps())}));
    }
    ...
    private void refreshEapSimSubscription() {
        ...
        if (this.mWifiEntry.getSecurity() == 3 && (wifiConfiguration = this.mWifiEntry.getWifiConfiguration()) != null && (wifiEnterpriseConfig = wifiConfiguration.enterpriseConfig) != null && wifiEnterpriseConfig.isAuthenticationSimBased()) {
            ...
            if (wifiConfiguration.carrierId == -1) {
                this.mEapSimSubscriptionPref.setSummary(C1992R$string.wifi_no_related_sim_card);
                return;
            }
    ...
    private void refreshMacAddress() {
        ...
        if (macAddress.equals("02:00:00:00:00:00")) {
            this.mMacAddressPref.setSummary(C1992R$string.device_info_not_available);
        } else {
    ...
    private int getWifiStandardTypeString(int i) {
        ...
        if (i == 4) {
            return C1992R$string.wifi_type_11N;
        }
        if (i == 5) {
            return C1992R$string.wifi_type_11AC;
        }
        if (i != 6) {
            return -1;
        }
        return C1992R$string.wifi_type_11AX;
    }

    private int getMacAddressTitle() {
        if (this.mWifiEntry.getPrivacy() != 1) {
            return C1992R$string.wifi_advanced_device_mac_address_title;
        }
        if (this.mWifiEntry.getConnectedState() == 2) {
            return C1992R$string.wifi_advanced_randomized_mac_address_title;
        }
        return C1992R$string.wifi_advanced_randomized_mac_address_disconnected_title;
    }
    ...
    private int getConnectDisconnectButtonTextResource() {
        ...
        if (connectedState == 0) {
            return C1992R$string.wifi_connect;
        }
        if (connectedState == 1) {
            return C1992R$string.wifi_connecting;
        }
        if (connectedState == 2) {
            return C1992R$string.wifi_disconnect_button_text;
        }
    ...
    private int getConnectDisconnectButtonIconResource() {
        ...
        if (connectedState == 0 || connectedState == 1) {
            return C1983R$drawable.ic_settings_wireless;
        }
        if (connectedState == 2) {
            return C1983R$drawable.ic_settings_close;
        }
        throw new IllegalStateException("Invalid WifiEntry connected state");
    }
    ...
    public void showConfirmForgetDialog() {
        new AlertDialog.Builder(this.mContext).setPositiveButton(C1992R$string.forget, new WifiDetailPreferenceController2$$ExternalSyntheticLambda0(this)).setNegativeButton(C1992R$string.cancel, (DialogInterface.OnClickListener) null).setTitle(C1992R$string.wifi_forget_dialog_title).setMessage(C1992R$string.forget_passpoint_dialog_message).create().show();
    }
    ...
    public void onConnectResult(int i) {
        if (i == 0) {
            Context context = this.mContext;
            Toast.makeText(context, context.getString(C1992R$string.wifi_connected_to_message, new Object[]{this.mWifiEntry.getTitle()}), 0).show();
        } else if (this.mWifiEntry.getLevel() == -1) {
            Toast.makeText(this.mContext, C1992R$string.wifi_not_in_range_message, 0).show();
        } else {
            Toast.makeText(this.mContext, C1992R$string.wifi_failed_connect_message, 0).show();
        }
    }
    ...
    public void onDisconnectResult(int i) {
        if (i == 0) {
            FragmentActivity activity = this.mFragment.getActivity();
            if (activity != null) {
                Toast.makeText(activity, activity.getString(C1992R$string.wifi_disconnected_from, new Object[]{this.mWifiEntry.getTitle()}), 0).show();
                return;
            }
            return;
        }
