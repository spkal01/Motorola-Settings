...
public class SummaryPreference extends Preference {
    ...
    public SummaryPreference(Context context, AttributeSet attributeSet) {
        ...
        setLayoutResource(C1987R$layout.settings_summary_preference);
    }

    public void setAmount(String str) {
        this.mAmount = str;
        if (str != null && this.mUnits != null) {
            setTitle(TextUtils.expandTemplate(getContext().getText(C1992R$string.storage_size_large), new CharSequence[]{this.mAmount, this.mUnits}));
        }
    }
    ...
    public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
        ...
        ProgressBar progressBar = (ProgressBar) preferenceViewHolder.itemView.findViewById(C1985R$id.color_bar);
        ...
        if (!this.mChartEnabled || (TextUtils.isEmpty(this.mStartLabel) && TextUtils.isEmpty(this.mEndLabel))) {
            preferenceViewHolder.findViewById(C1985R$id.label_bar).setVisibility(8);
            return;
        }
        preferenceViewHolder.findViewById(C1985R$id.label_bar).setVisibility(0);
