...
public class BatterySaverScheduleSettings extends RadioButtonPickerFragment {
    ...
    public int getPreferenceScreenResId() {
        return C1994R$xml.battery_saver_schedule_settings;
    }
    ...
    public List<? extends CandidateInfo> getCandidates() {
        ...
        String string = getContext().getResources().getString(17039871);
        newArrayList.add(new BatterySaverScheduleCandidateInfo(context.getText(C1992R$string.battery_saver_auto_no_schedule), (CharSequence) null, "key_battery_saver_no_schedule", true));
        if (!TextUtils.isEmpty(string)) {
            newArrayList.add(new BatterySaverScheduleCandidateInfo(context.getText(C1992R$string.battery_saver_auto_routine), context.getText(C1992R$string.battery_saver_auto_routine_summary), "key_battery_saver_routine", true));
        } else {
            BatterySaverUtils.revertScheduleToNoneIfNeeded(context);
        }
        newArrayList.add(new BatterySaverScheduleCandidateInfo(context.getText(C1992R$string.battery_saver_auto_percentage), (CharSequence) null, "key_battery_saver_percentage", true));
        return newArrayList;
    }
