...
public class BatteryTipDialogFragment extends InstrumentedDialogFragment implements DialogInterface.OnClickListener {
    ...
    public Dialog onCreateDialog(Bundle bundle) {
        ...
        if (type == 1) {
            ...
            AlertDialog.Builder negativeButton = new AlertDialog.Builder(context).setTitle((CharSequence) context.getResources().getQuantityString(C1990R$plurals.battery_tip_restrict_app_dialog_title, size, new Object[]{Integer.valueOf(size)})).setPositiveButton(C1992R$string.battery_tip_restrict_app_dialog_ok, (DialogInterface.OnClickListener) this).setNegativeButton(17039360, (DialogInterface.OnClickListener) null);
            if (size == 1) {
                negativeButton.setMessage((CharSequence) getString(C1992R$string.battery_tip_restrict_app_dialog_message, applicationLabel));
            } else if (size <= 5) {
                negativeButton.setMessage((CharSequence) getString(C1992R$string.battery_tip_restrict_apps_less_than_5_dialog_message));
                RecyclerView recyclerView = (RecyclerView) LayoutInflater.from(context).inflate(C1987R$layout.recycler_view, (ViewGroup) null);
                recyclerView.setLayoutManager(new LinearLayoutManager(context));
                recyclerView.setAdapter(new HighUsageAdapter(context, restrictAppList));
                negativeButton.setView((View) recyclerView);
            } else {
                negativeButton.setMessage((CharSequence) context.getString(C1992R$string.battery_tip_restrict_apps_more_than_5_dialog_message, new Object[]{restrictAppTip.getRestrictAppsString(context)}));
            }
            return negativeButton.create();
        } else if (type == 2) {
            HighUsageTip highUsageTip = (HighUsageTip) this.mBatteryTip;
            RecyclerView recyclerView2 = (RecyclerView) LayoutInflater.from(context).inflate(C1987R$layout.recycler_view, (ViewGroup) null);
            recyclerView2.setLayoutManager(new LinearLayoutManager(context));
            recyclerView2.setAdapter(new HighUsageAdapter(context, highUsageTip.getHighUsageAppList()));
            return new AlertDialog.Builder(context).setMessage((CharSequence) getString(C1992R$string.battery_tip_dialog_message, Integer.valueOf(highUsageTip.getHighUsageAppList().size()))).setView((View) recyclerView2).setPositiveButton(17039370, (DialogInterface.OnClickListener) null).create();
        } else if (type == 6) {
            return new AlertDialog.Builder(context).setMessage(C1992R$string.battery_tip_dialog_summary_message).setPositiveButton(17039370, (DialogInterface.OnClickListener) null).create();
        } else {
            if (type == 7) {
                Utils.getApplicationLabel(context, ((UnrestrictAppTip) this.mBatteryTip).getPackageName());
                return new AlertDialog.Builder(context).setTitle((CharSequence) getString(C1992R$string.battery_tip_unrestrict_app_dialog_title)).setMessage(C1992R$string.battery_tip_unrestrict_app_dialog_message).setPositiveButton(C1992R$string.battery_tip_unrestrict_app_dialog_ok, (DialogInterface.OnClickListener) this).setNegativeButton(C1992R$string.battery_tip_unrestrict_app_dialog_cancel, (DialogInterface.OnClickListener) null).create();
            }
