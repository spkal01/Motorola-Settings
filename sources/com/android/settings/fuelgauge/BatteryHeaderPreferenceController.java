...
public class BatteryHeaderPreferenceController extends BasePreferenceController implements PreferenceControllerMixin, LifecycleObserver, OnStart, BatteryPreferenceController {
    ...
    public void displayPreference(PreferenceScreen preferenceScreen) {
        ...
        usageProgressBarPreference.setBottomSummary(this.mContext.getString(C1992R$string.settings_license_activity_loading));
    ...
    private CharSequence generateLabel(BatteryInfo batteryInfo) {
        ...
        if (str != null && !batteryInfo.discharging) {
            return this.mContext.getString(C1992R$string.battery_state_and_duration, new Object[]{str, charSequence});
        } else if (this.mPowerManager.isPowerSaveMode()) {
            String string = this.mContext.getString(C1992R$string.battery_tip_early_heads_up_done_title);
            return this.mContext.getString(C1992R$string.battery_state_and_duration, new Object[]{string, batteryInfo.remainingLabel});
        } else {
            BatteryTip batteryTip = this.mBatteryTip;
            if (batteryTip == null || batteryTip.getType() != 5) {
                return batteryInfo.remainingLabel;
            }
            String string2 = this.mContext.getString(C1992R$string.low_battery_summary);
            return this.mContext.getString(C1992R$string.battery_state_and_duration, new Object[]{string2, batteryInfo.remainingLabel});
        }
    }
    ...
    private CharSequence formatBatteryPercentageText(int i) {
        return TextUtils.expandTemplate(this.mContext.getText(C1992R$string.battery_header_title_alternate), new CharSequence[]{NumberFormat.getIntegerInstance().format((long) i)});
    }
}
