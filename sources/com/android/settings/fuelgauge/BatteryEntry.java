...
public class BatteryEntry {
    ...
    public BatteryEntry(Context context, int i, String str, double d, double d2) {
        ...
        this.mIsHidden = false;
        this.mPowerComponentId = i;
        int i2 = C1983R$drawable.ic_power_system;
        this.iconId = i2;
        this.icon = context.getDrawable(i2);
        this.name = str;
        this.mConsumedPower = i != 0 ? d - d2 : d;
    ...
    public static NameAndIcon getNameAndIconFromUserId(Context context, int i) {
        ...
        if (userInfo != null) {
            ...
            str = context.getResources().getString(C1992R$string.running_process_item_removed_user_label);
            drawable = null;
        }
    ...
    public static NameAndIcon getNameAndIconFromUid(Context context, String str, int i) {
        Drawable drawable = context.getDrawable(C1983R$drawable.ic_power_system);
        if (i == 0) {
            str = context.getResources().getString(C1992R$string.process_kernel_label);
        } else if (i == -5) {
            str = context.getResources().getString(Utils.getTetheringLabel((TetheringManager) context.getSystemService(TetheringManager.class)));
        } else if ("mediaserver".equals(str)) {
            str = context.getResources().getString(C1992R$string.process_mediaserver_label);
        } else if ("dex2oat".equals(str) || "dex2oat32".equals(str) || "dex2oat64".equals(str)) {
            str = context.getResources().getString(C1992R$string.process_dex2oat_label);
        }
        return new NameAndIcon(str, drawable, 0);
    }
    ...
    public static NameAndIcon getNameAndIconFromPowerComponent(Context context, int i) {
        ...
        String str;
        if (i == 0) {
            str = context.getResources().getString(C1992R$string.power_screen);
            i2 = C1983R$drawable.ic_settings_display;
        } else if (i == 6) {
            str = context.getResources().getString(C1992R$string.power_flashlight);
            i2 = C1983R$drawable.ic_settings_display;
        } else if (i == 8) {
            str = context.getResources().getString(C1992R$string.power_cell);
            i2 = C1983R$drawable.ic_cellular_1_bar;
        } else if (i == 11) {
            str = context.getResources().getString(C1992R$string.power_wifi);
            i2 = C1983R$drawable.ic_settings_wireless;
        } else if (i == 2) {
            str = context.getResources().getString(C1992R$string.power_bluetooth);
            i2 = 17303713;
        } else if (i != 3) {
            switch (i) {
                case 13:
                case 16:
                    str = context.getResources().getString(C1992R$string.power_idle);
                    i2 = C1983R$drawable.ic_settings_phone_idle;
                    break;
                case 14:
                    str = context.getResources().getString(C1992R$string.power_phone);
                    i2 = C1983R$drawable.ic_settings_voice_calls;
                    break;
                case 15:
                    str = context.getResources().getString(C1992R$string.ambient_display_screen_title);
                    i2 = C1983R$drawable.ic_settings_aod;
                    break;
                default:
                    str = DebugUtils.constantToString(BatteryConsumer.class, "POWER_COMPONENT_", i);
                    i2 = C1983R$drawable.ic_power_system;
                    break;
            }
        } else {
            str = context.getResources().getString(C1992R$string.power_camera);
            i2 = C1983R$drawable.ic_settings_camera;
        }
