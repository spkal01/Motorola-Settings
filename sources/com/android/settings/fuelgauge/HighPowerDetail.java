...
public class HighPowerDetail extends InstrumentedDialogFragment implements DialogInterface.OnClickListener, View.OnClickListener {
    ...
    public Checkable setup(View view, boolean z) {
        ((TextView) view.findViewById(16908310)).setText(z ? C1992R$string.ignore_optimizations_on : C1992R$string.ignore_optimizations_off);
        ((TextView) view.findViewById(16908304)).setText(z ? C1992R$string.ignore_optimizations_on_desc : C1992R$string.ignore_optimizations_off_desc);
    ...
    public Dialog onCreateDialog(Bundle bundle) {
        AlertDialog.Builder view = new AlertDialog.Builder(getContext()).setTitle(this.mLabel).setNegativeButton(C1992R$string.cancel, (DialogInterface.OnClickListener) null).setView(C1987R$layout.ignore_optimizations_content);
        if (!this.mBackend.isSysAllowlisted(this.mPackageName)) {
            view.setPositiveButton(C1992R$string.done, (DialogInterface.OnClickListener) this);
        }
        return view.create();
    }
    ...
    public void onStart() {
        super.onStart();
        this.mOptionOn = setup(getDialog().findViewById(C1985R$id.ignore_on), true);
        this.mOptionOff = setup(getDialog().findViewById(C1985R$id.ignore_off), false);
        updateViews();
    }
    ...
    static CharSequence getSummary(Context context, PowerAllowlistBackend powerAllowlistBackend, String str) {
        ...
        if (powerAllowlistBackend.isSysAllowlisted(str) || powerAllowlistBackend.isDefaultActiveApp(str)) {
            i = C1992R$string.high_power_system;
        } else if (powerAllowlistBackend.isAllowlisted(str)) {
            i = C1992R$string.high_power_on;
        } else {
            i = C1992R$string.high_power_off;
        }
