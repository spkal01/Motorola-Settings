...
public class TimeZoneInfoPreferenceController extends BasePreferenceController {
    ...
    private CharSequence formatOffsetAndName(TimeZoneInfo timeZoneInfo) {
        ...
        return SpannableUtil.getResourcesText(this.mContext.getResources(), C1992R$string.zone_info_offset_and_name, timeZoneInfo.getGmtOffset(), genericName);
    }
    ...
    private CharSequence formatInfo(TimeZoneInfo timeZoneInfo) {
        ...
        if (!timeZone.observesDaylightTime()) {
            return this.mContext.getString(C1992R$string.zone_info_footer_no_dst, new Object[]{formatOffsetAndName});
        }
        ...
        if (daylightName == null) {
            if (z) {
                daylightName = this.mContext.getString(C1992R$string.zone_time_type_dst);
            } else {
                daylightName = this.mContext.getString(C1992R$string.zone_time_type_standard);
            }
        }
        Calendar instance = Calendar.getInstance(timeZone);
        instance.setTimeInMillis(findNextDstTransition.getTime());
        return SpannableUtil.getResourcesText(this.mContext.getResources(), C1992R$string.zone_info_footer, formatOffsetAndName, daylightName, this.mDateFormat.format(instance));
    }
