...
public class ChangeScreenLockPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, GearPreference.OnGearClickListener {
    ...
    public boolean isAvailable() {
        return this.mContext.getResources().getBoolean(C1980R$bool.config_show_unlock_set_or_change);
    }
    ...
    public void updateSummary(Preference preference, int i) {
        if (this.mLockPatternUtils.isSecure(i)) {
            ...
            if (keyguardStoredPasswordQuality == 65536) {
                preference.setSummary(C1992R$string.unlock_set_unlock_mode_pattern);
            } else if (keyguardStoredPasswordQuality == 131072 || keyguardStoredPasswordQuality == 196608) {
                if (!Build.IS_PRC_PRODUCT) {
                    preference.setSummary(C1992R$string.unlock_set_unlock_mode_pin);
                } else if (this.mLockPatternUtils.isSixBitForPinLock(i)) {
                    preference.setSummary(C1992R$string.security_password_pin_six_bit);
                } else {
                    preference.setSummary(C1992R$string.security_password_pin_custom);
                }
                ...
                preference.setSummary(C1992R$string.unlock_set_unlock_mode_password);
            }
        } else if (i == this.mProfileChallengeUserId || this.mLockPatternUtils.isLockScreenDisabled(i)) {
            preference.setSummary(C1992R$string.unlock_set_unlock_mode_off);
        } else {
            preference.setSummary(C1992R$string.unlock_set_unlock_mode_none);
        }
