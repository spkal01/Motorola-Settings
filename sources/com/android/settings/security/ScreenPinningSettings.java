...
public class ScreenPinningSettings extends SettingsPreferenceFragment implements OnMainSwitchChangeListener, DialogInterface.OnClickListener {
    public static final BaseSearchIndexProvider SEARCH_INDEX_DATA_PROVIDER = new BaseSearchIndexProvider() {
        public List<SearchIndexableResource> getXmlResourcesToIndex(Context context, boolean z) {
            ...
            searchIndexableResource.xmlResId = C1994R$xml.screen_pinning_settings;
    ...
    public void onActivityCreated(Bundle bundle) {
        ...
        settingsActivity.setTitle(C1992R$string.screen_pinning_title);
        ...
        addPreferencesFromResource(C1994R$xml.screen_pinning_settings);
        ...
        switchBar.setTitle(getContext().getString(C1992R$string.app_pinning_main_switch_title));
    ...
    public int getHelpResource() {
        return C1992R$string.help_url_screen_pinning;
    }
    ...
    private int getCurrentSecurityTitle() {
        ...
        if (keyguardStoredPasswordQuality != 65536) {
            if (keyguardStoredPasswordQuality == 131072 || keyguardStoredPasswordQuality == 196608) {
                return C1992R$string.screen_pinning_unlock_pin;
            }
            if (keyguardStoredPasswordQuality == 262144 || keyguardStoredPasswordQuality == 327680 || keyguardStoredPasswordQuality == 393216 || keyguardStoredPasswordQuality == 524288) {
                return C1992R$string.screen_pinning_unlock_password;
            }
        } else if (this.mLockPatternUtils.isLockPatternEnabled(UserHandle.myUserId())) {
            return C1992R$string.screen_pinning_unlock_pattern;
        }
        return C1992R$string.screen_pinning_unlock_none;
    }

    public void onSwitchChanged(Switch switchR, boolean z) {
        if (z) {
            new AlertDialog.Builder(getContext()).setMessage(C1992R$string.screen_pinning_dialog_message).setPositiveButton(C1992R$string.dlg_ok, (DialogInterface.OnClickListener) this).setNegativeButton(C1992R$string.dlg_cancel, (DialogInterface.OnClickListener) this).setCancelable(false).show();
            return;
        }
    ...
    private CharSequence getAppPinningContent() {
        if (isGuestModeSupported()) {
            return getActivity().getText(C1992R$string.screen_pinning_guest_user_description);
        }
        return getActivity().getText(C1992R$string.screen_pinning_description);
    }
}
