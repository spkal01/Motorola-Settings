...
public class CryptKeeperSettings extends InstrumentedPreferenceFragment {
    ...
    private View.OnClickListener mInitiateListener = new View.OnClickListener() {
        public void onClick(View view) {
            if (!CryptKeeperSettings.this.runKeyguardConfirmation(55)) {
                new AlertDialog.Builder(CryptKeeperSettings.this.getActivity()).setTitle(C1992R$string.crypt_keeper_dialog_need_password_title).setMessage(C1992R$string.crypt_keeper_dialog_need_password_message).setPositiveButton(17039370, (DialogInterface.OnClickListener) null).create().show();
            }
        }
    };
    ...
    public View onCreateView(LayoutInflater layoutInflater, ViewGroup viewGroup, Bundle bundle) {
        this.mContentView = layoutInflater.inflate(C1987R$layout.crypt_keeper_settings, (ViewGroup) null);
        IntentFilter intentFilter = new IntentFilter();
        this.mIntentFilter = intentFilter;
        intentFilter.addAction("android.intent.action.BATTERY_CHANGED");
        Button button = (Button) this.mContentView.findViewById(C1985R$id.initiate_encrypt);
        this.mInitiateButton = button;
        button.setOnClickListener(this.mInitiateListener);
        this.mInitiateButton.setEnabled(false);
        this.mPowerWarning = this.mContentView.findViewById(C1985R$id.warning_unplugged);
        this.mBatteryWarning = this.mContentView.findViewById(C1985R$id.warning_low_charge);
    ...
    public void onActivityCreated(Bundle bundle) {
        ...
        activity.setTitle(C1992R$string.crypt_keeper_encrypt_title);
    }
    ...
    public boolean runKeyguardConfirmation(int i) {
        ...
        return new ChooseLockSettingsHelper.Builder(getActivity(), this).setRequestCode(i).setTitle(getActivity().getResources().getText(C1992R$string.crypt_keeper_encrypt_title)).setReturnCredentials(true).show();
    }
    ...
    private void showFinalConfirmation(int i, byte[] bArr) {
        ...
        preference.setTitle(C1992R$string.crypt_keeper_confirm_title);
