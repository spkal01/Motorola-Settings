...
public class EncryptionStatusPreferenceController extends BasePreferenceController {
    ...
    public int getAvailabilityStatus() {
        if (!TextUtils.equals(getPreferenceKey(), PREF_KEY_ENCRYPTION_DETAIL_PAGE) || this.mContext.getResources().getBoolean(C1980R$bool.config_show_encryption_and_credentials_encryption_status)) {
    ...
    public void updateState(Preference preference) {
        if (LockPatternUtils.isDeviceEncryptionEnabled()) {
            ...
            preference.setSummary(C1992R$string.crypt_keeper_encrypted_summary);
            return;
        }
        if (TextUtils.equals(getPreferenceKey(), PREF_KEY_ENCRYPTION_DETAIL_PAGE)) {
            preference.setFragment(CryptKeeperSettings.class.getName());
        }
        preference.setSummary(C1992R$string.decryption_settings_summary);
    }
}
