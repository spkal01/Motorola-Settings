...
public class EmergencyGestureEntrypointPreferenceController extends BasePreferenceController {
    ...
    public EmergencyGestureEntrypointPreferenceController(Context context, String str) {
        super(context, str);
        String string = context.getResources().getString(C1992R$string.emergency_gesture_settings_package);
    ...
    public int getAvailabilityStatus() {
        if (this.mContext.getResources().getBoolean(C1980R$bool.config_show_emergency_gesture_settings) && canHandleClicks()) {
            return 0;
        }
        return 3;
    }
    ...
    public CharSequence getSummary() {
        ...
        String string = this.mContext.getResources().getString(C1992R$string.emergency_gesture_settings_package);
        try {
            PackageManager packageManager = this.mContext.getPackageManager();
            ApplicationInfo applicationInfo = packageManager.getApplicationInfo(string, 33280);
            return this.mContext.getString(C1992R$string.emergency_gesture_entrypoint_summary, new Object[]{applicationInfo.loadLabel(packageManager)});
