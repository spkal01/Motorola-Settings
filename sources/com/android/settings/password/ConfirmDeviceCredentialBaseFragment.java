...
public abstract class ConfirmDeviceCredentialBaseFragment extends InstrumentedFragment {
    ...
    protected boolean mIsPrivacyUser = false;
    ...
    public void onCreate(Bundle bundle) {
        ...
        int privacySpaceUserId = userManager.getPrivacySpaceUserId();
    ...
    public void onViewCreated(View view, Bundle bundle) {
        ...
        this.mCancelButton = (Button) view.findViewById(C1985R$id.cancelButton);
        int i = 0;
    ...
    private void setupForgotButtonIfManagedProfile(View view) {
        if (this.mUserManager.isManagedProfile(this.mUserId) && this.mUserManager.isQuietModeEnabled(UserHandle.of(this.mUserId)) && this.mDevicePolicyManager.canProfileOwnerResetPasswordWhenLocked(this.mUserId)) {
            Button button = (Button) view.findViewById(C1985R$id.forgotButton);
    ...
    public void updateErrorMessage(int i) {
        ...
        if (maximumFailedPasswordsForWipe > 0 && i > 0) {
            if (this.mErrorTextView != null) {
                showError((CharSequence) getActivity().getString(C1992R$string.lock_failed_attempts_before_wipe, new Object[]{Integer.valueOf(i), Integer.valueOf(maximumFailedPasswordsForWipe)}), 0);
            }
            ...
            if (i2 <= 1) {
                ...
                if (i2 == 1) {
                    LastTryDialog.show(childFragmentManager, getActivity().getString(C1992R$string.lock_last_attempt_before_wipe_warning_title), getLastTryErrorMessage(userTypeForWipe), 17039370, false);
                } else {
                    LastTryDialog.show(childFragmentManager, (String) null, getWipeMessage(userTypeForWipe), C1992R$string.lock_failed_attempts_now_wiping_dialog_dismiss, true);
                }
            }
        }
    }
    ...
    private int getWipeMessage(int i) {
        if (i == 1) {
            return C1992R$string.lock_failed_attempts_now_wiping_device;
        }
        if (i == 2) {
            return C1992R$string.lock_failed_attempts_now_wiping_profile;
        }
        if (i == 3) {
            return C1992R$string.lock_failed_attempts_now_wiping_user;
        }
