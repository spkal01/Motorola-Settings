...
public class GrayscaleConditionController implements ConditionalCardController {
    ...
    /* renamed from: ID */
    static final int f111ID = Objects.hash(new Object[]{"GrayscaleConditionController"});
    ...
    public boolean isDisplayable() {
        try {
            this.mIntent = Intent.parseUri(this.mAppContext.getString(C1992R$string.config_grayscale_settings_intent), 1);
    ...
    public ContextualCard buildContextualCard() {
        ConditionalContextualCard.Builder actionText = new ConditionalContextualCard.Builder().setConditionId((long) f111ID).setMetricsConstant(1683).setActionText(this.mAppContext.getText(C1992R$string.condition_turn_off));
        StringBuilder sb = new StringBuilder();
        sb.append(this.mAppContext.getPackageName());
        sb.append("/");
        Context context = this.mAppContext;
        int i = C1992R$string.condition_grayscale_title;
        sb.append(context.getText(i));
        return actionText.setName(sb.toString()).setTitleText(this.mAppContext.getText(i).toString()).setSummaryText(this.mAppContext.getText(C1992R$string.condition_grayscale_summary).toString()).setIconDrawable(this.mAppContext.getDrawable(C1983R$drawable.ic_gray_scale_24dp)).setViewType(ConditionContextualCardRenderer.VIEW_TYPE_HALF_WIDTH).build();
    }
