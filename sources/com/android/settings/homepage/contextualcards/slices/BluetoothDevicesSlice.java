...
public class BluetoothDevicesSlice implements CustomSliceable {
    ...
    public Intent getIntent() {
        return SliceBuilderUtils.buildSearchResultPageIntent(this.mContext, ConnectedDeviceDashboardFragment.class.getName(), "", this.mContext.getText(C1992R$string.connected_devices_dashboard_title).toString(), 1401).setClassName(this.mContext.getPackageName(), SubSettings.class.getName()).setData(getUri());
    }
    ...
    public PendingIntent getBluetoothDetailIntent(CachedBluetoothDevice cachedBluetoothDevice) {
        ...
        subSettingLauncher.setDestination(BluetoothDeviceDetailsFragment.class.getName()).setArguments(bundle).setTitleRes(C1992R$string.device_details_title).setSourceMetricsCategory(1009);
    ...
    public IconCompat getBluetoothDeviceIcon(CachedBluetoothDevice cachedBluetoothDevice) {
        ...
        if (drawable == null) {
            return IconCompat.createWithResource(this.mContext, 17303713);
        }
        return Utils.createIconWithDrawable(drawable);
    }

    private ListBuilder.RowBuilder getBluetoothOffHeader() {
        Drawable drawable = this.mContext.getDrawable(C1983R$drawable.ic_bluetooth_disabled);
        ...
        CharSequence text = this.mContext.getText(C1992R$string.bluetooth_devices_card_off_title);
        CharSequence text2 = this.mContext.getText(C1992R$string.bluetooth_devices_card_off_summary);
    ...
    private ListBuilder.RowBuilder getBluetoothOnHeader() {
        Drawable drawable = this.mContext.getDrawable(17303713);
        drawable.setTint(com.android.settingslib.Utils.getColorAccentDefaultColor(this.mContext));
        IconCompat createIconWithDrawable = Utils.createIconWithDrawable(drawable);
        CharSequence text = this.mContext.getText(C1992R$string.bluetooth_devices);
    ...
    private SliceAction getPairNewDeviceAction() {
        Drawable drawable = this.mContext.getDrawable(C1983R$drawable.ic_add_24dp);
        drawable.setTint(com.android.settingslib.Utils.getColorAccentDefaultColor(this.mContext));
        IconCompat createIconWithDrawable = Utils.createIconWithDrawable(drawable);
        String string = this.mContext.getString(C1992R$string.bluetooth_pairing_pref_title);
        Intent intent = new SubSettingLauncher(this.mContext).setDestination(BluetoothPairingDetail.class.getName()).setTitleRes(C1992R$string.bluetooth_pairing_page_title).setSourceMetricsCategory(1018).toIntent();
    ...
    private List<ListBuilder.RowBuilder> getBluetoothRowBuilders() {
        ...
        for (CachedBluetoothDevice next : pairedBluetoothDevices) {
            ...
            if (connectionSummary == null) {
                connectionSummary = this.mContext.getString(C1992R$string.connected_device_previously_connected_screen_title);
            }
    ...
    public SliceAction buildBluetoothDetailDeepLinkAction(CachedBluetoothDevice cachedBluetoothDevice) {
        return SliceAction.createDeeplink(getBluetoothDetailIntent(cachedBluetoothDevice), IconCompat.createWithResource(this.mContext, C1983R$drawable.ic_settings_accent), 0, cachedBluetoothDevice.getName());
    }
