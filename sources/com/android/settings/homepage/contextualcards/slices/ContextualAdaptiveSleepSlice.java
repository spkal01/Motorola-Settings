...
public class ContextualAdaptiveSleepSlice implements CustomSliceable {
    ...
    public Slice getSlice() {
        if (this.mContext.getSharedPreferences("adaptive_sleep_slice", 0).getLong(PREF_KEY_SETUP_TIME, DEFERRED_TIME_DAYS) == DEFERRED_TIME_DAYS) {
            ...
            IconCompat createWithResource = IconCompat.createWithResource(this.mContext, C1983R$drawable.ic_settings_adaptive_sleep);
            CharSequence text = this.mContext.getText(C1992R$string.adaptive_sleep_contextual_slice_title);
            CharSequence text2 = this.mContext.getText(C1992R$string.adaptive_sleep_contextual_slice_summary);
    ...
    public Intent getIntent() {
        CharSequence text = this.mContext.getText(C1992R$string.adaptive_sleep_title);
