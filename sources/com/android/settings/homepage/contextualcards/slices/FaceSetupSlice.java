...
public class FaceSetupSlice implements CustomSliceable {
    ...
    public Slice getSlice() {
        ...
        if (!hasEnrolledTemplates) {
            charSequence2 = this.mContext.getText(C1992R$string.security_settings_face_settings_enroll);
            charSequence = this.mContext.getText(C1992R$string.security_settings_face_settings_context_subtitle);
        } else if (reEnrollSetting == 1) {
            charSequence2 = this.mContext.getText(C1992R$string.security_settings_face_enroll_should_re_enroll_title);
            charSequence = this.mContext.getText(C1992R$string.security_settings_face_enroll_should_re_enroll_subtitle);
        } else if (reEnrollSetting != 3) {
            return new ListBuilder(this.mContext, CustomSliceRegistry.FACE_ENROLL_SLICE_URI, -1).setIsError(true).build();
        } else {
            charSequence2 = this.mContext.getText(C1992R$string.security_settings_face_enroll_must_re_enroll_title);
            charSequence = this.mContext.getText(C1992R$string.security_settings_face_enroll_must_re_enroll_subtitle);
        }
        return new ListBuilder(this.mContext, CustomSliceRegistry.FACE_ENROLL_SLICE_URI, -1).setAccentColor(com.android.settingslib.Utils.getColorAccentDefaultColor(this.mContext)).addRow(buildRowBuilder(charSequence2, charSequence, IconCompat.createWithResource(this.mContext, C1983R$drawable.ic_face_24dp), this.mContext, getIntent())).build();
    }
    ...
    public Intent getIntent() {
        if (!this.mFaceManager.hasEnrolledTemplates(UserHandle.myUserId())) {
            return SliceBuilderUtils.buildSearchResultPageIntent(this.mContext, SecuritySettings.class.getName(), FaceStatusPreferenceController.KEY_FACE_SETTINGS, this.mContext.getText(C1992R$string.security_settings_face_settings_enroll).toString(), 1401).setClassName(this.mContext.getPackageName(), SubSettings.class.getName());
        }
