...
public class LowStorageSlice implements CustomSliceable {
    ...
    public Slice getSlice() {
        ...
        IconCompat createWithResource = IconCompat.createWithResource(this.mContext, C1983R$drawable.ic_storage);
        if (d < 0.85d) {
            return accentColor.addRow(buildRowBuilder(this.mContext.getText(C1992R$string.storage_settings), this.mContext.getString(C1992R$string.storage_summary, new Object[]{format, formatFileSize}), createWithResource)).setIsError(true).build();
        }
        return accentColor.addRow(buildRowBuilder(this.mContext.getText(C1992R$string.storage_menu_free), this.mContext.getString(C1992R$string.low_storage_summary, new Object[]{format, formatFileSize}), createWithResource)).build();
    }
    ...
    public Intent getIntent() {
        return SliceBuilderUtils.buildSearchResultPageIntent(this.mContext, StorageDashboardFragment.class.getName(), "", this.mContext.getText(C1992R$string.storage_label).toString(), 1401).setClassName(this.mContext.getPackageName(), SubSettings.class.getName()).setData(CustomSliceRegistry.LOW_STORAGE_SLICE_URI);
    }
