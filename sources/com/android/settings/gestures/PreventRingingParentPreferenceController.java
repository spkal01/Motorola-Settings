...
public class PreventRingingParentPreferenceController extends TogglePreferenceController implements LifecycleObserver, OnStart, OnStop {
    ...
    public void updateState(Preference preference) {
        ...
        if (isVolumePowerKeyChordSetToHush()) {
            if (i == 1) {
                charSequence = this.mContext.getText(C1992R$string.prevent_ringing_option_vibrate_summary);
            } else if (i != 2) {
                charSequence = this.mContext.getText(C1992R$string.switch_off_text);
            } else {
                charSequence = this.mContext.getText(C1992R$string.prevent_ringing_option_mute_summary);
            }
            preference.setEnabled(true);
            this.mPreference.setSwitchEnabled(true);
        } else {
            charSequence = this.mContext.getText(C1992R$string.prevent_ringing_option_unavailable_lpp_summary);
    ...
    public int getAvailabilityStatus() {
        ...
        if (this.mContext.getResources().getBoolean(17891646)) {
            return 5;
        }
        return 3;
    }
    ...
    private boolean isVolumePowerKeyChordSetToHush() {
        return Settings.Global.getInt(this.mContext.getContentResolver(), "key_chord_power_volume_up", this.mContext.getResources().getInteger(17694847)) == 1;
    }
