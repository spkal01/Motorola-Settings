...
public class SystemNavigationPreferenceController extends BasePreferenceController {
    ...
    public CharSequence getSummary() {
        if (isGestureNavigationEnabled(this.mContext)) {
            return this.mContext.getText(C1992R$string.edge_to_edge_navigation_title);
        }
        if (is2ButtonNavigationEnabled(this.mContext)) {
            return this.mContext.getText(C1992R$string.swipe_up_to_switch_apps_title);
        }
        return this.mContext.getText(C1992R$string.legacy_navigation_title);
    }

    static boolean isGestureAvailable(Context context) {
        ComponentName unflattenFromString;
        if (!context.getResources().getBoolean(17891759) || (unflattenFromString = ComponentName.unflattenFromString(context.getString(17040011))) == null) {
            return false;
        }
    ...
    static boolean is2ButtonNavigationEnabled(Context context) {
        return 1 == context.getResources().getInteger(17694885);
    }

    static boolean isGestureNavigationEnabled(Context context) {
        return 2 == context.getResources().getInteger(17694885);
    }
}
