...
public class AmbientDisplayAlwaysOnPreferenceController extends TogglePreferenceController {
    ...
    public CharSequence getSummary() {
        ...
        if (isAodSuppressedByBedtime(context)) {
            i = C1992R$string.aware_summary_when_bedtime_on;
        } else {
            i = C1992R$string.doze_always_on_summary;
        }
    ...
    public static boolean isAodSuppressedByBedtime(Context context) {
        try {
            return ((PowerManager) context.getSystemService(PowerManager.class)).isAmbientDisplaySuppressedForTokenByApp(AOD_SUPPRESSED_TOKEN, context.getPackageManager().getApplicationInfo(context.getString(17039931), 0).uid);
