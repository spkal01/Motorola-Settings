    ...
    public String getAutoModeSummary(Context context, ColorDisplayManager colorDisplayManager) {
        ...
        if (nightDisplayAutoMode == 1) {
            if (isNightDisplayActivated) {
                return context.getString(C1992R$string.night_display_summary_on_auto_mode_custom, new Object[]{getFormattedTimeString(colorDisplayManager.getNightDisplayCustomEndTime())});
            }
            return context.getString(C1992R$string.night_display_summary_off_auto_mode_custom, new Object[]{getFormattedTimeString(colorDisplayManager.getNightDisplayCustomStartTime())});
        } else if (nightDisplayAutoMode == 2) {
            if (isNightDisplayActivated) {
                i2 = C1992R$string.night_display_summary_on_auto_mode_twilight;
            } else {
                i2 = C1992R$string.night_display_summary_off_auto_mode_twilight;
            }
            return context.getString(i2);
        } else {
            if (isNightDisplayActivated) {
                i = C1992R$string.night_display_summary_on_auto_mode_never;
            } else {
                i = C1992R$string.night_display_summary_off_auto_mode_never;
            }
