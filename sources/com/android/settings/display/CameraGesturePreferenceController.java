...
public class CameraGesturePreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, Preference.OnPreferenceChangeListener {
    ...
    public boolean isAvailable() {
        if (this.mContext.getPackageManager().hasSystemFeature("com.motorola.actions") || this.mContext.getResources().getBoolean(C1980R$bool.config_hide_camera_dbl_twist_in_display_settings)) {
            return false;
        }
        if (!(this.mContext.getResources().getInteger(17694761) != -1) || SystemProperties.getBoolean("gesture.disable_camera_launch", false)) {
            return false;
        }
        return true;
    }
