...
public class DensityPreference extends CustomEditTextPreferenceCompat {
    ...
    public void onAttached() {
        ...
        setSummary((CharSequence) getContext().getString(C1992R$string.density_pixel_summary, new Object[]{unicodeWrap}));
    }
    ...
    public void onDialogClosed(boolean z) {
        if (z) {
            try {
                DisplayMetrics displayMetrics = getContext().getResources().getDisplayMetrics();
                DisplayDensityConfiguration.setForcedDisplayDensity(0, Math.max((Math.min(displayMetrics.widthPixels, displayMetrics.heightPixels) * 160) / Math.max(Integer.parseInt(getText()), 320), R$styleable.AppCompatTheme_windowFixedHeightMajor));
            } catch (Exception e) {
