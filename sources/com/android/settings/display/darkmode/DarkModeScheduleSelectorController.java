...
public class DarkModeScheduleSelectorController extends BasePreferenceController implements Preference.OnPreferenceChangeListener {
    ...
    private int getCurrentMode() {
        ...
        if (nightMode == 0) {
            i = C1992R$string.dark_ui_auto_mode_auto;
        } else if (nightMode != 3) {
            i = C1992R$string.dark_ui_auto_mode_never;
        } else {
            i = C1992R$string.dark_ui_auto_mode_custom;
        }
    ...
    public final boolean onPreferenceChange(Preference preference, Object obj) {
        ...
        if (findIndexOfValue == this.mPreference.findIndexOfValue(this.mContext.getString(C1992R$string.dark_ui_auto_mode_never))) {
            if ((this.mContext.getResources().getConfiguration().uiMode & 32) != 0) {
                z = true;
            }
            this.mUiModeManager.setNightMode(z ? 2 : 1);
        } else if (findIndexOfValue == this.mPreference.findIndexOfValue(this.mContext.getString(C1992R$string.dark_ui_auto_mode_auto))) {
            if (!this.mLocationManager.isLocationEnabled()) {
                TwilightLocationDialog.show(this.mContext);
                return true;
            }
            this.mUiModeManager.setNightMode(0);
        } else if (findIndexOfValue == this.mPreference.findIndexOfValue(this.mContext.getString(C1992R$string.dark_ui_auto_mode_custom))) {
