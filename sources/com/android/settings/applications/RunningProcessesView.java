...
public class RunningProcessesView extends FrameLayout implements AdapterView.OnItemClickListener, AbsListView.RecyclerListener, RunningState.OnRefreshUiListener {
    ...
    public static class ActiveItem {
        ...
        public void updateTime(Context context, StringBuilder sb) {
            ...
            if (textView != null) {
                ...
                if (z2) {
                    textView.setText(context.getResources().getText(C1992R$string.service_restarting));
                } else {
                    textView.setText("");
                }
            }
    ...
    public static class ViewHolder {
        ...
        public ViewHolder(View view) {
            ...
            this.size = (TextView) view.findViewById(C1985R$id.widget_summary1);
            this.uptime = (TextView) view.findViewById(C1985R$id.widget_summary2);
            view.setTag(this);
        }
        ...
        public ActiveItem bind(RunningState runningState, RunningState.BaseItem baseItem, StringBuilder sb) {
            ...
            synchronized (runningState.mLock) {
                ...
                if (baseItem.mBackground) {
                    this.description.setText(view.getContext().getText(C1992R$string.cached));
                } else {
    ...
    class ServiceListAdapter extends BaseAdapter {
        ...
        ServiceListAdapter(RunningState runningState) {
            ...
            this.mPm = RunningProcessesView.this.getContext().getPackageManager();
            refreshItems();
        }
        ...
        public void refreshItems() {
            ...
            ArrayList<RunningState.MergedItem> arrayList2;
            ...
            if (this.mOrigItems != arrayList) {
                ...
                this.mItems.clear();
                if (InstalledAppUtils.get(RunningProcessesView.this.getContext()).hasHiddenPackages()) {
                    synchronized (this.mState.mLock) {
                        arrayList2 = new ArrayList<>();
                        Iterator<RunningState.MergedItem> it = arrayList.iterator();
                        while (it.hasNext()) {
                            RunningState.MergedItem next = it.next();
                            PackageItemInfo packageItemInfo = next.mPackageInfo;
                            if (packageItemInfo == null && (processItem = next.mProcess) != null) {
                                processItem.ensureLabel(this.mPm);
                                packageItemInfo = next.mProcess.mPackageInfo;
                            }
                            if (!InstalledAppUtils.get(RunningProcessesView.this.getContext()).isHiddenPackage(packageItemInfo != null ? packageItemInfo.packageName : null)) {
                                arrayList2.add(next);
                            }
                        }
                    }
        ...
        public View newView(ViewGroup viewGroup) {
            View inflate = this.mInflater.inflate(C1987R$layout.running_processes_item, viewGroup, false);
    ...
    public void refreshUi(boolean z) {
        ...
        synchronized (this.mState.mLock) {
            ...
            if (z2 != z3) {
                ...
                if (z3) {
                    this.mForegroundProcessPrefix.setText(getResources().getText(C1992R$string.running_processes_header_used_prefix));
                    this.mAppsProcessPrefix.setText(getResources().getText(C1992R$string.running_processes_header_cached_prefix));
                } else {
                    this.mForegroundProcessPrefix.setText(getResources().getText(C1992R$string.running_processes_header_system_prefix));
                    this.mAppsProcessPrefix.setText(getResources().getText(C1992R$string.running_processes_header_apps_prefix));
                }
            }
            ...
            if (!(this.mCurTotalRam == totalSize && this.mCurHighRam == j3 && this.mCurMedRam == j && this.mCurLowRam == j2)) {
                ...
                int i = C1992R$string.running_processes_header_ram;
    ...
    private void startServiceDetailsActivity(RunningState.MergedItem mergedItem) {
        if (this.mOwner != null && mergedItem != null) {
            ...
            new SubSettingLauncher(getContext()).setDestination(RunningServiceDetails.class.getName()).setArguments(bundle).setTitleRes(C1992R$string.runningservicedetails_settings_title).setSourceMetricsCategory(this.mOwner.getMetricsCategory()).launch();
        }
    }
    ...
    public void doCreate() {
        ...
        layoutInflater.inflate(C1987R$layout.running_processes_view, this);
        ...
        View inflate = layoutInflater.inflate(C1987R$layout.running_processes_header, (ViewGroup) null);
        this.mHeader = inflate;
        this.mListView.addHeaderView(inflate, (Object) null, false);
        this.mColorBar = (ProgressBar) this.mHeader.findViewById(C1985R$id.color_bar);
        Context context = getContext();
        this.mColorBar.setProgressTintList(ColorStateList.valueOf(context.getColor(C1981R$color.running_processes_system_ram)));
        ...
        this.mColorBar.setProgressBackgroundTintList(ColorStateList.valueOf(context.getColor(C1981R$color.running_processes_free_ram)));
        this.mColorBar.setProgressBackgroundTintMode(PorterDuff.Mode.SRC);
        this.mBackgroundProcessPrefix = (TextView) this.mHeader.findViewById(C1985R$id.freeSizePrefix);
        this.mAppsProcessPrefix = (TextView) this.mHeader.findViewById(C1985R$id.appsSizePrefix);
        this.mForegroundProcessPrefix = (TextView) this.mHeader.findViewById(C1985R$id.systemSizePrefix);
        this.mBackgroundProcessText = (TextView) this.mHeader.findViewById(C1985R$id.freeSize);
        this.mAppsProcessText = (TextView) this.mHeader.findViewById(C1985R$id.appsSize);
        this.mForegroundProcessText = (TextView) this.mHeader.findViewById(C1985R$id.systemSize);
