...
public class AppPermissionsPreferenceController extends BasePreferenceController {
    ...
    public void updateSummary(List<CharSequence> list) {
        ...
        if (i >= 4) {
            ...
            if (list2.isEmpty()) {
                str = this.mContext.getString(C1992R$string.runtime_permissions_summary_no_permissions_granted);
            } else if (z) {
                str = this.mContext.getString(C1992R$string.app_permissions_summary_more, new Object[]{ListFormatter.getInstance().format(list2).toLowerCase()});
            } else {
                str = this.mContext.getString(C1992R$string.app_permissions_summary, new Object[]{ListFormatter.getInstance().format(list2).toLowerCase()});
            }
