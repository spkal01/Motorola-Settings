...
public abstract class AppStateBaseBridge implements ApplicationsState.Callbacks {
    public static final Comparator<ApplicationsState.AppEntry> COMPARATOR_APPS_RECENT = AppStateBaseBridge$$ExternalSyntheticLambda0.INSTANCE;
    public static final ApplicationsState.AppFilter FILTER_APPS_LIST_ALPHA = new ApplicationsState.AppFilter() {
    ...
    public static final ApplicationsState.AppFilter FILTER_APPS_LIST_RECENT = new ApplicationsState.AppFilter() {
        public boolean filterApp(ApplicationsState.AppEntry appEntry) {
            return true;
        }
    ...
    public static final ApplicationsState.AppFilter FILTER_APPS_LIST_REMOVE_CLONE_APPS = new ApplicationsState.AppFilter() {
        public void init() {
        }

        public boolean filterApp(ApplicationsState.AppEntry appEntry) {
            return !CloneAppUtils.isCloneAppByAppEntry(appEntry);
        }
    };
    protected static final Map<String, UsageStats> USAGE_STATS = new HashMap();
    ...
    /* access modifiers changed from: protected */
    public void buildUsageStatsComparator(Context context) {
        USAGE_STATS.clear();
        Calendar instance = Calendar.getInstance();
        instance.add(6, -1);
        for (UsageStats next : ((UsageStatsManager) context.getSystemService(UsageStatsManager.class)).queryUsageStats(4, instance.getTimeInMillis(), System.currentTimeMillis())) {
            USAGE_STATS.put(next.getPackageName(), next);
        }
    }

    /* access modifiers changed from: private */
    public static /* synthetic */ int lambda$static$0(ApplicationsState.AppEntry appEntry, ApplicationsState.AppEntry appEntry2) {
        long j;
        Map<String, UsageStats> map = USAGE_STATS;
        UsageStats usageStats = map.get(appEntry.info.packageName);
        UsageStats usageStats2 = map.get(appEntry2.info.packageName);
        long j2 = 0;
        if (usageStats == null) {
            j = 0;
        } else {
            j = usageStats.getLastTimeVisible();
        }
        if (usageStats2 != null) {
            j2 = usageStats2.getLastTimeVisible();
        }
