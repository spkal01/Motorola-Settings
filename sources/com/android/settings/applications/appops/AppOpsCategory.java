...
public class AppOpsCategory extends ListFragment implements LoaderManager.LoaderCallbacks<List<AppOpsState.AppOpEntry>> {
    ...
    public static class AppListAdapter extends BaseAdapter {
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            boolean z = false;
            if (view == null) {
                view = this.mInflater.inflate(C1987R$layout.app_ops_item, viewGroup, false);
            }
            AppOpsState.AppOpEntry item = getItem(i);
            ((ImageView) view.findViewById(C1985R$id.app_icon)).setImageDrawable(item.getAppEntry().getIcon());
            ((TextView) view.findViewById(C1985R$id.app_name)).setText(item.getAppEntry().getLabel());
            ((TextView) view.findViewById(C1985R$id.op_name)).setText(item.getTimeText(this.mResources, false));
            view.findViewById(C1985R$id.op_time).setVisibility(8);
            Switch switchR = (Switch) view.findViewById(C1985R$id.op_switch);
