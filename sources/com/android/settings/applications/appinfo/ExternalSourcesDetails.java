...
public class ExternalSourcesDetails extends AppInfoWithHeader implements Preference.OnPreferenceChangeListener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        addPreferencesFromResource(C1994R$xml.external_sources_details);
    ...
    public static CharSequence getPreferenceSummary(Context context, ApplicationsState.AppEntry appEntry) {
        ...
        if ((userRestrictionSource & 1) != 0) {
            return context.getString(C1992R$string.disabled_by_admin);
        }
        if (userRestrictionSource != 0) {
            return context.getString(C1992R$string.disabled);
        }
    ...
    public boolean refreshUi() {
        ...
        if (Settings.Secure.getInt(getActivity().getContentResolver(), "secure_frp_mode", 0) == 1) {
            this.mSwitchPref.setEnabled(false);
            return true;
        } else if (this.mUserManager.hasBaseUserRestriction("no_install_unknown_sources", UserHandle.of(UserHandle.myUserId()))) {
            this.mSwitchPref.setChecked(false);
            this.mSwitchPref.setSummary(C1992R$string.disabled);
            this.mSwitchPref.setEnabled(false);
            return true;
        } else {
