...
import com.motorola.settings.application.MotoAttDiagnosticsUtility;
...
public class AppButtonsPreferenceController extends BasePreferenceController implements PreferenceControllerMixin, LifecycleObserver, OnResume, OnDestroy, ApplicationsState.Callbacks {
    ...
    private MotoAttDiagnosticsUtility mMotoAttDiagnosticsUtility;
    ...
    public AppButtonsPreferenceController(SettingsActivity settingsActivity, InstrumentedPreferenceFragment instrumentedPreferenceFragment, Lifecycle lifecycle, String str, ApplicationsState applicationsState, int i, int i2) {
        ...
        if (instrumentedPreferenceFragment instanceof ButtonActionDialogFragment.AppButtonsDialogListener) {
            ...
            if (!Build.IS_PRC_PRODUCT || !(instrumentedPreferenceFragment instanceof AppInfoDashboardFragment)) {
                this.mUserId = UserHandle.myUserId();
            } else {
                PackageInfo packageInfo = ((AppInfoDashboardFragment) instrumentedPreferenceFragment).getPackageInfo();
                if (packageInfo == null || !CloneAppUtils.isCloneAppByUid(packageInfo.applicationInfo.uid)) {
                    this.mUserId = UserHandle.myUserId();
                } else {
                    this.mUserId = UserHandle.getUserId(packageInfo.applicationInfo.uid);
                }
            }
            ...
            if (str != null) {
                ...
                if (MotoAttDiagnosticsUtility.isIqiApp(str)) {
                    MotoAttDiagnosticsUtility motoAttDiagnosticsUtility = new MotoAttDiagnosticsUtility(this.mContext, new MotoAttDiagnosticsUtility.Listener() {
                        public void forceStop(String str) {
                            AppButtonsPreferenceController.this.forceStopPackage(str);
                        }
                    });
                    this.mMotoAttDiagnosticsUtility = motoAttDiagnosticsUtility;
                    lifecycle.addObserver(motoAttDiagnosticsUtility);
                    return;
                }
                return;
            }
    ...
    private class ForceStopButtonListener implements View.OnClickListener {
        ...
        public void onClick(View view) {
            AppButtonsPreferenceController.this.mMetricsFeatureProvider.action((Context) AppButtonsPreferenceController.this.mActivity, 1775, (Pair<Integer, Object>[]) new Pair[0]);
            PackageManager access$1500 = AppButtonsPreferenceController.this.mPm;
            AppButtonsPreferenceController appButtonsPreferenceController = AppButtonsPreferenceController.this;
            if (access$1500.isPackageStateProtected(appButtonsPreferenceController.mAppEntry.info.packageName, appButtonsPreferenceController.mUserId)) {
    ...
    public void handleDialogClick(int i) {
        if (i == 0) {
            ...
            MotoAttDiagnosticsUtility motoAttDiagnosticsUtility = this.mMotoAttDiagnosticsUtility;
            if (motoAttDiagnosticsUtility != null) {
                motoAttDiagnosticsUtility.handleDisablePackage();
            } else {
                AsyncTask.execute(new DisableChangerRunnable(this.mPm, this.mAppEntry.info.packageName, 3));
            }
        } else if (i == 1) {
            ...
            MotoAttDiagnosticsUtility motoAttDiagnosticsUtility2 = this.mMotoAttDiagnosticsUtility;
            if (motoAttDiagnosticsUtility2 != null) {
                motoAttDiagnosticsUtility2.handleForceStopPackage();
            } else {
    ...
    public void retrieveAppEntry() {
        ...
        if (entry != null) {
            try {
                if (!Build.IS_PRC_PRODUCT || !CloneAppUtils.isCloneAppByAppEntry(entry)) {
                    this.mPackageInfo = this.mPm.getPackageInfo(this.mAppEntry.info.packageName, 4198976);
                } else {
                    this.mPackageInfo = this.mPm.getPackageInfoAsUser(this.mAppEntry.info.packageName, 4672, UserHandle.getUserId(this.mAppEntry.info.uid));
                }
    ...
    public void updateForceStopButton() {
        ...
        } else if (InstalledAppUtils.get(this.mContext).isPackageNonForceStoppable(this.mPackageInfo.packageName)) {
            Log.w(TAG, "Non-force stoppable app " + this.mPackageInfo.packageName);
            updateForceStopButtonInner(false);
    ...
    public void uninstallPkg(String str, boolean z, boolean z2) {
        ...
        if (!z && Build.IS_PRC_PRODUCT && (packageInfo = this.mPackageInfo) != null && (applicationInfo = packageInfo.applicationInfo) != null && CloneAppUtils.isCloneAppByUid(applicationInfo.uid)) {
            intent.putExtra("android.intent.extra.USER", UserHandle.getUserHandleForUid(this.mPackageInfo.applicationInfo.uid));
        }
    ...
    public void forceStopPackage(String str) {
        ...
        if (!Build.IS_PRC_PRODUCT || (packageInfo = this.mPackageInfo) == null || (applicationInfo = packageInfo.applicationInfo) == null || !CloneAppUtils.isCloneAppByUid(applicationInfo.uid)) {
            activityManager.forceStopPackage(str);
        } else {
            activityManager.forceStopPackageAsUser(str, UserHandle.getUserId(this.mPackageInfo.applicationInfo.uid));
        }
    ...
    public boolean handleDisableable() {
        boolean isPackageNonDisableable = InstalledAppUtils.get(this.mContext).isPackageNonDisableable(this.mAppEntry.info.packageName);
        if (this.mHomePackages.contains(this.mAppEntry.info.packageName) || isPackageNonDisableable || isSystemPackage(this.mActivity.getResources(), this.mPm, this.mPackageInfo)) {
            this.mButtonsPref.setButton2Text(C1992R$string.disable_text).setButton2Icon(C1983R$drawable.ic_settings_disable);
            return false;
        } else if (!this.mAppEntry.info.enabled || isDisabledUntilUsed()) {
            this.mButtonsPref.setButton2Text(C1992R$string.enable_text).setButton2Icon(C1983R$drawable.ic_settings_enable);
            MotoAttDiagnosticsUtility motoAttDiagnosticsUtility = this.mMotoAttDiagnosticsUtility;
            if (motoAttDiagnosticsUtility == null) {
                return true;
            }
            motoAttDiagnosticsUtility.setDisableInProgress(false);
            return true;
        } else {
            this.mButtonsPref.setButton2Text(C1992R$string.disable_text).setButton2Icon(C1983R$drawable.ic_settings_disable);
    ...
    private void initButtonPreference() {
        this.mButtonsPref = ((ActionButtonsPreference) this.mScreen.findPreference(KEY_ACTION_BUTTONS)).setButton1Text(C1992R$string.launch_instant_app).setButton1Icon(C1983R$drawable.ic_settings_open).setButton1OnClickListener(new AppButtonsPreferenceController$$ExternalSyntheticLambda0(this)).setButton2Text(C1992R$string.uninstall_text).setButton2Icon(C1983R$drawable.ic_settings_delete).setButton2OnClickListener(new UninstallAndDisableButtonListener()).setButton3Text(C1992R$string.force_stop).setButton3Icon(C1983R$drawable.ic_settings_force_stop).setButton3OnClickListener(new ForceStopButtonListener()).setButton3Enabled(false);
    }
