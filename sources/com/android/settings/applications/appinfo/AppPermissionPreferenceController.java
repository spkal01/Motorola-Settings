...
public class AppPermissionPreferenceController extends AppInfoPreferenceControllerBase implements LifecycleObserver, OnStart, OnStop {
    ...
    final PermissionsSummaryHelper.PermissionsResultCallback mPermissionCallback = new PermissionsSummaryHelper.PermissionsResultCallback() {
        public void onPermissionSummaryResult(int i, int i2, int i3, List<CharSequence> list) {
            ...
            if (i2 == 0) {
                str = resources.getString(C1992R$string.runtime_permissions_summary_no_permissions_requested);
                AppPermissionPreferenceController.this.mPreference.setEnabled(false);
            } else {
                ArrayList arrayList = new ArrayList(list);
                if (i3 > 0) {
                    arrayList.add(resources.getQuantityString(C1990R$plurals.runtime_permissions_additional_count, i3, new Object[]{Integer.valueOf(i3)}));
                }
                if (arrayList.size() == 0) {
                    str = resources.getString(C1992R$string.runtime_permissions_summary_no_permissions_granted);
                } else {
    ...
    public void updateState(Preference preference) {
        if (CloneAppUtils.isCloneAppByAppEntry(this.mParent.getAppEntry())) {
            try {
                PermissionsSummaryHelper.getPermissionSummary(this.mContext.createContextAsUser(UserHandle.getUserHandleForUid(this.mParent.getAppEntry().info.uid), 0), this.mPackageName, this.mPermissionCallback);
            } catch (Exception e) {
                Log.e(TAG, "getPermissionSummary exception" + e.getMessage());
            }
        } else {
    ...
    private void startManagePermissionsActivity() {
        ...
        if (appEntry != null && appEntry.info != null) {
            ...
            intent.putExtra("android.intent.extra.PACKAGE_NAME", appEntry.info.packageName);
            if (Build.IS_PRC_PRODUCT) {
                intent.putExtra("android.intent.extra.UID", appEntry.info.uid);
                intent.putExtra("android.intent.extra.USER", UserHandle.getUserHandleForUid(appEntry.info.uid));
            }
