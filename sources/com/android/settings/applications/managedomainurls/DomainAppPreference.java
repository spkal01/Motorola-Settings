...
public class DomainAppPreference extends AppPreference {
    ...
    private CharSequence getDomainsSummary(String str) {
        if (this.mPm.getIntentVerificationStatusAsUser(str, UserHandle.myUserId()) == 3) {
            return getContext().getText(C1992R$string.domain_urls_summary_none);
        }
        ...
        if (handledDomains.size() == 1) {
            return getContext().getString(C1992R$string.domain_urls_summary_one, new Object[]{handledDomains.valueAt(0)});
        }
        return getContext().getString(C1992R$string.domain_urls_summary_some, new Object[]{handledDomains.valueAt(0)});
    }
}
