...
public class ProcessStatsDetail extends SettingsPreferenceFragment {
    ...
    private void createDetails() {
        addPreferencesFromResource(C1994R$xml.app_memory_settings);
        ...
        Formatter.BytesResult formatBytes = MotoDataUsageUtils.formatBytes(activity, (long) d3, 1);
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater menuInflater) {
        this.mForceStop = menu.add(0, 1, 0, C1992R$string.force_stop);
        checkForceStop();
    }
    ...
    private void fillProcessesSection() {
        ...
        for (int i2 = 0; i2 < arrayList.size(); i2++) {
            ...
            preference.setSummary((CharSequence) getString(C1992R$string.memory_use_running_format, Formatter.formatShortFileSize(getActivity(), Math.max((long) (d * d2), (long) (procStatsEntry2.mBgWeight * d2))), ProcStatsPackageEntry.getFrequency(((float) max) / ((float) this.mTotalTime), getActivity())));
    ...
    private void showStopServiceDialog(final String str, final String str2) {
        new AlertDialog.Builder(getActivity()).setTitle(C1992R$string.runningservicedetails_stop_dlg_title).setMessage(C1992R$string.runningservicedetails_stop_dlg_text).setPositiveButton(C1992R$string.dlg_ok, (DialogInterface.OnClickListener) new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialogInterface, int i) {
                ProcessStatsDetail.this.doStopService(str, str2);
            }
        }).setNegativeButton(C1992R$string.dlg_cancel, (DialogInterface.OnClickListener) null).show();
    }
        ...
        if (this.mForceStop != null) {
            ...
            for (int i = 0; i < this.mApp.mEntries.size(); i++) {
                ...
                for (int i2 = 0; i2 < procStatsEntry.mPackages.size(); i2++) {
                    ...
                    if (this.mDpm.packageHasActiveAdmins(str) || InstalledAppUtils.get(getContext()).isPackageNonForceStoppable(str)) {
