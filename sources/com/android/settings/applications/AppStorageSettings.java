...
public class AppStorageSettings extends AppInfoWithHeader implements View.OnClickListener, ApplicationsState.Callbacks, DialogInterface.OnClickListener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        addPreferencesFromResource(C1994R$xml.app_storage_settings);
        setupViews();
        initMoveDialog();
    }
    ...
    private void setupViews() {
        ...
        button.setText(C1992R$string.change);
        this.mChangeStorageButton.setOnClickListener(this);
        this.mButtonsPref.setButton2Text(C1992R$string.clear_cache_btn_text).setButton2Icon(C1983R$drawable.ic_settings_delete);
        ...
        button2.setText(C1992R$string.clear_uri_btn_text);
    ...
    public void handleClearCacheClick() {
        if (this.mAppsControlDisallowedAdmin == null || this.mAppsControlDisallowedBySystem) {
            ...
            if (!Build.IS_PRC_PRODUCT || !UserHandle.isAppCloneUser(this.mUserId)) {
                this.mPm.deleteApplicationCacheFiles(this.mPackageName, this.mClearCacheObserver);
            } else {
                this.mPm.deleteApplicationCacheFilesAsUser(this.mPackageName, this.mUserId, this.mClearCacheObserver);
            }
        } else {
    ...
    private void initDataButtons() {
        ...
        if ((z2 || !z3) && z) {
            if (z2) {
                this.mButtonsPref.setButton1Text(C1992R$string.manage_space_text);
            } else {
                this.mButtonsPref.setButton1Text(C1992R$string.clear_user_data_text);
            }
            this.mButtonsPref.setButton1Icon(C1983R$drawable.ic_settings_delete).setButton1OnClickListener(new AppStorageSettings$$ExternalSyntheticLambda0(this));
        } else {
            this.mButtonsPref.setButton1Text(C1992R$string.clear_user_data_text).setButton1Icon(C1983R$drawable.ic_settings_delete).setButton1Enabled(false);
    ...
    private void initMoveDialog() {
        ...
        if (packageCandidateVolumes.size() > 1) {
            ...
            this.mDialogBuilder = new AlertDialog.Builder(getContext()).setTitle(C1992R$string.change_storage).setSingleChoiceItems(charSequenceArr, i, (DialogInterface.OnClickListener) this).setNegativeButton(C1992R$string.cancel, (DialogInterface.OnClickListener) null);
            return;
        }
    ...
    public void initiateClearUserData() {
        ...
        String str2 = TAG;
        Log.i(str2, "Clearing user data for package : " + str + ", mUserId: " + this.mUserId);
        ...
        try {
            if (!Build.IS_PRC_PRODUCT || !UserHandle.isAppCloneUser(this.mUserId)) {
                z = activityManager.clearApplicationUserData(str, this.mClearDataObserver);
            } else {
                z = ActivityManager.getService().clearApplicationUserData(str, false, this.mClearDataObserver, this.mUserId);
            }
        ...
        this.mButtonsPref.setButton1Text(C1992R$string.recompute_size);
    }
    ...
    public void processClearMsg(Message message) {
        int i = message.arg1;
        String str = this.mAppEntry.info.packageName;
        this.mButtonsPref.setButton1Text(C1992R$string.clear_user_data_text).setButton1Icon(C1983R$drawable.ic_settings_delete);
        if (i == 1) {
            String str2 = TAG;
    ...
    private void refreshGrantedUriPermissions() {
        ...
        for (Map.Entry entry : treeMap.entrySet()) {
            ...
            preference.setSummary((CharSequence) getPrefContext().getResources().getQuantityString(C1990R$plurals.uri_permissions_text, i, new Object[]{Integer.valueOf(i)}));
            preference.setSelectable(false);
            preference.setLayoutResource(C1987R$layout.horizontal_preference);
    ...
    public AlertDialog createDialog(int i, int i2) {
        if (i == 1) {
            return new AlertDialog.Builder(getActivity()).setTitle(getActivity().getText(C1992R$string.clear_data_dlg_title)).setMessage(getActivity().getText(C1992R$string.clear_data_dlg_text)).setPositiveButton(C1992R$string.dlg_ok, (DialogInterface.OnClickListener) new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialogInterface, int i) {
                    AppStorageSettings.this.initiateClearUserData();
                }
            }).setNegativeButton(C1992R$string.dlg_cancel, (DialogInterface.OnClickListener) null).create();
        }
        if (i != 2) {
            return null;
        }
        return new AlertDialog.Builder(getActivity()).setTitle(getActivity().getText(C1992R$string.clear_user_data_text)).setMessage(getActivity().getText(C1992R$string.clear_failed_dlg_text)).setNeutralButton(C1992R$string.dlg_ok, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialogInterface, int i) {
