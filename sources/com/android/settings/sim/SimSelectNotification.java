...
public class SimSelectNotification extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        } else if (!context.getResources().getBoolean(C1980R$bool.disable_sim_selection_popup_all) && !hasMSimSettings(context)) {
    ...
    private void onEnableMmsDataRequest(Context context, Intent intent) {
        ...
        if (intExtra2 == 0) {
            charSequence = context.getResources().getText(C1992R$string.enable_receiving_mms_notification_title);
        } else if (intExtra2 == 1) {
            charSequence = context.getResources().getText(C1992R$string.enable_sending_mms_notification_title);
        } else {
        ...
        String string = context.getResources().getString(C1992R$string.enable_mms_notification_summary, new Object[]{SubscriptionUtil.getUniqueSubscriptionDisplayName(activeSubscriptionInfo, context)});
    ...
    private void createSimSelectNotification(Context context) {
        ...
        NotificationChannel notificationChannel = new NotificationChannel(SIM_SELECT_NOTIFICATION_CHANNEL, resources.getText(C1992R$string.sim_selection_channel_title), 2);
        Notification.Builder autoCancel = new Notification.Builder(context, SIM_SELECT_NOTIFICATION_CHANNEL).setSmallIcon(C1983R$drawable.ic_sim_alert).setColor(context.getColor(C1981R$color.sim_noitification)).setContentTitle(resources.getText(C1992R$string.sim_notification_title)).setContentText(resources.getText(C1992R$string.sim_notification_summary)).setAutoCancel(true);
    ...
    private void createEnableMmsNotification(Context context, CharSequence charSequence, CharSequence charSequence2, int i) {
        NotificationChannel notificationChannel = new NotificationChannel(ENABLE_MMS_NOTIFICATION_CHANNEL, context.getResources().getText(C1992R$string.enable_mms_notification_channel_title), 4);
        Notification.Builder autoCancel = new Notification.Builder(context, ENABLE_MMS_NOTIFICATION_CHANNEL).setSmallIcon(C1983R$drawable.ic_settings_24dp).setColor(context.getColor(C1981R$color.sim_noitification)).setContentTitle(charSequence).setContentText(charSequence2).setStyle(new Notification.BigTextStyle().bigText(charSequence2)).setAutoCancel(true);
    ...
    private void createSimCombinationWarningNotification(Context context, Intent intent) {
        ...
        if (stringExtra != null) {
            String string = resources.getString(C1992R$string.dual_cdma_sim_warning_notification_summary, new Object[]{stringExtra});
            NotificationChannel notificationChannel = new NotificationChannel(SIM_WARNING_NOTIFICATION_CHANNEL, resources.getText(C1992R$string.dual_cdma_sim_warning_notification_channel_title), 4);
            Notification.Builder autoCancel = new Notification.Builder(context, SIM_WARNING_NOTIFICATION_CHANNEL).setSmallIcon(C1983R$drawable.ic_sim_alert).setColor(context.getColor(C1981R$color.sim_noitification)).setContentTitle(resources.getText(C1992R$string.sim_combination_warning_notification_title)).setContentText(string).setStyle(new Notification.BigTextStyle().bigText(string)).setAutoCancel(true);
    ...
    public static boolean hasMSimSettings(Context context) {
        ApplicationInfo applicationInfo;
        try {
            PackageManager packageManager = context.getPackageManager();
            if (packageManager == null || (applicationInfo = packageManager.getApplicationInfo("com.motorola.msimsettings", 1048576)) == null || !applicationInfo.enabled) {
                return false;
            }
            return true;
        } catch (PackageManager.NameNotFoundException unused) {
            Log.w("SimSelectNotification", "App name not found for com.motorola.msimsettings");
        }
        return false;
    }
}
