...
public class SimActivationNotifier {
    ...
    public SimActivationNotifier(Context context) {
        ...
        notificationManager.createNotificationChannel(new NotificationChannel("sim_setup", context.getString(C1992R$string.sim_setup_channel_id), 4));
        notificationManager.createNotificationChannel(new NotificationChannel("carrier_switching", context.getString(C1992R$string.sim_switch_channel_id), 4));
    }
    ...
    public void sendSwitchedToRemovableSlotNotification() {
        ...
        if (TextUtils.isEmpty(activeCarrierName)) {
            str = this.mContext.getString(C1992R$string.switch_to_removable_notification_no_carrier_name);
        } else {
            str = this.mContext.getString(C1992R$string.switch_to_removable_notification, new Object[]{activeCarrierName});
        }
        this.mNotificationManager.notify(2, new Notification.Builder(this.mContext, "carrier_switching").setContentTitle(str).setContentText(this.mContext.getString(C1992R$string.network_changed_notification_text)).setContentIntent(pendingIntent).setSmallIcon(C1983R$drawable.ic_sim_alert).setColor(this.mContext.getResources().getColor(C1981R$color.homepage_generic_icon_background, (Resources.Theme) null)).setAutoCancel(true).build());
    }

    public void sendEnableDsdsNotification() {
        Intent intent = new Intent(this.mContext, Settings.MobileNetworkListActivity.class);
        this.mNotificationManager.notify(1, new Notification.Builder(this.mContext, "sim_setup").setContentTitle(this.mContext.getString(C1992R$string.dsds_notification_after_suw_title)).setContentText(this.mContext.getString(C1992R$string.dsds_notification_after_suw_text)).setContentIntent(TaskStackBuilder.create(this.mContext).addNextIntentWithParentStack(intent).addNextIntent(new Intent(this.mContext, DsdsDialogActivity.class)).getPendingIntent(0, 201326592)).setSmallIcon(C1983R$drawable.ic_sim_alert).setAutoCancel(true).build());
    }
