...
public class DataUsagePreference extends Preference implements TemplatePreference {
    ...
    public void setTemplate(NetworkTemplate networkTemplate, int i, TemplatePreference.NetworkServices networkServices) {
        ...
        if (this.mTemplate.isMatchRuleMobile()) {
            setTitle(C1992R$string.app_cellular_data_usage);
        } else {
            NetworkTemplate refTemplate = MotoDataUsageUtils.getRefTemplate(getContext(), this.mTemplate);
            DataUsageController.DataUsageInfo dataUsageInfo = dataUsageController.getDataUsageInfo(this.mTemplate, (refTemplate == null || networkServices == null) ? null : networkServices.mPolicyEditor.getPolicy(refTemplate));
            ...
            setSummary((CharSequence) getContext().getString(C1992R$string.data_usage_template, new Object[]{DataUsageUtils.formatDataUsage(getContext(), dataUsageInfo.usageLevel), dataUsageInfo.period}));
        }
    ...
    public Intent getIntent() {
        ...
        if (this.mTemplate.isMatchRuleMobile()) {
            sourceMetricsCategory.setTitleRes(C1992R$string.app_cellular_data_usage);
        } else {
