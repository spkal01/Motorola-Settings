...
public class ChartDataUsagePreference extends Preference {
    ...
    public ChartDataUsagePreference(Context context, AttributeSet attributeSet) {
        ...
        setLayoutResource(C1987R$layout.data_usage_graph);
    }

    public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
        super.onBindViewHolder(preferenceViewHolder);
        UsageView usageView = (UsageView) preferenceViewHolder.findViewById(C1985R$id.data_usage);
    ...
    private void setupContentDescription(UsageView usageView, List<NetworkCycleData> list) {
        ...
        sb.append(this.mResources.getString(C1992R$string.data_usage_chart_brief_content_description, new Object[]{formatDateTime, formatDateTime2}));
        if (list == null || list.isEmpty()) {
            sb.append(this.mResources.getString(C1992R$string.data_usage_chart_no_data_content_description));
    ...
    public List<DataUsageSummaryNode> getDensedStatsData(List<NetworkCycleData> list) {
        ...
        for (NetworkCycleData next : list) {
            j += next.getTotalUsage();
            arrayList.add(new DataUsageSummaryNode(next.getStartTime(), next.getEndTime(), max > 0 ? (int) ((100 * j) / max) : 0));
        }
    ...
    private void bindNetworkPolicy(UsageView usageView, NetworkPolicy networkPolicy, int i) {
        ...
        if (networkPolicy != null) {
            ...
            if (j != -1) {
                i2 = this.mLimitColor;
                charSequenceArr[2] = getLabel(j, C1992R$string.data_usage_sweep_limit, i2);
            } else {
                i2 = 0;
            }
            ...
            if (j2 != -1) {
                ...
                charSequenceArr[1] = getLabel(networkPolicy.warningBytes, C1992R$string.data_usage_sweep_warning, i3);
            }
    ...
    private CharSequence getLabel(long j, int i, int i2) {
        Formatter.BytesResult formatBytes = MotoDataUsageUtils.formatBytes(getContext(), j, 9);
