...
public class DataUsageSummaryPreferenceController extends TelephonyBasePreferenceController implements PreferenceControllerMixin, LifecycleObserver, OnStart {
    ...
    private void updateConfiguration(Context context, int i, SubscriptionInfo subscriptionInfo) {
        ...
        if (subscriptionInfo != null) {
            this.mDataUsageTemplate = C1992R$string.cell_data_template;
            this.mDefaultTemplate = DataUsageLib.getMobileTemplate(context, i);
        } else if (DataUsageUtils.hasWifiRadio(context)) {
            this.mDataUsageTemplate = C1992R$string.wifi_data_template;
            this.mDefaultTemplate = NetworkTemplate.buildTemplateWifi(NetworkTemplate.WIFI_NETWORKID_ALL, (String) null);
        } else {
            this.mDataUsageTemplate = C1992R$string.ethernet_data_template;
    ...
    public void updateState(Preference preference) {
        ...
        if (dataUsageInfo == null) {
            ...
            dataUsageSummaryPreference.setEnabled(false);
            return;
        }
        ...
        if (subscriptionInfo != null) {
            ...
            if (j2 > 0 && dataUsageInfo.limitLevel > 0) {
                dataUsageSummaryPreference.setLimitInfo(TextUtils.expandTemplate(this.mContext.getText(C1992R$string.cell_data_warning_and_limit), new CharSequence[]{DataUsageUtils.formatDataUsage(this.mContext, dataUsageInfo.warningLevel), DataUsageUtils.formatDataUsage(this.mContext, dataUsageInfo.limitLevel)}));
            } else if (j2 > 0) {
                dataUsageSummaryPreference.setLimitInfo(TextUtils.expandTemplate(this.mContext.getText(C1992R$string.cell_data_warning), new CharSequence[]{DataUsageUtils.formatDataUsage(this.mContext, dataUsageInfo.warningLevel)}));
            } else if (dataUsageInfo.limitLevel > 0) {
                dataUsageSummaryPreference.setLimitInfo(TextUtils.expandTemplate(this.mContext.getText(C1992R$string.cell_data_limit), new CharSequence[]{DataUsageUtils.formatDataUsage(this.mContext, dataUsageInfo.limitLevel)}));
            } else {
