...
public final class DataUsageUtils extends com.android.settingslib.net.DataUsageUtils {
    public static CharSequence formatDataUsage(Context context, long j) {
        if (MotoDataUsageUtils.isFeature4232VzwDataUnitsEnabled(context)) {
            return MotoDataUsageUtils.formatDataUsageForVzw(context, j, 8);
        }
        Formatter.BytesResult formatBytes = Formatter.formatBytes(context.getResources(), j, 8);
        return BidiFormatter.getInstance().unicodeWrap(context.getString(17040285, new Object[]{formatBytes.value, formatBytes.units}));
    }
