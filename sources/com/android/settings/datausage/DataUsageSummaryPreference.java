...
public class DataUsageSummaryPreference extends Preference {
    ...
    public DataUsageSummaryPreference(Context context, AttributeSet attributeSet) {
        ...
        setLayoutResource(C1987R$layout.data_usage_summary_preference);
    }
    ...
    public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
        ...
        if (z && this.mSingleWifi) {
            ...
            usageTitle.setText(C1992R$string.data_usage_wifi_title);
            ...
            launchButton.setText(C1992R$string.launch_wifi_text);
    ...
    static void launchWifiDataUsage(Context context) {
        ...
        sourceMetricsCategory.setTitleRes(C1992R$string.wifi_data_usage);
    ...
    private void updateDataUsageLabels(PreferenceViewHolder preferenceViewHolder) {
        ...
        Formatter.BytesResult formatBytes = MotoDataUsageUtils.formatBytes(getContext(), this.mDataplanUse, 10);
        SpannableString spannableString = new SpannableString(formatBytes.value);
        spannableString.setSpan(new AbsoluteSizeSpan(getContext().getResources().getDimensionPixelSize(C1982R$dimen.usage_number_text_size)), 0, spannableString.length(), 33);
        dataUsed.setText(TextUtils.expandTemplate(getContext().getText(C1992R$string.data_used_formatted), new CharSequence[]{spannableString, formatBytes.units}));
        ...
        if (j >= 0) {
            dataRemaining.setText(TextUtils.expandTemplate(getContext().getText(C1992R$string.data_remaining), new CharSequence[]{DataUsageUtils.formatDataUsage(getContext(), j)}));
            dataRemaining.setTextColor(Utils.getColorAttr(getContext(), 16843829));
        } else {
            dataRemaining.setText(TextUtils.expandTemplate(getContext().getText(C1992R$string.data_overusage), new CharSequence[]{DataUsageUtils.formatDataUsage(getContext(), -j)}));
    ...
    private void updateCycleTimeText(PreferenceViewHolder preferenceViewHolder) {
        ...
        if (currentTimeMillis <= 0) {
            cycleTime.setText(getContext().getString(C1992R$string.billing_cycle_none_left));
            return;
        }
        int i = (int) (currentTimeMillis / MILLIS_IN_A_DAY);
        if (i < 1) {
            str = getContext().getString(C1992R$string.billing_cycle_less_than_one_day_left);
        } else {
            str = getContext().getResources().getQuantityString(C1990R$plurals.billing_cycle_days_left, i, new Object[]{Integer.valueOf(i)});
        }
    ...
    private void updateCarrierInfo(TextView textView) {
        ...
        if (calculateTruncatedUpdateAge != 0) {
            if (this.mCarrierName != null) {
                i2 = C1992R$string.carrier_and_update_text;
            } else {
                i2 = C1992R$string.no_carrier_update_text;
            }
            i = i2;
            ...
            i = C1992R$string.carrier_and_update_now_text;
        } else {
            i = C1992R$string.no_carrier_update_now_text;
        }
    ...
    public TextView getUsageTitle(PreferenceViewHolder preferenceViewHolder) {
        return (TextView) preferenceViewHolder.findViewById(C1985R$id.usage_title);
    }

    /* access modifiers changed from: protected */
    public TextView getCycleTime(PreferenceViewHolder preferenceViewHolder) {
        return (TextView) preferenceViewHolder.findViewById(C1985R$id.cycle_left_time);
    }

    /* access modifiers changed from: protected */
    public TextView getCarrierInfo(PreferenceViewHolder preferenceViewHolder) {
        return (TextView) preferenceViewHolder.findViewById(C1985R$id.carrier_and_update);
    }

    /* access modifiers changed from: protected */
    public TextView getDataLimits(PreferenceViewHolder preferenceViewHolder) {
        return (TextView) preferenceViewHolder.findViewById(C1985R$id.data_limits);
    }

    /* access modifiers changed from: protected */
    public TextView getDataUsed(PreferenceViewHolder preferenceViewHolder) {
        return (TextView) preferenceViewHolder.findViewById(C1985R$id.data_usage_view);
    }

    /* access modifiers changed from: protected */
    public TextView getDataRemaining(PreferenceViewHolder preferenceViewHolder) {
        return (TextView) preferenceViewHolder.findViewById(C1985R$id.data_remaining_view);
    }

    /* access modifiers changed from: protected */
    public Button getLaunchButton(PreferenceViewHolder preferenceViewHolder) {
        return (Button) preferenceViewHolder.findViewById(C1985R$id.launch_mdp_app_button);
    }

    /* access modifiers changed from: protected */
    public LinearLayout getLabelBar(PreferenceViewHolder preferenceViewHolder) {
        return (LinearLayout) preferenceViewHolder.findViewById(C1985R$id.label_bar);
    }
    ...
    public ProgressBar getProgressBar(PreferenceViewHolder preferenceViewHolder) {
        return (ProgressBar) preferenceViewHolder.findViewById(C1985R$id.determinateBar);
    }

    /* access modifiers changed from: protected */
    public MeasurableLinearLayout getLayout(PreferenceViewHolder preferenceViewHolder) {
        return (MeasurableLinearLayout) preferenceViewHolder.findViewById(C1985R$id.usage_layout);
    }
}
