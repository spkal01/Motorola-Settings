...
public class DataUsageSummary extends DataUsageBaseFragment implements DataUsageEditController {
    ...
    public int getHelpResource() {
        return C1992R$string.help_url_data_usage;
    }
    ...
    private void addMobileSection(int i, SubscriptionInfo subscriptionInfo) {
        TemplatePreferenceCategory templatePreferenceCategory = (TemplatePreferenceCategory) inflatePreferences(C1994R$xml.data_usage_cellular);
    ...
    public void addWifiSection() {
        ((TemplatePreferenceCategory) inflatePreferences(C1994R$xml.data_usage_wifi)).setTemplate(NetworkTemplate.buildTemplateWifi(NetworkTemplate.WIFI_NETWORKID_ALL, (String) null), 0, this.services);
    }

    private void addEthernetSection() {
        ((TemplatePreferenceCategory) inflatePreferences(C1994R$xml.data_usage_ethernet)).setTemplate(NetworkTemplate.buildTemplateEthernet(), 0, this.services);
    }
    ...
    static CharSequence formatUsage(Context context, String str, long j, float f, float f2) {
        Formatter.BytesResult formatBytes = MotoDataUsageUtils.formatBytes(context, j, 10);
        ...
        CharSequence expandTemplate = TextUtils.expandTemplate(new SpannableString(context.getString(17040285).replace("%1$s", "^1").replace("%2$s", "^2")), new CharSequence[]{spannableString, formatBytes.units});
