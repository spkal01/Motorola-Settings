...
public class UnrestrictedDataAccessPreference extends AppSwitchPreference implements DataSaverBackend.Listener {
    ...
    public UnrestrictedDataAccessPreference(Context context, ApplicationsState.AppEntry appEntry, ApplicationsState applicationsState, DataSaverBackend dataSaverBackend, DashboardFragment dashboardFragment) {
        super(context);
        setWidgetLayoutResource(C1987R$layout.restricted_switch_widget);
    ...
    public void onClick() {
        if (this.mDataUsageState.isDataSaverDenylisted) {
            AppInfoDashboardFragment.startAppInfoFragment(AppDataUsage.class, C1992R$string.data_usage_app_summary_title, (Bundle) null, this.mParentFragment, this.mEntry);
        } else {
            super.onClick();
        }
    }
    ...
    public void onBindViewHolder(PreferenceViewHolder preferenceViewHolder) {
        ...
        preferenceViewHolder.findViewById(C1985R$id.restricted_icon).setVisibility(isDisabledByAdmin ? 0 : 8);
    ...
    public void updateState() {
        ...
        if (dataUsageState != null) {
            ...
            if (isDisabledByAdmin()) {
                setSummary(C1992R$string.disabled_by_admin);
            } else if (this.mDataUsageState.isDataSaverDenylisted) {
                setSummary(C1992R$string.restrict_background_blocklisted);
            } else {
