...
public class DataUsageList extends DataUsageBaseFragment implements MobileDataEnabledListener.Client {
    ...
    public void onViewCreated(View view, Bundle bundle) {
        ...
        View pinnedHeaderView = setPinnedHeaderView(C1987R$layout.apps_filter_spinner);
        this.mHeader = pinnedHeaderView;
        pinnedHeaderView.findViewById(C1985R$id.filter_settings).setOnClickListener(new DataUsageList$$ExternalSyntheticLambda0(this));
        Spinner spinner = (Spinner) this.mHeader.findViewById(C1985R$id.filter_spinner);
        ...
        LoadingViewController loadingViewController = new LoadingViewController(getView().findViewById(C1985R$id.loading_container), getListView());
    ...
    public /* synthetic */ void lambda$onViewCreated$0(View view) {
        ...
        new SubSettingLauncher(getContext()).setDestination(BillingCycleSettings.class.getName()).setTitleRes(C1992R$string.billing_cycle).setSourceMetricsCategory(getMetricsCategory()).setArguments(bundle).launch();
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater menuInflater) {
        if (MotoDataUsageUtils.isFeature4232VzwDataUnitsEnabled(getContext()) && UserManager.get(getContext()).isAdminUser()) {
            menuInflater.inflate(C1988R$menu.data_usage, menu);
            menu.findItem(C1985R$id.data_usage_menu_display_gb_mb).setVisible(true);
        }
        super.onCreateOptionsMenu(menu, menuInflater);
    }

    public void onPrepareOptionsMenu(Menu menu) {
        MenuItem findItem = menu.findItem(C1985R$id.data_usage_menu_display_gb_mb);
        if (findItem != null && findItem.isVisible()) {
            if (MotoDataUsageUtils.isShowingSizeInGB(getActivity())) {
                findItem.setTitle(C1992R$string.data_usage_menu_display_mb);
            } else {
                findItem.setTitle(C1992R$string.data_usage_menu_display_gb);
            }
        }
    }

    public boolean onOptionsItemSelected(MenuItem menuItem) {
        if (menuItem.getItemId() != C1985R$id.data_usage_menu_display_gb_mb) {
            return false;
        }
        MotoDataUsageUtils.setShowSizeInGB(getActivity(), !MotoDataUsageUtils.isShowingSizeInGB(getActivity()));
        getLoaderManager().restartLoader(2, buildArgs(this.mTemplate), this.mNetworkCycleDataCallbacks);
        updateBody();
        return true;
    }
    ...
    public int getPreferenceScreenResId() {
        return C1994R$xml.data_usage_list;
    }
    ...
    private void updateBody() {
        ...
        if (isAdded()) {
            ...
            int color = activity.getColor(C1981R$color.sim_noitification);
    ...
    public void updatePolicy() {
        NetworkTemplate refTemplate = MotoDataUsageUtils.getRefTemplate(getActivity(), this.mTemplate);
        NetworkPolicyEditor networkPolicyEditor = this.services.mPolicyEditor;
        if (refTemplate == null) {
            refTemplate = this.mTemplate;
        }
        NetworkPolicy policy = networkPolicyEditor.getPolicy(refTemplate);
        View findViewById = this.mHeader.findViewById(C1985R$id.filter_settings);
    ...
    public void updateDetailData() {
        ...
        this.mUsageAmount.setTitle((CharSequence) getString(C1992R$string.data_used_template, DataUsageUtils.formatDataUsage(getActivity(), (list == null || list.isEmpty()) ? 0 : this.mCycleData.get(this.mCycleSpinner.getSelectedItemPosition()).getTotalUsage())));
    }
    ...
    public void startAppDataUsage(AppItem appItem) {
        ...
        new SubSettingLauncher(getContext()).setDestination(AppDataUsage.class.getName()).setTitleRes(C1992R$string.data_usage_app_summary_title).setArguments(bundle).setSourceMetricsCategory(getMetricsCategory()).launch();
    }
