...
public final class ThemeHelper {
    ...
    public static boolean trySetDynamicColor(Context context) {
        if (!shouldApplyExtendedPartnerConfig(context)) {
            LOG.mo20057w("SetupWizard does not supports the extended partner configs.");
            return false;
        } else if (!isSetupWizardDynamicColorEnabled(context)) {
            LOG.mo20057w("SetupWizard does not support the dynamic color or supporting status unknown.");
            return false;
        } else {
            try {
                ...
                LOG.mo20057w("Error occurred on getting dynamic color theme.");
                return false;
            } catch (IllegalArgumentException e) {
                Logger logger = LOG;
                String message = e.getMessage();
                Objects.requireNonNull(message);
                logger.mo20053e(message);
                return false;
            }
        }
    }
