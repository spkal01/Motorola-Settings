...
public class PartnerCustomizationLayout extends TemplateLayout {
    ...
    public void onBeforeTemplateInflated(AttributeSet attributeSet, int i) {
        ...
        boolean allowCustomizationInSetupFlow = WizardManagerHelper.allowCustomizationInSetupFlow(this.activity.getIntent());
        TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attributeSet, R$styleable.SucPartnerCustomizationLayout, i, 0);
        int i2 = R$styleable.SucPartnerCustomizationLayout_sucUsePartnerResource;
        if (!obtainStyledAttributes.hasValue(i2)) {
            LOG.mo20053e("Attribute sucUsePartnerResource not found in " + this.activity.getComponentName());
        }
        if ((!isAnySetupWizard || allowCustomizationInSetupFlow) && !obtainStyledAttributes.getBoolean(i2, true)) {
            z = false;
        }
