...
public abstract class BaseStrokeContent implements BaseKeyframeAnimation.AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    /* renamed from: pm */
    private final PathMeasure f47pm = new PathMeasure();
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        C0474L.beginSection("StrokeContent#draw");
        if (Utils.hasZeroScaleAxis(matrix)) {
            C0474L.endSection("StrokeContent#draw");
            return;
        }
        ...
        for (int i2 = 0; i2 < this.pathGroups.size(); i2++) {
            ...
            if (pathGroup.trimPath != null) {
                ...
                C0474L.beginSection("StrokeContent#buildPath");
                this.path.reset();
                for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
                    this.path.addPath(((PathContent) pathGroup.paths.get(size)).getPath(), matrix);
                }
                C0474L.endSection("StrokeContent#buildPath");
                C0474L.beginSection("StrokeContent#drawPath");
                canvas.drawPath(this.path, this.paint);
                C0474L.endSection("StrokeContent#drawPath");
            }
        }
        C0474L.endSection("StrokeContent#draw");
    }
    ...
    private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        C0474L.beginSection("StrokeContent#applyTrimPath");
        if (pathGroup.trimPath == null) {
            C0474L.endSection("StrokeContent#applyTrimPath");
            return;
        }
        this.path.reset();
        for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
        ...
        this.f47pm.setPath(this.path, false);
        float length = this.f47pm.getLength();
        while (this.f47pm.nextContour()) {
            length += this.f47pm.getLength();
        }
        ...
        for (int size2 = pathGroup.paths.size() - 1; size2 >= 0; size2--) {
            ...
            this.f47pm.setPath(this.trimPathPath, false);
            float length2 = this.f47pm.getLength();
            float f2 = 1.0f;
    ...
    public void getBounds(RectF rectF, Matrix matrix, boolean z) {
        C0474L.beginSection("StrokeContent#getBounds");
        this.path.reset();
        ...
        C0474L.endSection("StrokeContent#getBounds");
    }

    private void applyDashPatternIfNeeded(Matrix matrix) {
        C0474L.beginSection("StrokeContent#applyDashPattern");
        if (this.dashPatternAnimations.isEmpty()) {
            C0474L.endSection("StrokeContent#applyDashPattern");
            return;
        }
